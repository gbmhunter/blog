...
Design criteria:
* 12VDC in
* Assume power supply is very noisy, with the voltage fluctuating at most 1Vp2p with frequency components from 10Hz to 100kHz.

Given the noise fluctuations can be up to 1Vp2p, we want the BJT transistor to be dropping more than 1Vp2p in it's steady-state conditions so that it can filter out noise if the voltage rail suddenly drops a volt. Let's aim for nominal 10V output.
...
Arbitrarily choosing R2 to be 10kR, this means that R1 must be:
