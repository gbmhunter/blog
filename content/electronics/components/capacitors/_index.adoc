...
Given the noise fluctuations can be up to 1Vp2p, we want the BJT transistor to be dropping more than 1Vp2p so that in input voltage is always larger than our output voltage, even with the ripple present on the input. Let's aim for nominal 10V output at no load.
...
This constrains the ratio of stem:[R1] and stem:[R2] as:
...
Our load resistance is stem:[100\Omega]. At 10V, this will be a current of stem:[100mA]. We need to make sure this current will not cause a significant extra voltage drop across stem:[R1], which would cause the output voltage to droop. Let's allow for stem:[100mV] of drop (i.e. stem:[V_{OUT} = 9.9V]) at stem:[100mA].

stem:[100mA] through the load will cause:
...
Now we have found stem:[R1] and stem:[R2], stem:[C1] is determined for us by the cut-off equation:
...
The forward current gain (stem:[\beta]) of the 2N2222 is simulated at 200. stem:[R3] is our load resistor of stem:[100\Omega]. We can now draw the finished schematic:
...
Looking at the "DC" gain, it is a value of stem:[-1.58dB]. At an input voltage of stem:[12V] this corresponds to an output voltage of stem:[12V \cdot 10^{\frac{-1.58}{20}} = 10.0V], bang on the stem:[10V] we were aiming for.

Our cut-off frequency should then be -3dV ontop of that, i.e. at stem:[-1.58dB - 3dB = -4.58dB]. Plotting across and then down does indeed give us a cut-off frequency of approx. stem:[10Hz]. The roll-off in the stop band is the same as a standard RC low-pass filter at stem:[-20dB/decade].
