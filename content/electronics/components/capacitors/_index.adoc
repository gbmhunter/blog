...
The effective capacitance seen by the circuit is the capacitance of C1 multiplied by the current gain stem:[\beta + 1] of the transistor:
...
Given the noise fluctuations can be up to 1Vp2p, we want the BJT transistor to be dropping more than 1Vp2p so that in input voltage is always larger than our output voltage, even with the ripple present on the input. Let's aim for nominal 10V output.
...
Somewhat arbitrarily choosing stem:[R2] to be stem:[10k\Omega], this means that stem:[R1] must be:

[stem]
++++
\begin{align}
R1 &= \frac{V_{IN} - V_{OUT}}{V_{OUT}} R2 \nonumber \\
   &= \frac{12V - 10V}{10V} 10k\Omega \nonumber \\
   &= 2k\Omega
\end{align}
++++

Now we have chosen stem:[R1] and stem:[R2], stem:[C1] is determined for us by the cut-off equation:

[stem]
++++
\begin{align}
C1 &= \frac{R1 + R2}{2\pi R1 R2 f_c} \nonumber \\
   &= \frac{2k\Omega + 10k\Omega}{2\pi*2k\Omega*10k\Omega*10Hz}\nonumber  \\
   &= 9.5uF \nonumber \\
   &= 10uF\ \text{(E12 series)}
\end{align}
++++

If stem:[C1] has turned out to be too small or too large, we could at this point go back and change stem:[R1] and stem:[R2].

The forward current gain (stem:[\beta]) of the 2N2222 is simulated at 200. stem:[R3] is our load resistor of stem:[100\Omega].
...
Looking at the "DC" gain, it is a value of stem:[-1.61dB]. At an input voltage of stem:[12V] this corresponds to an output voltage of stem:[12V \cdot 10^{\frac{-1.61}{20}} = 9.97V], pretty much bang on the stem:[10V] we were aiming for.
