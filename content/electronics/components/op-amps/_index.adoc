...
An op-amp can be combined with a regular diode to make an "ideal diode" (also called a _superdiode_<<bib-superdiode>> or _precision rectifier_), one that rectifies and has *no voltage drop*. Ideal diode circuits are useful when:
...
The basic op-amp based _sample and hold_ circuit is an extension of the <<_ideal_diodes, ideal diode>> circuit, but with an added capacitor on the output to maintain with the voltage when the input signal is removed.
