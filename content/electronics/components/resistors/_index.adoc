...
For most day-to-day applications, resistors can just be treated as if they have a resistance. However, in high frequency circuits, there are other parasitic elements to a resistor that you must consider. Typically, the main parasitics are modelled as a inductor in series with the resistance and a parallel capacitance, as shown in <<parasitic-model-resistor>>.

[[parasitic-model-resistor]]
.Basic parasitic model of a resistor.
image::parasitic-model-of-resistor.svg[width=600px]
