...
<<floating-buck-boost-converter-current-when-switch-closed>> shows the current paths through the floating buck-boost converter when the switch is closed. stem:[V_{IN}] provides a linearly increasing charging current to stem:[L1], whilst output capacitor stem:[C_{OUT}] provides current to the load. stem:[D1] is reverse-biased and so open-circuit.
