...
TIP: Because of the typically large stem:[R_S > 100\Omega] this circuit is only suitable for making low-power current sources up to the stem:[10mA-100mA] range. Also, because of the large uncertainty in stem:[V_{GS(off)}] (e.g. for the 2N5457, the datasheet says it is somewhere between stem:[-0.5V] and stem:[-6.0V]!<<bib-onsemi-2n5457-ds>>), these don't make very accurate current sources (without trimming). But they are stable!
...
--
Let's design a constant-current diode using the ubiquitous 2N5457 JFET (we'll choose the one from On Semi) for a constant current stem:[I_D] of stem:[1.0mA].

From the link:https://www.onsemi.com/pdf/datasheet/2n5457-d.pdf[OnSemi 2N5457 datasheet], we get the following values:
...
Plugging these values into stem:[Eq.\ \ref{eq:jfet-vgs}] we can work out the required gate-source voltage (with the common assumption that stem:[k = 2]):
...
Knowing the current stem:[I_D] through the resistor and the voltage stem:[V_{GS}] across it, we can find the resistance using stem:[Eq.\ \ref{eq:rs-vgs-id}]:
...
An important parameter determining the accuracy/stability of a constant-current diode is the link:/electronics/components/transistors/junction-gate-field-effect-transistor-jfets/#_output_conductance[output conductance] stem:[g_{oss}] of the JFET. This is the ratio of change in stem:[I_D] to change in stem:[V_{DS}]. This is a measure of how stable the constant-current source will be to changes in the voltage across it, which occur if either the supply voltage or load resistance changes. The lower the stem:[g_{oss}] the better (an ideal current source would have stem:[g_{oss} = 0], which is the equivalent to a resistance of stem:[\infty]).
...
The following schematic shows such a device which can control between 0-1A through the load (shown as stem:[R_{load}]):
...
.A schematic showing gate capacitance compensation circuitry on a op-amp based current sink using the LT1492<<bib-linear-lt1492-ds>>.
...
* [[[bib-onsemi-2n5457-ds, 4]]] On Semiconductor (now On Semi) (2010, Feb). _2N5457, 2N5458 JFETs: General Purpose N−Channel − Depletion (datasheet)_. Retrieved 2022-01-11, from https://www.onsemi.com/pdf/datasheet/2n5457-d.pdf.
* [[[bib-linear-lt1492-ds, 5]]] Linear Technology. _LT1492/LT1493: 5MHz, 3V/µs, Low Power Single Supply, Dual and Quad Precision Op Amps (datasheet)_. Retrieved 2020-12-25 from https://www.analog.com/media/en/technical-documentation/data-sheets/14923f.pdf.
