...
Represented by `AC`/`ACT` in part numbers, or `HC`/`HCT` for high-speed equivalents.
...
The 5400 series is the military rated version of the 7400 series. The 4000 series is the newer CMOS alternative to the 7400 TTL logic.
...
The `x` is a placeholder for the logic subfamily. For example, in `74LSxx` the `LS` represent the low-power Schottky subfamily. Logic subfamilies:

== Flip-Flops

Flip-flops (a.k.a. _latches_) are digital circuits which are able to store a single "bit" of information. They have two stable states (representing a digital `1` or `0`), and they can be made to change state by manipulating digital inputs. Flip-flops form the basic storage element in sequential logic.
