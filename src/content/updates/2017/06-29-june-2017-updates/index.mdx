---
authors: [gbmhunter]
date: 2017-06-29
lastUpdated: 2017-06-29
tags: [C, linear programming, Linux, memory, protobuf, serial ports, serialization, Valgrind]
title: June 2017 Updates
type: updates
---

* New [Linear Programming page](/mathematics/linear-programming) under Mathematics.

* New [Valgrind page](/programming/operating-systems/linux/programs/valgrind) under Programming->Operating Systems->Linux->Programs.  

<Image src={import('./_assets/valgrind-logo.jpg')} width="229px">The Valgrind logo.</Image>

* New [Linux Serial Ports Using C page](/programming/operating-systems/linux/linux-serial-ports-using-c-cpp) under Programming->Languages->C.  

<Image src={import('./_assets/amphenol-ltw-waterproof-serial-de-9-receptacle-connector.jpg')} width="300px"></Image>

* Updates to the [Protobuf page](/programming/serialization-formats/protobuf/), including code examples on serialization/deserialization.  

<Image src={import('./_assets/google-protobuf-logo.png')} width="310px">Icon for Google's protobuf library.</Image>
