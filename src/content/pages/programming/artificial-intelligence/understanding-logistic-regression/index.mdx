...
For someone new to categorization, these terms in the classification report can be confusing. Briefly[^scikit-classification-report]:

- The _precision_ is, of all the instances labelled positive, what percentage really were positive.
- The _recall_ is, of all the instances that really were positive, what percentage the classifier managed to find.
- The _f1-score_ is the [harmonic mean](/mathematics/statistics/the-three-classical-pythagorean-means/) of the precision and recall, combining both into a single number between 0 and 1.
- The _support_ is the number of actual occurrences of a class in a specific dataset.

See [Classification Metrics](/programming/artificial-intelligence/classification-metrics/) for the equations behind these, why accuracy on its own is misleading, and how precision and recall trade off against each other.
