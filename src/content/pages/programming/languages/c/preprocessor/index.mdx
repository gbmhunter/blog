...
They are also used to create clever assert and debug messages which can be built to automatically report the filename and line number of the code.
...
The general syntax for an object-style macro is:

```c
#define <identifier>
```

The first syntax layout just defines an identifier to be present. This doesn't do much on its own, but can be used to control the outcome of conditional preprocessor directives (which are explained below). A typical example would be:

```c
#define PWM_FREQ_50KHZ
```

One would assume this states that the PWM frequency needs to be set to 50kHz, and could cause the inclusion of another block of code which configures a PWM module to run at 50kHz.
...
During a build sequence, before the code reaches the compiler, the preprocessor will replace all instances of `NUM_OF_COUNTS` with the value 12.
...
The C preprocessor allows variadic macros, just like the C compiler allows for [variadic functions](/programming/languages/c/variadic-functions/). The variable-length parameters are encapsulated within the variable `__VA_ARGS__`, and you indicate in a macro that you want it to be variadic with `...`.
...
Variadic macros were added as part of the [C99 standard](/programming/languages/c/standards-and-versions/). The GCC compiler had supported them long before this standard was introduced, but only in the format `args...`.
...
You can make the preprocessor turn a macro argument into a string by prefixing the parameter with the `#` character inside a function-style macro. Note that the `#` operator only works on a parameter of a function-style macro — it cannot be applied to an object-style macro or arbitrary text.

```c    
// Note the use of the '#' character to get the preprocessor
// to turn the argument into a string
#define STRINGIFY(tok) #tok

char* msg = STRINGIFY(This is a message);
...
printf("This is an " STR(PF_UINT32_T) ".", aNum);
```
...
You can use #if statements for commenting out sections of code. This is useful if you want to comment out a large section of code (e.g. an entire C file), which has heaps of `/* comment */`  style comments in its body. Because `/*` style comments don't nest, you can't use them again to comment out blocks of code which already contain them. `#if 0` is a great alternative.
...
Watch out for circular inclusions! This is when you have one header file that depends on a second, and the second also depends on the first. This can be fixed by doing **forward declarations** or by adding [header guards](/programming/languages/c/header-guards/).
