...
The standard behaviour for this is to evaluate the expression and if it's false, print an error message and abort. If I run this program on Linux, I get the following message printed (and the program aborts):
...
Note that use of `__FILE__` **may not take up as much flash memory as you think,** as most compilers with any sort of optimisation turned on will collate all string literals which are exactly the same into one (i.e. all the `__FILE__` pointers in the same file will point to the same string literal).
...
If using the GCC compiler, and you write an assert with side effects, you should get the following error:
