...
authors: [gbmhunter]
date: 2013-03-18
description: A tutorial on C strings, including null termination, string literals, copying, concatenating, printf() and converting between strings and numbers.
...
The page teaches you how to work with strings in the C programming language. If you are used to higher level languages such as Python or Javascript, you'll quickly discover that C does not have some of the "nice" string manipulation features such as string concatenation with `+` (e.g. `str1 + str2`), no easy `.length()` property (although there is a function for that, `strlen()`) nor things like a built in regex engine. It doesn't really even have the concept of a string type, just "[pointer to char](/programming/languages/c/pointers/)" (`char*`) (some would argue that for all intents and purposes that is a string type).

## What Are Strings In C?

Strings in C are represented by either a pointer to char `char* myStr` or [array of char](/programming/languages/c/arrays/) `char myStr[10]`. What is laid out in memory is a series of byte sized ASCII encoded characters, terminated by the null character (`'\0'` or `0x00`).

Say we wanted to store the string "Hello". We could write `char* myStr = "Hello";`. If the compiler decided to plonk this at memory address `0x01` (I avoided `0x00` since that is invalid -- it's the null pointer) then the memory would look like this:
...
But this is not all, the memory above is used just to create the literal `"Hello"`. What also is created is the variable `myStr`, which points to the first character in this string. Let's assume the compiler decides to plonk this at memory address `0x08`, then your memory will look like this (simplified for illustration -- on a real 64-bit architecture the pointer itself would occupy 8 bytes).
...
One thing C does differently to a lot of other languages is its choice of using null to determine the end of the string, rather than reserving some bytes at the start of the character array for holding its length. To find the length of the string you have to iterate through the bytes until you find `0x00`, which is what `strlen()` does (more on this below).
...
These two ways are mostly equivalent, BUT you must remember that `myStr2` is an array with a known size (at compile time), whilst `myStr` is just a pointer to a char. Thus, `sizeof()` is going to behave differently.

When compiled for a 64-bit architecture, `sizeof(myStr)` returns 8, the number of bytes required for the pointer `myStr` which points to the memory address of the `a` in `"abc"`; However, `sizeof(myStr2)` returns 4, the number of bytes in the array `myStr2` which consists of `['a', 'b', 'c', '\0']`. This difference is bound to trip you up at some point, so be aware of this difference.

<Aside type="tip">
Most of the time you will be passing strings into functions, and if you use the `char myStr[]` style syntax, the compiler will have no idea what size the array of chars is (you could call it with a `char str[10]` from one function and a `char str[20]` from another). The array will "decay" to the same thing as `char* myStr`, and sizeof will return the size of a pointer (unless the function is designed to only ever take a string of a certain size, e.g. `char myStr[10]`, but that is a very odd use case).

Thus you can't use `sizeof()` to work out how big your string is. You have two options. Either use `strlen()` to get the number of characters in the string, or if you are interested in not just how many valid characters there are in the string but how big the array is, **you'll have to pass the size into the function also**. Passing the size of the string array is very common practice in C applications (as with any array, not just for strings), and after a while it will become second nature.
</Aside>
...
<Aside type="caution">
`char myEmptyString[50];` inside a function will not initialize any of the bytes (it will however be zero-initialized if it's declared outside of a function, since it then has static storage duration and is placed in the [.bss section](/programming/languages/c/bss-section/)). This means it will not be a well-defined string, and may or may not have a null character in it depending on what the memory happens to be. For example:
...
In C, `'A'` and `"A"` are completely different types (as opposed to say, Javascript, where you can swap `"` for `'` and it doesn't change anything). `'A'` is a single character, whilst `"A"` is a string literal (which is an array of characters). You can't do this:
...
<Aside type="caution">
Note the use of `char myString[]` and not `char* myString` above. Modifying a string through a `char*` that points at a string literal is undefined behaviour -- string literals are typically placed in read-only memory, and writing to one will usually crash your program. Assigning the literal to an array instead copies the characters into writable memory.
</Aside>
...
The null character (represented by `'\0'` or `0x00`, and sometimes called `NUL` -- not to be confused with `NULL`, which is the null *pointer* constant) is used to terminate strings. Most string-based commands will automatically insert the null character at the end of a string, while memory commands won't. Special care has to be taken to make sure the memory set aside for a string can accommodate the null character (when talking in bytes, the number of ASCII characters + 1).

### Carriage Return And New Line

These two characters are used to begin a new line of text. If you take the names literally, the carriage return moves the cursor back to the start of the line, and the new line shifts the cursor down one line. These characters are reminiscent of the typewriter days.
...
`\ddd`
      ...
      <td>Ascii char represented by one to three octal digits 'ddd'. Note that \0 is a special case of this more generic notation</td>
      <td>n/a</td>
    </tr>
    <tr>
      <td>
...
Use the `strlen()` function provided by `string.h`:
...
`strcpy()` is a standard library function (from `string.h`) for copying the contents of one C string to another. It copies characters from the source string into the destination buffer until (and including) the null character. It is up to you to make sure the destination buffer is large enough -- if it isn't, you get a buffer overflow.
...
int main() {
    char src[] = "Hello";
    char dest[10]; // Must be large enough to hold "Hello" plus the null character
    strcpy(dest, src);
    printf("%s\n", dest); // Prints "Hello"
    return 0;
}
...
A safer alternative is `strncpy(dest, src, n)`, which will never write more than `n` bytes to the destination. Beware of its one big gotcha though: if `src` doesn't fit, `dest` is **not** null-terminated, so it's common practice to manually set the last byte to `'\0'` afterwards:
...
Unlike many higher level languages, you cannot just concatenate C "strings" together like so: `my_string_1 + my_string_2` (remember, they are just arrays of characters!). Instead you have to use the `strcat()` function (from `string.h`), which appends a copy of the source string onto the end of the destination string. As with `strcpy()`, it is up to you to make sure the destination buffer is large enough to hold the combined result:
...
int main() {
    char dest[20] = "Hello"; // Must be large enough to hold "Hello World" plus the null character
    strcat(dest, " World");
...
`printf()` is the most commonly used string output function. It is a [variadic function](/programming/languages/c/variadic-functions/) (it takes a variable number of arguments, note that this is not the same as function overloading, which is something that C does not support).
...
Prints a signed integer (whose exact width is implementation-specific, usually 16 or 32-bit). There is no difference between `%d` and `%i` when used for output, but when used for input with `scanf()`, `%i` will interpret a number as octal if preceded with a `0`, and hexadecimal if preceded with a `0x`.
</td>
<td>
...
Prints a float (or double). All floats are converted to doubles anyway via default argument promotions. `%f` prints infinity and NaN as `inf` (or `infinity`) and `nan`, whilst `%F` prints them as `INF` (or `INFINITY`) and `NAN`. The default precision is 6 decimal places.
</td>
<td>

```c
printf("%f", 2.345); // Prints "2.345000" (default precision is 6)
printf("%.2f", 2.345); // Prints "2.35"
```
</td>
</tr>
...
An example of `snprintf()` (notice the use of `sizeof()` to pass the size of the buffer into the function, this prevents overruns if the formatted string is larger than the buffer):
...
`atof()` is a historic way of converting a string to a double-precision float (yes, even though the function has `f` in its name, it actually returns a `double`).
...
/// Converts a string to a double. Returns true on success, false if the
/// input did not start with a valid number. The converted number is
/// written to output.
bool ConvertStringToDouble(const char* input, double* output) {
    char* numEnd;

    // Do the conversion
    *output = strtod(input, &numEnd);
...
`getopt()` is a standard function for finding command-line arguments passed into `main()` as an array of strings (see the [Processing Command-Line Input page](/programming/languages/c/processing-command-line-input/) for an in-depth guide). It is included in the [GCC glibc library](http://www.gnu.org/software/libc/). The files are also downloadable locally [here](/misc/string-manipulation-in-c/getopt.zip) (taken from GCC gLibC v2.17).
 ...
 *    major and minor numbers must be in the range [0, 255]. The patch
 *    number must be in the range [0, 65535]. There must not be a "v" at
 *    the start.
...
You can test this code live at [https://replit.com/@gbmhunter/c-version-string-to-numbers](https://replit.com/@gbmhunter/c-version-string-to-numbers).

{/* ============================================================================================ */}
{/* REFERENCES */}
{/* ============================================================================================ */}

[^wikipedia-printf]: Wikipedia (2023, Aug 23). _printf_ [wiki]. Retrieved 2023-12-25, from https://en.wikipedia.org/wiki/Printf.
[^wikibooks-itoa]: Wikibooks (2020, Apr 16). _C Programming/stdlib.h/itoa_ [wiki]. Retrieved 2023-12-26, from https://en.wikibooks.org/wiki/C_Programming/stdlib.h/itoa.
[^ibm-zos-docs-itoa]: IBM (2021). _z/OS Docs - itoa() - Convert int into a string_ [documentation]. Retrieved 2023-12-26, from https://www.ibm.com/docs/en/zos/2.1.0?topic=functions-itoa-convert-int-into-string.
[^cplusplus-itoa]: cplusplus.com (2023). _function - itoa_ [documentation]. Retrieved 2023-12-26, from https://cplusplus.com/reference/cstdlib/itoa/.
