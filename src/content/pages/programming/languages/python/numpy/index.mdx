...
NumPy is a popular library for [Python](/programming/languages/python/) that provides a powerful N-dimensional array object and fast numerical routines to operate on it. It forms the foundation of much of the scientific Python ecosystem, including [SciPy](/programming/languages/python/scipy/) and [Matplotlib](/programming/languages/python/matplotlib/).

## The Array

The main object that you throw around in NumPy is called a multidimensional array. Typically you store numbers in it. Each "dimension" is called an axis. For example, a single co-ordinate in 3D space could be stored as:
...
A 2x3 matrix could be described with:
...
Another really useful way of creating arrays is with `np.arange()`. This does exactly what it says, it creates an array with a range of values:
...
NumPy arrays have one index per axis, forming a tuple. The indices are zero-indexed, like all sensible languages/libraries :-D.
...
Note that `:` is the same as `0:<len>` (the stop index is exclusive), and captures all data.
...
Stacking is used to join arrays along a _new_ axis. A classic example of this would be if you had two separate arrays of x and y values, and you wanted to combine them into a single array of (x, y) coordinate pairs.
...
If you want to start off with an empty array and add values to it in a loop, you can use concatenate, but you have to `reshape` your empty array first to make sure it has the right dimensions:

```python
data = np.array([]).reshape(0,3) # This reshape() call is important!
for i in range(3):
  data = np.concatenate([data, np.array([[ i*1, i*2, i*3]])])
print(data)
```

## Reading A CSV File

You can use Numpy's `genfromtxt()` method to read in CSV files and convert the data into a Numpy array (for more full-featured CSV handling, see [Pandas](/programming/languages/python/pandas/) and its `read_csv()` function):

```python
data = np.genfromtxt('my_file.csv', delimiter=',')
```

Each line will be a different element in axis 0 of the array. Each CSV value on a line will be a different element in axis 1 of the array.
...
You can trim down an array and remove all masked values by using the `compressed()` function that belongs to a `MaskedArray`:
...
For example, running `np.mean([])` using Python 3.7 and an up-to-date version of Numpy will produce the following:
...
Note how this is not an exception. Numpy prints a warning stating that you are trying to calculate a mean of an empty slice, as well as that there is an invalid value, but continues execution and returns `nan`. These warnings are usually helpful in debugging problems with the data you are providing, but in some cases you will want to silence the warnings as the data is as expected.

The safest way to suppress Numpy warnings is to use the `np.errstate` context manager, which only changes the warning state while the context is active. However, this has some problems...
...
The problem is that it will silence the `RuntimeWarning: invalid value encountered in double_scalars` warning, BUT NOT the `RuntimeWarning: Mean of empty slice.` warning. A better approach is to use the `warnings` module (which is shipped with Python), however this comes at the expense of silencing a larger group of warnings (what if a `RuntimeWarning` was emitted here for a different reason?):
...
If you want to convert all warnings into exceptions, you can use the following code. This is particularly dangerous as it applies to all code after this call.
