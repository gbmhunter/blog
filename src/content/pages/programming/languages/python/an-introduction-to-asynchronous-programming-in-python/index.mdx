...
So now you are probably thinking, since the parent function, and the parent's parent function, and the parent's parent's parent function all have to defined with `async` to be able to use `await`...where does it stop? What if my `main()` is not `async`? And even if that was, how would I call it? This is where the `asyncio` library comes into play.
...
<Aside type="note">
`asyncio.run()` is only available in Python v3.7 or higher.
</Aside>
