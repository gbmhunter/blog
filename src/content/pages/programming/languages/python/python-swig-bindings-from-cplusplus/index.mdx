...
Although there are many different ways of calling C++ code from Python (e.g. [pybind11](/programming/languages/python/pybind11/)), this page focuses on using [SWIG](http://www.swig.org/).
...
SWIG can build C/C++ bindings to a large number of programming languages, including:
...
The basic invocation of SWIG for generating a Python wrapper for C code is:
...
`-threads`: SWIG will release the [GIL](/programming/languages/python/python-threading/) when calling any C/C++ library code from Python (excluding classes that have been wrapped in directors)
...
You also need to provide a list of all the source files required to generate the library/bindings:
...
Note that `SWIG_ADD_MODULE()` and `SWIG_LINK_LIBRARIES()` were deprecated in CMake 3.8 — in newer projects use `swig_add_library()` and plain `target_link_libraries()` instead.
...
Notice the mention of `_<PythonModuleName>`. The SWIG package automatically adds an underscore to the compiled C/C++ library, while the generated `<PythonModuleName>.py` file does not have this underscore.

## Order Is Important

**The order in which to add classes** (or header files via %include) to the SWIG .i file **is important**. For example, if class B uses class A (say class B gets passed a pointer to a class A object in one of its methods), then class A should be included before class B, otherwise you won't be able to pass in a Python object of class A into B (it will complain about a TypeError).

## Typemaps

**Typecheck Typemaps**

There is a specific type of type map called a _typecheck_. The job of a typecheck function is to check that a provided object is of the correct type. You may stumble across the need to create custom type checks when trying to create C++ overloaded functions that have an input variable with a custom typemap. SWIG requires custom typechecks for these variables in order to work out which version of the overloaded function to call.

For example, say we provided a custom typemap for `std::vector<uint8_t>`, we should then provide a custom typecheck if we ever want to call overloaded functions that accept a `std::vector<uint8_t>` from Python. Our SWIG interface file would then look like:
...
%typemap(in) std::vector<uint8_t, std::allocator<uint8_t>> {  
    ...
    for(int i = 0; i < PyList_Size($input); i++) {
        temp.push_back(PyLong_AsLong(PyList_GetItem($input, i)));
    }
    $1 = temp;  
}
...
class AbstractClass {
public:
    virtual void AddOne(int * outputVar) = 0;
};
...
%typemap(directorin) int * {
    $input = PyList_New(1);
    PyList_SetItem($input, 0, PyLong_FromLong(*$1));
}

%typemap(directorargout) int * {
    *$1 = PyLong_AsLong(PyList_GetItem($input, 0));
}
```
...
Now let's write some Python code to test the callback functionality! Let's create a file called test.py:
...
As you can see above, we can now create a Callback object, passing in a Python function we wish to call (in this case it is called TestFunction), and then pass it to an instance of an Example class (which is a C++ class). The Python function is then called when CallCallback() is executed.
...
Sometimes, even though SWIG compiled successfully, it can be difficult to determine if SWIG created the Python bindings as intended. There are some useful tools you can use within the Python environment to determine what SWIG has exactly created.
...
This allows you to see what functions, classes, enumerations etc. are available from the Python code.

## Return Values

Whilst the C++ compiler enforces return values (if the function's signature says that it will return a value), Python does not. This can lead to issues when implementing a concrete class in Python of a base class that was defined in C++. If you forget to return the correct type, your program can segmentation fault.

## Smart Pointers

As of September 2017, SWIG only had support for `std::shared_ptr`, but not `std::unique_ptr` or `std::weak_ptr` (support for `std::unique_ptr` was later added in SWIG 4.1).

The SWIG support for `std::shared_ptr` is quite automatic. You have to first include the `std_shared_ptr.i` file in your SWIG interface file:
...
However, I have run into a problem with this feature, where I could no longer pass the underlying type back and forth if I had used the %shared_ptr() macro on it. So instead of enabling this special shared pointer type mapping, I created a custom type map for the shared pointer:
...
A similar type map was created for the other direction (C++ to Python) in the case that you are using directors:
...
%typemap(directorin) std::shared_ptr<std::vector<uint8_t>> {
    ...
    for(int i = 0; i < $1->size(); i++) {
        PyList_SetItem($input, i, PyLong_FromLong($1->at(i)));
    }
}
```
...
This could be because you have a C++ overloaded function like:
...
But you have forgotten to include the relevant typemaps for `uint32_t` and `std::string` in the SWIG interface file. In this particular instance, adding:
