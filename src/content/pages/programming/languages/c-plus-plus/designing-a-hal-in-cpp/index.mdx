...
Using [C++ for an embedded system](/programming/languages/c-plus-plus/cpp-on-embedded-systems/)? Great! At some point you might find yourself needing (or wanting) to implement a HAL (Hardware Abstraction Layer) atop your hardware peripherals (e.g. for the uninitiated, a HAL typically includes things like GPIO drivers, UART drivers, etc.). Why? Perhaps there is not one provided by your MCU vendor (although this is rare these days), or perhaps the existing one doesn't work well (unfortunately not as rare), or perhaps it doesn't easily provide the ability to [mock it for testing](/programming/languages/c-plus-plus/unit-testing-and-mocking/) (very common). You might have specific requirements on what you want the HAL to be able to do. Vendor provided or framework provided HAL layers can be hard to test.
...
[Inheritance](/programming/languages/c-plus-plus/inheritance/) and [virtual methods](/programming/languages/c-plus-plus/virtual-methods/) allow us to implement runtime polymorphism as a way to create real and mock implementations of the HAL. The process goes like this:
...
<Aside type="tip">
Although implementing inheritance and virtual methods is more suited to an OO-first language such as C++, it is still possible to do in C. If you want to see an example on how to do this in C, see the [Object-Orientated C page](/programming/languages/c/object-orientated-c/).
</Aside>
...
Now we can write an app class which doesn't care if it's using a real or fake GPIO. To do this, we make it accept a reference to a `GpioBase` object rather than any specific implementation. Note that this means it can only call methods which are defined in `GpioBase`, but this is what we want, as this defines the interface.
...
Our rudimentary `App` class just sets the GPIO pin high, but this is enough to demonstrate the concept.
...
Then in `main()`, we can create a `Hal` object, populate it with the appropriate GPIO objects for the platform we are targeting, and pass it into the App (whose constructor would now take a `Hal&` instead of a `GpioBase&`):
...
<Aside type="note">
Note that the derived class defines a function with the same name as the base class — the derived `set()` _shadows_ the base class method, which is why the `static_cast<T*>(this)->set(value);` call in the base class resolves to the derived implementation. Be careful though: if you forget to implement `set()` in the derived class, the base class method calls itself and you get infinite recursion. This will likely lead to a stack blow out and possibly a segmentation fault. One way to avoid this pitfall is to give the function in the derived class a different name (e.g. `set_impl()`), as we do in the concepts examples below.
</Aside>
...
We can demonstrate this with the following `main()` function below. C++17 or later is required for class template argument deduction (CTAD).
...
Because the casting of `this` to a pointer to `T` is going to be very common in a large base class, we can add a helper method called `self()` to make this easier (I saw this idea on Stack Overflow):[^stackoverflow-confusion-around-crtp-static-polymorphism]
...
C++ concepts allow us to expand on the idea of using templates to implement static polymorphism. They allow us to enforce that the derived classes implement the required methods, just as `virtual my_func() = 0` enforces this for dynamic polymorphism.

Without concepts, in the above template example, the only thing the compiler will enforce is that the derived class has a `set()` method and that it can be passed a `uint8_t`. If you happened to define the derived function as `void set(int32_t value)`, it would still work fine. Likewise, if you added a return value and defined it as `bool set(int32_t value)`, it would still be happy. You can start to see how the enforcement of the API of the derived class is loose. Concepts can help tighten this up!

[Roger Booth](https://medium.com/@rogerbooth/using-the-crtp-and-c-20-concepts-to-enforce-contracts-for-static-polymorphism-a27d93111a75) shows a way of using concepts along with inheritance to provide better compiler errors. However, you do have to define the functions in the base class, even though you have already defined them in the concept, and will also have to define them in the derived classes.[^medium-using-crtp-and-cpp20-concepts-for-static-polymorphism] Thomas Sedlmair shows a method which doesn't involve inheritance, and also gets around the hassle of having to define the functions in the base class.[^coding-with-thomas-crtp-and-concepts] We will compare both of the methods below.
...
And a similarly defined fake GPIO:

```c++ title="concepts/GpioFake.hpp"
#include "GpioBase.hpp"
...
[GuillaumeDua/cpp_legacy_inheritance_vs_std_variant.md](https://gist.github.com/GuillaumeDua/0239fda353264b67ddcb39b5d9a01105) is a good overview of standard dynamic polymorphism vs. static polymorphism via CRTP. It covers best practices for base classes such as making the constructor private and making the base class a `friend` of its derived class. It also covers the use of `std::variant` and `std::visit` to create containers which can hold any of the derived types.

{/* ============================================================================================ */}
{/* REFERENCES */}
{/* ============================================================================================ */}

[^medium-using-crtp-and-cpp20-concepts-for-static-polymorphism]: Roger Booth (2024, Feb 2). _Using the CRTP and C++20 Concepts to Enforce Contracts for Static Polymorphism_ [blog post]. Medium. Retrieved 2024-12-03, from https://medium.com/@rogerbooth/using-the-crtp-and-c-20-concepts-to-enforce-contracts-for-static-polymorphism-a27d93111a75.
[^coding-with-thomas-crtp-and-concepts]: Thomas Sedlmair (2024, Sep 24). _[C++] Static, Dynamic Polymorphism, CRTP and C++20’s Concepts_ [blog post]. Coding with Thomas. Retrieved 2024-12-03, from https://www.codingwiththomas.com/blog/c-static-dynamic-polymorphism-crtp-and-c20s-concepts.
[^stackoverflow-confusion-around-crtp-static-polymorphism]: Stack Overflow (2021, Jan 11). _Confusion about CRTP static polymorphism_ [forum post]. Retrieved 2024-12-04, from https://stackoverflow.com/questions/43821541/confusion-about-crtp-static-polymorphism.
