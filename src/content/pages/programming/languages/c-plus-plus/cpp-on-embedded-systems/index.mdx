...
Just like `std::expected`, `etl::expected` is a great way to return either a value or an error code from a function. In my opinion, it is a cleaner way than the "C way" of returning the error code and having to provide a pointer as a function input to save the result to. It is similar in nature to [Rust's](/programming/languages/rust/) `Return` type.
