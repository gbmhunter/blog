...
ETL provides a `etl::expected` class which is very similar to the `std::expected` class. As far as I know, the `std::expected` would be perfectly fine to use in an embedded environment (no dynamic allocation, exceptions, e.t.c), but it was only added in C++23, which may not be a viable option for certain embedded projects. Thus `etl::expected` is a good alternative as long as you can use the ETL library. If you do have `std::expected` available, I would recommend using it instead of `etl::expected` for better compatibility with other C++ code.
