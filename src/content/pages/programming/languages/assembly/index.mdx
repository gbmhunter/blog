...
In x86 language, a byte is 8-bits, a word is 16-bits, a double word is 32-bits, and a quad word is 64-bits. This terminology remains the same across 16-bit, 32-bit and 64-bit x86 architectures.
...
1. `push rdp` saves the address of the previous stack frame base pointer (which is currently in `rbp`) onto the stack.
1. `mov rbp, rsp` saves `rsp` into `rbp` to create a new stack frame.
1. `mov DWORD PTR [rbp-4], edi` copies the value of `edi` (the first argument) into the memory location `[rbp-4]`.
...
1. `ret` pops the return address off the stack and jumps to it.
