...
This would create the state machine shown in <IRef iref="fig-hierarchical-state-diagram" />.
...
* `EVT1` arriving would cause a transition from `S1` to `S1A`. This would call `s1_exit()` then `s1a_entry()`.
* `EVT2` arriving would cause a transition from `S1A` to `S1B`. This would call `s1a_exit()` then `s1b_entry()`.
* `EVT3` arriving would cause a transition from `S1B` to `S2`. This would call `s1b_exit()`, `s1_exit()`, and then `s2_entry()`.
...
A large state machine in a `.c` or `.cpp` file can be difficult for unfamiliar developers to understand. State diagrams can help, but they require maintenance to keep them in sync with the code. Instead, I recommend adding large comment blocks before each state.

```c
/* ======================================================================== */
/* S1 - Initial idle state. */
/* ======================================================================== */
static void s1_entry(void *data) {
    printf("State 1 entry.\n");
}
...
/* ======================================================================== */
/* S2 - Runs when an event ID of 1 occurs. */
/* ======================================================================== */

// ...

/* ======================================================================== */
/* S3 - Error state, device goes into disabled state until manual reset. */
/* ======================================================================== */
