...
* An initial transition to `S1A` would call `root_entry()`, `s1_entry()`, and `s1a_entry()`.
* Transitioning from `S1A` to `S1B` would call `s1a_exit()` then `s1b_entry()`.
* Transitioning from `S1B` to `S2` would call `s1b_exit()`, `s1_exit()`, and then `s2_entry()`.
