...
void LogResetCause()
{
    ...
    for(int i = 0; i < sizeof(resetCauseFlags) / sizeof(resetCauseFlag_t); i++)
    {
        if(resetCause & resetCauseFlags[i].flag) 
        {
            ...
            strncat(resetReasonBuffer, resetCauseFlags[i].name, buffSize - currLength - 1);
            currLength = strlen(resetReasonBuffer);

            // Add ", " between reasons
            strncat(resetReasonBuffer, ", ", buffSize - currLength - 1);
            currLength = strlen(resetReasonBuffer);
        } 
    }
...
The above code gets the device tree spec based off its path, checks that the GPIO is ready for use (I'm not sure why it wouldn't be, but good practice!) and then configures it as an output. To then set the pin high or low, you use `gpio_pin_set_dt()`:
...
There are many interrupt configuration flags to choose from, including both physical (actual voltage at the pin) and logical (depends on the pin's active high or active low configuration) triggers. For both physical and logical, there are both edge (transition) and level triggers. The table below lists the different flags[^zephyr-docs-gpio-h-file-reference].
...
The interrupt callback object has no provision for storing user data (e.g. a `void *` that you could use to pass in the "object" associated with the interrupt). However, you can use the `CONTAINER_OF` macro to do almost the same thing, as long as you store the interrupt callback object in the "object". The following example shows how to do this:
...
{
    // WARNING: This will be called in an interrupt context.
...
Notice how the mapping between the [PWM](/electronics/circuit-design/pulse-width-modulation-pwm/) channels and the pins is not directly set here (some examples online directly assign pins here), instead pointing to some structures defined in the pin control file. So then in your pin control file, we need to define the mapping between the PWM channels and the pins. For example:
...
Then define the PWM devices in your `.dts` file using the `pwm-leds` compatible:
...
This defines two PWM devices, both using the `pwm0` peripheral.
...
bool boolRc = pwm_is_ready_dt(&myPwmSpec);
__ASSERT_NO_MSG(boolRc);
```
...
The example below shows how to setup and configure the ADC on a nRF52 device. Some of the code, especially the `.dts` file, will be different between MCUs and vendors. The C code should remain similar (or the same!) since Zephyr aims to provide a standard interface across devices.
...
I have had some weird experiences with getting the ADC channel spec with:
...
in where the returned `spec.channel_cfg` was all zeros, indicating it was not set up correctly (and indeed, the `adc_channel_setup_dt()` would fail). I'm not sure why this is.
...
rc = adc_sequence_init_dt(&l_myAdcCh0Spec, &sequence);
...
int SampleChannel(uint8_t channelId, int32_t * sample_counts, int32_t * sample_mV)
{
    ...
    int32_t convertedVoltage_mV = tempSample_counts; // Needs to start off with the raw value, the function reads then modifies it!
...
advConfig.internal_timer_cc = INTERNAL_TIMER_CC; // Your CC value for the SAADC internal timer (sets the sampling rate). Set to 0 if not using the internal timer.
...
2. **High-level I2C Device Comms**: This is where you not only define the I2C peripheral in your `.dts` file, but also define the devices on the bus that you want to communicate with. Then in your C/C++ code, you grab handles to these devices and communicate with `read()` and `write()` functions that do not need an address (the bus and address are abstracted away into the Zephyr `.dts` file in your board directory).
...
Your microcontroller file will already define the [UART](/electronics/communication-protocols/uart-communication-protocol/) peripherals available (e.g. `uart0`, `uart1`, ...). All you have to do is enable them in your `.dts` file, and provide things like the baud rate:
...
And then in the `pinctrl` peripheral you can configure the pins that `i2s0` uses. The following example configures `i2s0` to use pin 0.5 for SCK, pin 0.7 for LRCK, pin 0.8 for SDOUT and pin 0.4 for SDIN (NOTE: this code includes nRF specific macros, this will be different on other vendor's MCUs):
...
Enable the [USB](/electronics/communication-protocols/usb-protocol/) device stack with:
...
Zephyr provides a feature-rich API for storing things in non-volatile storage (e.g. [flash memory](/electronics/components/memory/flash/)). The NVS API is provided by `#include <zephyr/fs/nvs.h>`. It represents elements as id-data pairs.
...
Nordic has contributed significantly to the Zephyr [Bluetooth](/electronics/communication-protocols/bluetooth/) API ever since they adopted Zephyr as their official platform for the nRF52, nRF53 and nRF91 MCU families. 
...
Zephyr provides a software and hardware based [watchdog](/programming/mcu-peripherals/watchdogs/) API you can use to monitor your threads and perform actions (usually a system reset) in the case that your threads become unresponsive and do not feed the watchdog in time. The API provides the ability to monitor multiple application threads at once via its software watchdog. This software watchdog can in turn be monitored by a hardware watchdog. A hardware watchdog (i.e. a physical peripheral provided by the MCU) can be trusted to reliably reset the device, even if the software watchdog locks up (which can be the case with certain errors).

<Aside type="note">
Why not just use the hardware watchdog? The useful thing about the software watchdog thread is that it provides the ability to independently monitor multiple threads, with different timeouts associated with each thread. It provides more granularity and control over what you watch and how often you expect each thread to "check in".
</Aside>

### Hardware Watchdog

To enable the hardware watchdog, first add the following to your `prj.conf`:
...
<Aside type="tip">
The window `.min` is so you can set a lower bound on the time between watchdog feeds. If the firmware feeds the watchdog too quickly, the watchdog will trigger. This is useful to detect things like quick infinite loops or catch issues where the sleep/delay functions are not working as expected.
...
Now let's cover how to use the software watchdog. Firstly, add the following to your `prj.conf`:
...
<Aside type="tip">
If you get a `undefined reference to 'task_wdt_init'` error, you likely have forgotten to add `CONFIG_TASK_WDT=y` to your `prj.conf`.
</Aside>
...
Notice the hardware watchdog is not started in `task_wdt_init()`. It is started on the first call to `task_wdt_add()`. When it starts it, it provides the option `WDT_OPT_PAUSE_HALTED_BY_DBG`. You don't have any control over changing this option.
...
Make sure you have enough available channels to be able to install timeouts. You can change this in `prj.conf` with `CONFIG_TASK_WDT_CHANNELS`. By default is set to `5`, but can be changed to anything in the range `[2, 100]`.[^zephyr-docs-kconfig-search-wdt-channels]
...
To disable the software watchdog at boot, you can add the following to your `prj.conf`:
...
When providing `NULL` as the second parameter to `task_wdt_add()`, Zephyr calls the system reset handler. Instead, you can provide your own handler to implement custom functionality. I prefer to provide my own handler, and generally have this handler log an error, wait for a period of time, and then reset the device.
...
Below is example code showing how to initialize the hardware watchdog for Nordic nRF devices. Firstly, the following is added to your `prj.conf`:
