...
I've had luck using the CMake `INTERFACE` library type to create libraries using Zephyr. An `INTERFACE` does not build a library artifact (e.g. static library file) but rather provides a collection of source files, header files, compile rules etc. that other projects can "link against" (which just means incorporate --- since there is nothing to link).
