...
[Zephyr features a "shell"](https://docs.zephyrproject.org/latest/services/shell/index.html) (one of its modules) that it can provide over a serial transport such as a [UART](/electronics/communication-protocols/uart-communication-protocol/), [USB](/electronics/communication-protocols/usb-protocol/) or Segger RTT. The shell provides Linux command-line style commands, [logging](/programming/operating-systems/zephyr/logging/), auto-complete, command history and more. It is great for implementing a debug interface to control your microcontroller from a terminal application such as [NinjaTerm](https://ninjaterm.mbedded.ninja/) (shameless plug, I developed this app).
...
`shell_print()` is used rather than `printf()` so that the output is routed to the backend of the shell instance that invoked the command.
...
int main() {
    const struct shell * main_shell;
    main_shell = shell_backend_uart_get_ptr();
    __ASSERT(main_shell != NULL, "Failed to get shell backend.");
}
```

`shell_set_bypass()` can be used to register a callback which receives all incoming data raw, bypassing the shell's normal command interpretation.
...
The modem_shell example project makes use of this bypass feature to provide a direct [AT command](/electronics/communication-protocols/at-commands/) mode between the shell and the modem.
...
// LED to blink if a fatal error occurs before the USB serial is available
static const struct gpio_dt_spec l_userLedGpioSpec = GPIO_DT_SPEC_GET(DT_ALIAS(led0), gpios);
...
The official example waits for the DTR line to be asserted before sending data. In traditional serial communication, the DTR (data terminal ready) line is driven by the Data Terminal Equipment (DTE), e.g. a computer, and read by the Data Communication Equipment (DCE), e.g. a modem. The DTE uses it to signal it is ready to send data.[^wikipedia-data-terminal-ready]
