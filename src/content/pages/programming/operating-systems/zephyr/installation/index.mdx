...
authors: [gbmhunter]
date: 2020-04-19
description: Installation guides for the Zephyr RTOS on Linux, Windows and WSL, including west workspace setup, SDK/toolchain installation and getting VS Code IntelliSense working.
lastUpdated: 2026-07-02
tags: [programming, operating systems, OSes, RTOS, Zephyr, installation, guide, Linux, Windows, WSL]
title: The Zephyr Installation Guide
type: page
---

Below are some basic [Zephyr](/programming/operating-systems/zephyr/) installation guides for various OSes. Another good read is the official [Getting Started Guide](https://docs.zephyrproject.org/latest/develop/getting_started/index.html). Installing Zephyr is not the easiest thing in the world, compared to what you might expect from modern package managers such as `npm`, `pip` or `cargo`. But we have to give Zephyr a break because it is complex in the sense that it supports many different architectures and compiler toolchains.

<Aside type="note">
Zephyr has updated their installation process significantly since I first created this guide. I have updated the main instructions to use the latest version of Zephyr (v4.0.0). Note that the example manifest below pins nRF Connect SDK (sdk-nrf) v3.0.1 — that is an nRF Connect SDK version, not a Zephyr version (it is based on Zephyr v4). If you are after older installation instructions, see the [Older Guide](#older-guide) section below.
</Aside>
...
We are not going to run `west init`, as it will put zephyr in the project root directory. I prefer to keep all west managed dependencies in a single sub-directory so that it keeps the project root directory clean and is easy to set up `.gitignore` rules for.
...
Create an `app` directory, and then create a `west.yml` file in it with the following contents:
...
<Aside type="note">
This example uses the `nrf` port of Zephyr, since I was developing for a nRF52840-based board at the time. You can find the list of ports [here](https://docs.zephyrproject.org/latest/boards/index.html). If you want to define your own board, see [Adding a Custom Board to Zephyr](/programming/operating-systems/zephyr/adding-a-custom-board/).
</Aside>
...
Note that the entire SDK is rather large, sitting around 12GB at `v0.17.1` (see the screenshots below). You can reduce this by only specifying specific toolchains to install with the `--toolchains` flag. The two I used the most are `arm-zephyr-eabi` (compiling for real MCU) and `x86_64-zephyr-elf` (compiling tests for `native_sim`).
...
<Image src={import('./_assets/screenshot-showing-the-size-of-the-zephyr-sdk.webp')} width="800px">A screenshot showing the total size of v0.17.1 of the Zephyr SDK. 12GB!</Image>

```bash
west sdk install --toolchains arm-zephyr-eabi x86_64-zephyr-elf
```

<Image src={import('./_assets/west-sdk-download-screenshot.webp')} width="800px">A screenshot of the Zephyr SDK download progress, showing all the different toolchains that will be downloaded by default.</Image>
...
I think this applied to Zephyr prior to v4. In v4 they added commands such as `west sdk install` and `west packages pip --install` to make installation easier.
...
1. Install the Python dependencies that Zephyr needs:
...
Follow these instructions to set up/install Zephyr on UNIX like systems, including Ubuntu and WSL. Use of `apt` assumes you are running a Debian like system.
...
1. Next, download the _Zephyr SDK_. The SDK contains toolchains (compilers, linkers, assemblers and other tools such as QEMU and OpenOCD) for all of Zephyr's supported architectures (e.g. `ARM`, `RISCV64`, `x86_64`, `Xtensa`, `aarch64`). Find the release you want from https://github.com/zephyrproject-rtos/sdk-ng/tags (use the latest if you don't otherwise care). The example below uses `v0.16.4` for x86-64 Linux.
    ...
    This file is over 1GB in size, so make sure you have space! There is also a _minimal release_ which doesn't come with all the toolchains, but instead lets you select and download the ones you need.
...
1. Install `udev` rules, which lets you program most boards as a regular user (serial port permissions):
...
1. Initialize the west workspace:
    ...
    This creates a directory called `.west` inside the current working directory. Inside `.west` is another directory called `manifest-tmp`.
...
The built executable is at `./zephyr/build/zephyr/zephyr.exe` (even though the extension is `.exe`, it is compiled for Linux not Windows). Of course, there is no basic LED we can blink when running on Linux, but it is mocked for us and instead prints to `stdout`:
...
Once you've followed the installation instructions, you should be able to build and flash _repository_ applications that are contained within the Zephyr repo (e.g. blinky). But what if you want to develop your own application? This is where it pays to understand the three different application types:

- **Repository Application:** An application contained within the Zephyr repository itself (inside the `zephyr/` directory in the workspace you created above). Typically these are example projects under `samples/` like "Hello, world!" and "blinky". In the directory structure below, `hello_world` is a repository application.
    ```text
    zephyr-project/
    ├─── .west/
    │    └─── config
...
Now create a file called `CMakeLists.txt` with the following contents:
...
The last thing you need to do is create a directory called `src` to store all the source code, and then create a file in it called `main.c` with the following contents.
...
To build and run, first `cd` back into the west workspace at `~/zephyr-project/`. Then run this at the command-line:
...
The most effective way of getting VS Code's IntelliSense working well with Zephyr is to use the "compile_commands" method as described below. Once you have IntelliSense working, you might also want to [setup debugging in VSCode for Zephyr](/programming/operating-systems/zephyr/debugging/). I've found this to be much more effective than trying to provide include paths.

Add the following to your `.west/config` file (if you followed the Linux guide above, your `.west/config` already includes this):
...
Rebuild your project from scratch. A `compile_commands.json` should be generated in your build directory. This will be picked up by VSCode's Intellisense and should fix any include errors you have!

If you are working on multiple apps with one west workspace, you can separate configurations for each. The following example sets up two configurations, one for the application and one for its [ztest unit tests](/programming/operating-systems/zephyr/#tests) that are in a `tests/` sub-directory, and with a build folder set to `build-tests`:
...
If you want more info on Zephyr unit tests and IntelliSense, see the [Tests](/programming/operating-systems/zephyr/#tests) section of the main Zephyr page.
