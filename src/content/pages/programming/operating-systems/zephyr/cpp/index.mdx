...
You can then change `main.c` to `main.cpp`. Remember to update the path in the `CMakeLists.txt` file also! However, you will still be missing a lot of the C++ standard library (e.g. `std::function`). To make this available, add the following to your `prj.conf`:
...
You have to add `LOG_MODULE_DECLARE(<module_name>);` to the beginning of any function in the header file which you want to log from. You also have to add `LOG_MODULE_REGISTER(<module_name>)` to one `.cpp` file as shown below:
...
public:
    void myFunction()
    {
        LOG_MODULE_DECLARE(MyClass); // Note how we have to declare the log module before we can use it
        LOG_INF("Hello, world!");
    }

    void myOtherFunction()
    {
        LOG_MODULE_DECLARE(MyClass); // We have to do this at the start of each function
        LOG_INF("Hello, again!");
    }
};
```

And then in a `.cpp` file somewhere you have to provide the log module implementation. I would typically do this in a `MyClass.cpp` file. Note you can't just jam all of these implementations into a single file (e.g. in `main.cpp`) as you will get naming collisions.

```c++ title="MyClass.cpp"
#include "MyClass.h"

LOG_MODULE_REGISTER(MyClass, LOG_LEVEL_INF);
```

Zephyr also supports instance based logging, where log messages are associated with a specific instance of a class. However, I think Zephyr's instance based logging is a bit of a mess and don't really use it. This is unfortunate as it would have paired well with instances of C++ classes.
...
Exceptions are disabled by default when you enable C++ support in Zephyr. If you try and add a `throw std::runtime_error("test");` to your code, you will get the following compiler error:
...
as Zephyr adds its own compiler flag `-fno-exceptions` which will appear after the one you specify.
</Aside>
