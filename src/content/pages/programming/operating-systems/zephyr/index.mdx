...
<Aside type="note">
You don't have to write `ASSERT_NO_MSG(thePointerYouPassedIn != NULL)` since C/C++ guarantees that checking the "truthiness" of a pointer will return `true` only if the pointer is not `NULL`.
</Aside>
...
<IRef iref="fig-zephyr-stack-canaries-demo-showing-error-in-terminal" /> shows a screenshot of the Zephyr stack canaries in action. I made a test where I decided to blow up the stack by allocating a large array:
...
<Image iref="fig-zephyr-stack-canaries-demo-showing-error-in-terminal" src={import('./_assets/zephyr-stack-canaries-demo-showing-error-in-terminal.webp')} width="700px">A screenshot of the Zephyr stack canaries in action, showing the canaries detecting a stack overflow in my main thread (printed from the shell/terminal).</Image>
