...
Sometimes the MCU you are programming may not be recognized by JLink (if using the `--runner jlink` option). You may get an error as shown in <IRef iref="fig-j-link-unknown-device-nrf9151-pop-up" />. This happened to me when I was programming the newish [nRF9151 SoCs](/programming/microcontrollers/nordic/nrf91/) from a nRF9161 DK using JLink v7.94i (presumably JLink v7.94i was not new enough to support the nRF9151).
