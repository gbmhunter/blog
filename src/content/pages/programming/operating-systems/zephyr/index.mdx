...
_Zephyr_ is the combination of a **real-time operating system, peripheral/system API framework, and build system** that is designed for resource-constrained devices such as microcontrollers. It is part of the Linux Foundation.

<Image src={import('./_assets/zephyr-project-logo.webp')} width="500px">The Zephyr Project logo.</Image>

Zephyr provides a firmware developer with a rich ecosystem of out-of-the-box OS and peripheral functionality that is consistent across MCU manufacturers (e.g. you can use the same UART API on both a [STM32](/programming/microcontrollers/stm32/) and nRF53 MCU). It also features an integrated build system called `west`.

Some criticism arises from Zephyr's complexity. Once such example is that Zephyr adds layers of additional configurability with the use of Kconfig (a configuration language that was originally designed to configure the Linux kernel[^kernel-org-kconfig-language]) and the device tree. Firmware behaviour can now be controlled via a combination of Kconfig settings, device tree settings, preprocessor macros and API calls at runtime. The Kconfig is layered in a hierarchical manner and dependencies and overrides can make it difficult to determine how your MCU is being configured.

Another issue is that the Zephyr device tree is very "static" and is designed for a static hardware configuration. While fine for most basic firmware projects, some advanced projects may require dynamic hardware configuration. For example, you may want the same firmware to support different hardware configurations, and need to configure pins/peripherals differently at runtime based of a version number read from flash. Zephyr has no way of "uninitializing" peripherals via it's API, and you typically have to reach around and use the raw hardware registers to do this. An example would be if you wanted to reconfigure an ADC input as a GPIO pin.
...
Sometimes the MCU you are programming may not be recognized by JLink (if using the `--runner jlink` option). You may get the pop-up shown below. This happened to me when I was programming the newish [nRF9151 SoCs](/programming/microcontrollers/nordic/nrf91/) from a nRF9161 DK using JLink v7.94i (presumably JLink v7.94i was not new enough to support the nRF9151).

<Image src={import('./_assets/j-link-unknown-device-nrf9151-pop-up.webp')} width="700px">The JLink "Unknown device" pop-up shown when programming a nRF9151 with a JLink version that does not yet recognize it.</Image>
...
Use `DT_PATH()` to get the node from a fully qualified path from the root node. e.g `DT_PATH(soc, serial_12340000)`. Note that non alphanumeric characters in the devicetree are converted to `_`, including hyphens and at symbols.
...
Sometimes when building Zephyr projects you can get "duplicate unit-address" warnings which are false positives. These are emitted by the `dtc` (devicetree compiler). This happens for example with the [nRF52](/programming/microcontrollers/nordic/nrf52/) SoC, where different Zephyr "peripherals" do share the same memory address.
...
You can sleep the current thread with the `k_sleep(timeout_t timeout)` function. This function is typically used in conjunction with the timeout macros such as `K_MSEC()`, `K_SECONDS()` etc. to create a timeout:
...
k_timer_init(&myTimer, &MyHandlerFn, NULL);
```
...
```c
// Start the timer 
k_timer_start(&myTimer, K_MSEC(1000), K_NO_WAIT);
```
...
You'll notice that in the above example, although the thread is created at runtime, the stack is statically defined at compile time with `K_THREAD_STACK_DEFINE()`. Statically defining the stacks does not play well if you want to write your code in a "object-oriented" manner. Luckily, Zephyr allows you to specify a stack as a member of a struct using the macro `K_KERNEL_STACK_MEMBER(sym, size)`.[^zephyr-docs-thread-stack-apis] For example:
...
`k_thread_stack_alloc()` depends on a number of configuration options being set in `prj.conf`. You will likely need:
...
A Zephyr mutex is a kernel primitive that **allows multiple threads to safely access a shared resource by ensuring mutually exclusive access**. It provides the same functionality as mutexes in most other operating systems.
...
`__ASSERT_NO_MSG()` can be used if you don't want to have to provide a message. This is useful when the condition by itself is enough to provide a helpful debug message. This is normally the case when checking booleans, or for non-null pointers. For example:

```c
__ASSERT_NO_MSG(heaterIsOn);
__ASSERT_NO_MSG(thePointerYouPassedIn);
```

<Aside type="note">
You don't have to write `__ASSERT_NO_MSG(thePointerYouPassedIn != NULL)` since C/C++ guarantees that checking the "truthiness" of a pointer will return `true` only if the pointer is not `NULL`.
</Aside>
...
You first have to enable the polling API by adding the following into your `prj.conf`:
...
Zephyr provides `k_malloc()` and `k_free()` for dynamic memory allocation of a shared block of heap memory. To use these (and have them defined) you need to add the following to your `prj.conf`:
...
Atomic operations are guaranteed to implement a full memory barrier on all target architectures.[^zephyr-docs-atomic-services]
...
Zephyr-based applications can get large, in part due to the powerful features it provides out-of-the-box. Just things like using logging throughout your code can increase flash usage significantly, due to every call saving the log message (before substitution takes place at runtime) as a string literal in ROM. This can easily use up many "kB" of space. If you weren't using float printing beforehand, this call also brings in float formatting functionality. Similarly, all `ASSERT()` style macros save the file name and line number of the assert as a string literal in ROM. However these are quite useful, even in production, so think carefully before disabling them.
...
On one project I was working on, just setting `CONFIG_SIZE_OPTIMIZATIONS=y` in `prj.conf` resulted in a flash size reduction from 421kB to 330kB!
...
puncover can be used to visualize the memory usage via a web GUI. Install puncover in the projects [Python](/programming/languages/python/) virtual environment:
...
Twister dumps it's build output into a directory called `twister-out`. If `twister-out` already exists (i.e. twister has already been run before) then it will rename the existing directory to `twister-out1`, `twister-out2`, etc. Part of the build output are the test reports. These are JUnit style XML reports that can be read by CI tools such as GitHub Actions, GitLab, Jenkins, etc. See the [Setting Up CI for Zephyr Projects](#setting-up-ci-for-zephyr-projects) section for more information.

### Adding Native Unit Tests To Your Application

Let's assume you have a workspace application at `~/zephyr-project/app/` and want to add unit tests to it using Zephyr and ztest. We'll run the tests on the `native_sim` board so that we can run them directly on the host machine without needing real hardware.
...
The Zephyr Project provides a [Docker](/programming/virtual-machines-and-containers/docker/) container that is suitable for CI pipelines. The images can be found on Docker Hub [here](https://hub.docker.com/r/zephyrprojectrtos/ci) (they are also on GitHub's container registry). The screenshot below shows some of the available images as of November 2024. Note the images are quite large, with some of them being around 4GB in size!

<Image src={import('./_assets/zephyr-docker-images-on-docker-hub.webp')} width="1000px">The Zephyr Docker images on Docker Hub.[^dockerhub-zephyr-project-rtos]</Image>
...
If you need to customize the container further, you can extend the image by making your own Dockerfile. Use `FROM zephyrprojectrtos/ci:v0.27.4` and then your own additional `RUN` commands. For example:
...
GitLab will display the test results in a merge request, as shown below.

<Image src={import('./_assets/gitlab-test-summary-in-merge-request.webp')} width="600px">The Zephyr twister test results displayed in a GitLab merge request.</Image>
...
The no `elftools` module error is a common error that can occur:
...
If trying to print a float using `%f` in any printf style functions (or log macros) results in the output `*float*`, it's likely you need to enable floating-point print support with `CONFIG_FPU=y` in `prj.conf`:
...
With certain older versions of Zephyr, you might get the following IntelliSense error when using macros such as `LOG_INF()` with only one argument (e.g. `LOG_INF("Hello, world!")`):

```text
too few arguments in invocation of macro "Z_FOR_LOOP_1" 
```

This is a known issue and has been fixed in newer versions of Zephyr. It builds fine, but IntelliSense (e.g. VS Code with the Cpp Tools Extension Pack) gets confused and gives you red squiggles under the macros. As far as I know there is no workaround except to update to newer versions of Zephyr.[^github-zephyr-too-few-arguments-in-invocation-of-macro]
...
The screenshot below shows the Zephyr stack canaries in action. I made a test where I decided to blow up the stack by allocating a large array:
...
<Image src={import('./_assets/zephyr-stack-canaries-demo-showing-error-in-terminal.webp')} width="700px">A screenshot of the Zephyr stack canaries in action, showing the canaries detecting a stack overflow in my main thread (printed from the shell/terminal).</Image>
...
One requirement is that a "Signed-off-by" line is added to each commit message. This is so that the Developer Certificate of Origin (DCO) process is followed. This can be done by adding the `-s` option to your [`git`](/programming/version-control-systems/git/) `commit` command, e.g.:[^zephyr-docs-contribution-guidelines]
...
The Zephyr repo on GitHub has extensive CI workflows that run before a PR is allowed to be merged into the `main` branch. The screenshot below shows the CI workflows.

<Image src={import('./_assets/zephyr-pr-github-ci-workflows-screenshot.webp')} width="600px">Screenshot of the Zephyr PR GitHub CI workflows.</Image>
...
[Nordic](/programming/microcontrollers/nordic/) chose to move from their nRF5 platform to Zephyr as the officially supported development environment for their nRF52, [nRF53](/programming/microcontrollers/nordic/nrf53/) and nRF91 MCU/SoC families. Zephyr support for the nRF52 family was added around April 2020.[^nordic-semi-nrf52-zephyr-support-added]
...
[^scalar-zephyr-operating-system]: Scaler. _Scaler Topics - How does the Zephyr Operating System Work?_. Retrieved 2024-02-01, from https://www.scaler.com/topics/zephyr-operating-system/.
...
[^zephyr-docs-thread-stack-apis]: Zephyr (2025, Feb 25). _Thread Stack APIs_ [documentation]. Retrieved 2025-02-25, from https://docs.zephyrproject.org/apidoc/latest/group__thread__stack__api.html.
...
[^github-zephyr-too-few-arguments-in-invocation-of-macro]: GitHub. _zephyrproject-rtos/zephyr: too few arguments in invocation of macro "Z_FOR_LOOP_1" #62460_ [issue]. Retrieved 2026-02-18, from https://github.com/zephyrproject-rtos/zephyr/issues/62460.
