...
Also create a `memory.x` file with the following contents:
...
Using usbipd, I can pass-through the connected nRF52 DK from Windows into WSL. First you have to bind the USB device from a terminal on Windows with admin privileges:
...
We can now flash the nRF52 DK using `cargo run`:
