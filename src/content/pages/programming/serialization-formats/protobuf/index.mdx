...
It is designed as an efficient, **language agnostic** way of sending data over messaging protocols such as TCP/UDP, or UART/SPI/I2C etc.
...
Messages defined in .proto files can be composed of variables which have scalar (primitive) types, as well as variables which are of another message type (e.g. nested types).
...
`sint32`    | Uses variable length encoding with ZigZag encoding, efficient at encoding negative numbers.
`sint64`    | Uses variable length encoding with ZigZag encoding, efficient at encoding negative numbers.
...
Each one of these scalar types maps to a corresponding type in your language of choice. See [https://developers.google.com/protocol-buffers/docs/proto#scalar](https://developers.google.com/protocol-buffers/docs/proto#scalar) for more details.
...
Of course, once serialized into a string, you can **send the data over a serial communications protocol**, save it to file, send over a websocket etc., before deserializing at the other end!
