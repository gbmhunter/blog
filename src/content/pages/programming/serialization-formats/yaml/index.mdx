...
[JSON](/programming/serialization-formats/json/) is a valid subset of the YAML specification, which means a `.json` file is a valid YAML file, and you can embed JSON syntax inside a YAML file.
...
All members of a basic list start with `- ` (a hyphen and a space):
...
By default, PyYAML will use aliases when it comes across references to the same object in memory when serializing (dumping). The following example shows a basic example which doesn't use aliases, because string assignment in Python creates a new object:
...
But things happen differently for a `datetime` object, where the assignment just takes a reference to the object:
...
yaml.dump(data, Dumper=VerboseSafeDumper)
```
