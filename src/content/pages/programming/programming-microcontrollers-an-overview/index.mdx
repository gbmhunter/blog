...
`SWDCLK`, `SWDIO` and `SWO` should be pulled high by approx. $10k\Omega$ so that they remain in a stable state when not driven[^bib-arm-dstream-swd-conn].
...
SWD communication begins with a line reset, in where the DP generates 50+ clock cycles on `SWDCLK` which driving `SWDIO` high (as shown in <IRef iref="fig-swd-line-reset-and-idcode-traces" />). This gets the AP state machine into a known state for further communication.

<Image iref="fig-swd-line-reset-and-idcode-traces" src={import('./_assets/swd-line-reset-and-idcode-traces.webp')} width="900px">A capture of the SWDCLK (top) and SWDIO (bottom) traces during a line reset and IDCODE request. The line reset is the first 50+ clock cycles on `SWDCLK` which drives `SWDCLK` high. The IDCODE is the next 32 bits on `SWDIO`.</Image>

If a MCU is not working for some reason (not connected properly, dead, etc.), your first sign of failure will usually be when if fails to ACK the IDCODE request from the DP. <IRef iref="fig-swd-mcu-working-and-not-working-comparison" /> shows a comparison of a working MCU (top) and a not working MCU (bottom) during a line reset and IDCODE request. The working MCU ACKs the IDCODE request from the DP, while the not working MCU does not.

<Image iref="fig-swd-mcu-working-and-not-working-comparison" src={import('./_assets/swd-mcu-working-and-not-working-comparison.webp')} width="900px">A comparison of a working MCU (top) and a not working MCU (bottom) during a line reset and IDCODE request.</Image>
