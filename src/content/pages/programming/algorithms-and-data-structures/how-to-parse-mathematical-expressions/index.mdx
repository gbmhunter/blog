...
tags: [programming, data structures, algorithms, code, software, extended Backus-Naur form, EBNF, recursive descent parsers, RPN, parsers, grammar, lexers, evaluators, abstract syntax trees, AST, infix notation, postfix notation]
title: How To Parse Mathematical Expressions
type: page
---

The aim of this tutorial is to introduce the concept of parsing instructions expressed as text into executable code. This serves as a basis for working with machine readable languages (e.g. compilers, JSON deserialization and mathematical expression evaluators). This page will use examples for evaluating basic mathematical expressions.
...
RPN does not require the use of parentheses to preserve the correct order of operations. RPN can linearize a tree data-structure, which can have memory benefits.

You may have noticed that some older calculators don't work as you're "used to". This could be because they use RPN! The Hewlett-Packard HP-48 graphic calculator presented RPN to the user (no infix notation was allowed!).
...
Parser: Reads the tokens outputted from the lexer and parses them into a structure such as RPN or AST. It reshapes the tokens into a structure according to rules such as operator precedence.
...
Grammar is a way of formally describing the structure of a language. You can call grammar the _language of languages_. Grammar is usually written as a sequence of _rules_. Each rule has two parts, a _name_, and an _expansion_ of the name. The expansion shows what the name can be built from. The idea of rules is to describe the syntax as a hierarchy, starting at the broad structure of the language and then breaking it down into smaller and smaller components. The _name_ can also be called a _non-terminal symbol_. This refers to the fact that it is just an intermediary representation and will be expanded into smaller components. Things such as a `number` are called _terminal symbols_.

We will use a simple calculator program as an example. The calculator allows you to type in _expressions_. Each _expression_ is built from two _terms_, which are added together.
...
Let's now expand the calculator's functionality so that we can also multiply!
...
A _Recursive Descent Parser_ (RDP) is a type of parser which uses function recursion as a primary technique for parsing expressions. One of the main benefits of RDPs is that their design closely matches the structure of EBNF grammar syntax.
