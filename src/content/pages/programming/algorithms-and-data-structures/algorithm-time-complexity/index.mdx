...
- $ o(f(n)) $ (little-O) also describes the **upper bound** in complexity, but is a stronger statement. Big-O is an inclusive upper bound (grows less than or equal to), while little-o is a strict upper bound (grows strictly less than). Big-O is to little-o as $ \leq $ is to $ < $.
- $ \Omega(f(n)) $ describes the **lower bound** in complexity
- $ \Theta(f(n)) $ describes the **exact bound** in complexity

$ \mathcal{O}(f(n)) $ (Big-O) is the most commonly used complexity, as we are normally interested in the worst case computational cost/memory usage when choosing/designing an algorithm. Note that even when using Big-O, you can still talk about average-case and worst-case scenarios, but this is referring to specific inputs and algorithm states which cause the algorithm to perform more poorly than usual.
...
Big-O notation ignores any constant terms and any constant coefficients (e.g. an algorithm that grows at $ 2n $ is still written as $ \mathcal{O}(n) $). This is because constant terms and constant coefficients do not affect the growth, you can think of Big-O notation as kind of analysis on the "derivative".

## Complexity Algebra

When adding two big-O complexity classes we can move the functions inside the $ \mathcal{O} $:

<div>$$ \mathcal{O}(f(n)) + \mathcal{O}(g(n)) = \mathcal{O}(f(n) + g(n)) $$</div>
...
- Reading a value from an array (e.g. a Python `list`, a C `array`, or a C++ `std::vector`). To read a value from an array, you just read the memory address at `base_address + index * value_size`. This takes a constant amount of time, no matter the size of the array.
- Reading an element from a [hash table](/programming/algorithms-and-data-structures/associative-arrays/) (e.g. a Python `dictionary`). Computing the hash of a key takes a constant amount of time, and then looking up the memory location at that hash is also constant.
...
Notice the `i *= 2`, which makes it run in $ \mathcal{O}(\log{n}) $ time.
...
```python
for i in range(0, n, 2): # Increment i by 2 each time
    print('hello')
```
...
$ \mathcal{O}(n\log{n}) $ complexity can be thought of as a combination of $ \mathcal{O}(n) $ and $ \mathcal{O}(\log{n}) $ complexity (and more often than not, this is how the algorithm actually works). It is famously known as the **best complexity that you can [sort an arbitrary collection of elements](/programming/algorithms-and-data-structures/sorting-algorithms/) in**.
...
Naturally, we could keep going forever explaining poorer and poorer complexities ($ n^4 $, $ n^5 $, etc.), but I think by now you understand the concept, and these higher complexities are rarely seen in real software algorithms.
...
Factorial time is written as $ \mathcal{O}(n!) $. It is a very rare and hugely expensive complexity class. Generating all permutations of a collection is one example of an algorithm that runs in factorial time.
...
For example, take the example of adding a value to an array. The array starts off being able to hold one value, and its size is doubled every time we run out of space. When we add a value, it normally doesn't need resizing, which is $\mathcal{O}(1)$. But when adding the 1st, 2nd, 4th, 8th, 16th, etc. value, we need to resize the array. This takes $\mathcal{O}(n)$. The probability of having to resize the array is $ \frac{1}{n} $. This gives an amortized complexity of $ \frac{\mathcal{O}(n)}{n} = \mathcal{O}(1) $.
