...
import FixedPointExplorer from 'src/components/tools/fixed-point-explorer/FixedPointExplorer.jsx';

Fixed-point mathematics is a method for representing numbers on a binary computer architecture. It allows the storage of numbers with decimal points, similar to the float and double, but with the benefit of requiring less computation time. The trade-off is lower precision and flexibility.

If you think about it, normal integers are just a special case of a fixed-point number where the decimal point is to the right of the least-significant bit. A 32-bit unsigned integer (`uint32_t`), would be `fp(32, 0)`. It is common to use the bit-width of the architecture as the default fixed-point length, as the architecture has native support for manipulating these variables (commonly 32-bit on the more powerful embedded microcontrollers).

Most programming languages and hardware that code runs on does not have native support for fixed-point mathematics (however there are many third-party libraries out there!). C++ has a nice advantage over C in the fact that it supports [operator overloading](/programming/languages/c-plus-plus/operator-overloading/), meaning that you can write a fixed-point library so that you could multiply/divide two fixed-point numbers just by using the `*` or `/` syntax, just like when dealing with other native number types (in C you would have to use functions/macros).
...
Fixed-point numbers are just stored in software as regular integers (signed or unsigned depends on your needs).

For example, a `Q4.12` number storing the value of `6.5` will be stored in an unsigned 16-bit integer (`uint16_t`) as:
...
$$
0 \textrm{ to } (2^i -1) + 2^{-f} \cdot (2^{f} - 1)
$$
...
$$
-2^{i-1} \textrm{ to } (2^{i-1} -1) + 2^{-f} \cdot (2^{f} - 1)
$$
...
## Interactive Fixed-Point Explorer

Use the tool below to see how a real number is encoded in fixed-point format. Choose the number of integer and fractional bits and whether it's signed, then type a value (or click individual bits to flip them) and watch the stored integer, hex/binary representation, quantized value and quantization error update.

<FixedPointExplorer client:load />

## Converting To Fixed-Point

**Integer To Fixed Point Number**

Converting from an integer to a fixed point number is easy, all you need to do is [left-shift](/programming/languages/c/bit-fields-and-bit-manipulation-masking/) the integer by the number of fractional bits:
...
The rawVal_ class variable is how the fixed-point number is stored in memory.

**Double To Fixed Point Number**

Converting from a double to a fixed point number is not much harder! We just need to multiply the double by a scaling factor, where the scaling factor is defined as 1 &lt;&lt; num. fractional bits.
...
**Of course, you run into the same overflow issue as you do with multiplication.** Even if the resulting division is small enough to be held in the output variable (fp3 in the example above), the intermediary left-shift of fp1 can result in an overflow, giving an incorrect result.
...
```c    
// The division must be fixed-point division (the '1' is left-shifted internally)
fp2 = 1 / fp3;
```
...
I have written an embedded C++ fixed-point library called MFixedPoint. It is freely available for [download from GitHub here](https://github.com/gbmhunter/MFixedPoint).
