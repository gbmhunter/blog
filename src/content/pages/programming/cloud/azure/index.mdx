...
Microsoft provides a [VS Code extension](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurefunctions) that helps you write and deploy functions from your local machine.
...
`local.settings.json` contains local environment variables under the `Values` property.
