...
authors: [gbmhunter]
date: 2019-10-09
description: Notes on Microsoft Azure, including resource groups, storage accounts, load balancers, Azure AD (Entra ID), RBAC, Azure Functions, and IoT Hub device provisioning and authentication.
...
A resource is a single service instance in Azure. A resource group is a logical grouping of resources. An ARM (Azure Resource Manager) template is a .json file that allows you to declaratively describe a set of resources.
...
- IaaS: Infrastructure as a Service. You look after the OS and libraries.
...
Blobs are a key/value object store (similar to AWS S3). Control user permissions with blob access policies. Blob storage is about 30x cheaper per GB than Azure Files.
...
A [traffic manager](https://azure.microsoft.com/en-ca/services/traffic-manager/) is a smart DNS form of load balancer that can resolve a `CNAME` based on performance/geographic requirements.

## Azure Active Directory

Azure Active Directory (Azure AD) is a cloud service that offers multi-tenant access and identity control. It was renamed to Microsoft Entra ID in 2023.

Federation is a collection of domains that have established trust.

## Management Groups

Azure automatically creates a root management group (the Tenant Root Group) for each directory. Management groups can be used to group together subscriptions. Management groups exist in a tree-like hierarchy. 
...
The Azure Import/Export service supports both import and export jobs for transferring large amounts of data into and out of Azure. (This is distinct from Azure File Sync, which synchronises on-premises file servers with Azure Files.)

For really large amounts of data, you can use the data boxes (physical storage devices that get sent to your location and then shipped back to Azure).
...
It is best practice to use delegated permissions wherever possible. Only use application permissions when the app is non-interactive or the app runs without requiring a user to login.
...
Application Insights can monitor request rates, response times, failure rates etc. Exception stack traces from both the server and browser (client-side application code) are logged.

Application Insights can then provide its data via an API or through Visual Studio (for debugging). It can also create visualizations in the Azure dashboard.
...
Azure Functions are a way to run small pieces of code, in which you are only billed for compute time when the code is running (i.e. serverless). They must be triggered by a specific event (like a HTTP request or a timer schedule) which is supported by the Azure Function framework. They are similar to [AWS Lambda functions](/programming/cloud/aws/lambda/).
...
You can configure a function that runs on a schedule (using [CRON](/programming/operating-systems/linux/programs/cron/)-like NCRONTAB expressions) by using the `@app.timer_trigger` decorator. The below example shows a function that runs every day at 00:00:00 (i.e. once a day at midnight). Note that `run_on_startup=True` means the function will also run every time the host restarts, so it is not recommended in production.
...
- You can route devices to different IoT Hubs based on particular criteria such as geographic location, product, etc.
...
- Symmetric key authentication: The IoT device has a pre-created symmetric key which IoT Hub also knows about. The device presents a signature based on the symmetric key as proof of identity.
...
The Python `cryptography` library will be used to generate the certificates (`openssl` is used later just to inspect them). These examples will assume you are using Linux/macOS, although with a bit of modification you can make them work on Windows too.
...
You can invoke a "direct method" on a device from IoT Hub using the `invoke_device_method` method. Note that direct methods are distinct from cloud-to-device (C2D) messages, which are sent with `send_c2d_message` instead. The code example below shows how to invoke a direct method:
...
[^microsoft-iot-hub-create-and-upload-certificates-for-testing]: Microsoft. _Tutorial: Create and upload certificates for testing_ [documentation]. Retrieved 2025-05-01, from https://learn.microsoft.com/en-us/azure/iot-hub/tutorial-x509-test-certs.
