...
import EmaFilter from 'src/components/calculators/ema-filter/EmaFilter.jsx';
...
Unlike a [simple moving average (SMA) filter](/programming/signal-processing/digital-filters/windowed-moving-average-filters/), most EMA filters are not windowed, and the next value depends on all previous inputs. Thus most EMA filters are a form of infinite impulse response (IIR) filter, whilst a SMA is a finite impulse response (FIR) filter. There are exceptions, and you can indeed build a windowed exponential moving average filter where the coefficients are weighted exponentially.
...
Notice that the calculation does not require the storage of past values of $x$ and only the previous value of $y$, **which makes this filter memory and computation friendly (especially relevant for microcontrollers)**. Only one addition, one subtraction, and two multiplication operations are needed. If you are really tight on cycles, you can rearrange the difference equation into the algebraically-equivalent form $y[i] = y[i-1] + \alpha (x[i] - y[i-1])$, which needs just one addition, one subtraction, and a single multiplication.

The constant $ \alpha $ determines how aggressive the filter is. It can vary between $0$ and $1$ (inclusive). As $ \alpha \to 0 $, the filter gets more and more aggressive, until at $ \alpha = 0 $, where the input has no effect on the output (if the filter started like this, then the output would stay at $0$). As $ \alpha \to 1 $, the filter lets more of the raw input through, resulting in less filtered data, until at $ \alpha = 1 $, where the filter is not "filtering" at all (pass-through from input to output).
...
... \nonumber \\
\label{eq:ema-sum}
y[i] &= \alpha \sum_{k=0}^i (1-\alpha)^k x[i-k] \\
\end{align}
$$

<Aside type="tip">
Strictly, this finite expansion also carries a residual term $(1 - \alpha)^{i+1} \, y[-1]$ from the filter's initial value. Since $|1 - \alpha| < 1$, this term shrinks with every sample and vanishes as $i \to \infty$, so it has been dropped above.
</Aside>
...
This transfer function can be used to create bode plots of the magnitude and phase response of the EMA filter. The below bode plot shows the response of an EMA filter with $\alpha=0.25$. The x-axis frequency is the normalized frequency, in units of $cycles/sample$, which makes the plot applicable for any sampling frequency.
...
## Interactive EMA Filter Explorer

The widget below lets you explore both sides of the EMA filter at once. Drag $\alpha$ and watch how it trades smoothing against responsiveness in the **Time domain** tab, then switch to the **Frequency response (Bode)** tab to see the same $\alpha$ shape the magnitude and phase response. Push $\alpha$ above $0.82843$ and the -3dB cut-off marker disappears, illustrating the "no valid cut-off before Nyquist" condition derived above.

<EmaFilter client:load />
...
$$
\begin{align}
y[i] &= \alpha \sum_{k=0}^i (1-\alpha)^k \delta[i-k] \\
\end{align}
$$

Given the unit sample function is 0 at most points, the only sum term that matters is when $k = i$, so we can simplify this to:
