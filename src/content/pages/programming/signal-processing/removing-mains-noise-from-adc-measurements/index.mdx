...
Another way to fix this aliasing problem is to increase the sampling rate. For no aliasing to occur, you need to sample at least twice as fast as the highest frequency in the signal (i.e. the Nyquist frequency). So if we wanted to get rid of 50Hz aliasing, we would need to sample at least 100Hz.

Let's re-run the simulation, but this time sample at 200Hz (giving ourselves some margin above Nyquist).
