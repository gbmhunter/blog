...
authors: [gbmhunter]
date: 2014-02-08
description: Git is a very popular version control system (VCS) used for software and firmware development.
image: ./_assets/cover-image.webp
lastUpdated: 2026-06-29
tags: [programming, version control systems, VCS, git, amend, fixup, squash, history, commits, vim, merge-base, diff, bisect, cherry-pick, stat, ssh]
title: Git
type: page
---

Git is a very popular version control system (VCS) used for software and firmware development.

<Image src={import('./_assets/git-logo-2-color.svg')} width="400px">The Git logo.[^git-scm-logos]</Image>
...
The following command will add the staged changes to a special `fixup` commit.
...
<Aside type="caution">
Just like `--amend`, `--fixup` will also **modify the git history** (once you rebase). You should `fixup` commits on private (local) branches, and never on public branches. Fixing up commits that have been shared with other developers will cause history conflicts which are difficult to resolve.
</Aside>

`--fixup` behaves slightly differently to `--amend`, one of the main differences is that you can rewrite any commit in the current history, not just the last one as you can with `--amend`. `--fixup` is a great tool for keeping your git history clean when making small bug fixes and improvements to already committed features on a development branch. This is best shown with an example:
...
$ git rebase -i --autosquash af82c^
```

In the `rebase` command above, `af82c^` was used to say "I want to rebase off the commit BEFORE `af82c`. Note all these hashes are shortened versions of the full hash, which will work as long as there are no hash collisions (with 5 characters, it's unlikely there will be for a small number of commits).
...
To include information on what files were changed in each commit, add the `--stat` option:
...
The purpose of `git reflog` is to provide a fail-safe in case you do a `git` operation that would otherwise wipe your data. `git reflog` allows you to:
...
If you do apply custom changes like this, make sure your script resets the repo back to the exact state `git bisect` had configured before it returns &mdash; do not leave the repo in any other state, or the bisection results will be unreliable.
...
"Force pushing" with git is common when you are working on a branch and using `git commit --amend` and `git rebase` to keep your commit history clean and rebase off master. However, `git push -f` can have disastrous consequences if someone else has pushed changes to the branch that you don't have in your local copy.
...
The problem with the above command is that if other changes have made their way onto `master` since you branched off it, these will also be displayed in the diff along with the changes you have made on your branch. What you really want to do is display the diff between the `HEAD` of your branch and `master` at the point you branched off it. This is called the `merge-base`. You can find the `merge-base` commit hash with:
...
## Rewriting https as ssh

Sometimes you might be checking out a repo that contains a large number of submodules (perhaps even recursively) that contain https-style URLs to the submodules. For example:
...
This can be an issue if you use `ssh` for authentication. One way around this is to make Git locally rewrite all https-style URLs into ssh-style URLs. 
...
This only applies the changes locally, i.e. none of this info saved with the repo.

## Using Different SSH Keys for Different Repos

Firstly, make sure you have added the two SSH keys into your `~/.ssh` directory. For example, lets pretend we want to use different keys for work and personal use. These will be attached to different GitHub accounts (GitHub doesn't let you use the same key for multiple accounts, for obvious reasons). The contents of the `~/.ssh` directory would contain:
...
<Aside type="tip">
I'm using the `~/` syntax for the users home directory, which works on Linux and macOS. However, this works just as well on Windows if you replace `~` with the path to your user home directory.
</Aside>

Then in `~/.ssh/config`, you define two different hosts, one called `work` and one called `personal`. They both use the same `HostName`, but use different `IdentityFile` entries:
...
<Aside type="tip">
You are free to create the `config` file if it doesn't exist.
</Aside>

Lastly, you need to modify the SSH URLs used when cloning and/or the remotes. The SSH URLs usually look like this:

```text
git@github.com:my_org/my_repo.git
```

We need to rename this to replace `github.com` with `work` or `personal`. For example, if we wanted to use the work SSH key:

```text
git@work:my_org/my_repo.git
```

And if we wanted to use the personal SSH key:

```text
git@personal:my_org/my_repo.git
```

If you have already cloned the repo, you will need to change the remote URL with `git remote set-url origin` or similar. If you are doing a fresh clone, you can run:

```bash
git clone git@work:my_org/my_repo.git
```
...
[^git-scm-logos]: Git. _Logos_ [web page]. Retrieved 2025-01-22, from https://git-scm.com/downloads/logos.
