...
GitLab is a web-based "dev ops" software system that provides [Git](/programming/version-control-systems/git/) server based hosting, bug tracking, CI/CD and more for software projects. It is a strong competitor to GitHub, especially for commercial uses.

<Image src={import('./_assets/cover-image.webp')} width="500px">The GitLab logo[^gitlab-our-new-logo].</Image>

## Container Registries

Each repo can have its own container registry. You can use this to store [Docker](/programming/virtual-machines-and-containers/docker/) images relating to the project.
...
Then you will be able to push a built image:
...
Jobs run as the image's default Docker user. If you want to override this, you can change the structure of the `image` key a bit and add a `docker`/`user` key as shown below.
...
However, this will not run the job's dependencies, which might be what you were expecting. To run the job and all of its dependencies, add the `--needs` flag:
...
When a pipeline is running on GitLab's servers, a number of pre-defined environment variables get provided that the pipeline scripts can use. When running them locally, `gitlab-ci-local` will do its best to provide sensible defaults for these variables. However those dealing with authentication and other URLs will not work, as `gitlab-ci-local` cannot work out what these should be. In these cases, you can create a file called `.gitlab-ci-local-variables.yml` in the root directory of the repository and manually provide the variables.
...
It doesn't look like `gitlab-ci-local` supports defining variables using other variables. For example, this works when running on GitLab (appending `/another/path` to the existing value of `MY_PATH`):
...
But when this same job is run locally with `gitlab-ci-local`, the variable does not get updated — it just keeps whatever value it already had, as if the expression was never evaluated.
...
If you need to access the repo's image registry (or another GitLab repo's image registry) you will need to set both `CI_JOB_TOKEN` and `CI_REGISTRY_IMAGE` in `.gitlab-ci-local-variables.yml`.
...
To permanently remove files from a repository's history, GitLab provides a feature called [Repository maintenance](https://docs.gitlab.com/ee/user/project/repository/repository_size.html) which can be used alongside the [git-filter-repo tool](https://github.com/newren/git-filter-repo).[^gitlab-repository-size]
...
Go to _Settings > Repository > Repository maintenance_ in the GitLab web interface. Upload the `commit-map` file and then click "Start cleanup" as shown below.

<Image src={import('./_assets/gitlab-ui-repo-maintenance-start-cleanup.webp')} width="600px">A screenshot of the GitLab UI showing the Repository maintenance page and the "Start cleanup" button.</Image>
