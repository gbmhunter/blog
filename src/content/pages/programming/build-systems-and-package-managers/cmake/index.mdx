...
The output of cmake is standard build/project files (e.g. Makefiles if targeting GNU [make](/programming/makefiles/)). These build files are then run using the standard build command to generate the final binaries/executables.
...
One thing that cmake does differently from other "languages" is enforce that all variables passed into a function are linked by naming, rather than their order.
...
cmake calls the modules that convert cmake code into these various build systems **generators**.
...
This creates a new variable called `SOURCES`.
...
`link_libraries()` causes all subsequent targets in the `CMakeLists.txt` to be linked to this library.
...
```cmake
file(COPY src/ DESTINATION include/myproj
...
```cmake
add_definitions(-DMY_DEF_1 -DMY_DEF_2)
```
...
This command adds the flags to all sources in the current directory and below.
...
`add_custom_command()` is used to specify how to make the defined OUTPUT files, based on command-line(s) COMMAND.
...
Basic maths (addition, subtraction, etc.) can be done in a `CMakeLists.txt` file via the `math()` function. e.g. to increment a counter we would use the syntax:
...
This is of course assuming an out-of-source build setup (the recommended approach!). This standard way of building is used by [OpenCV](https://opencv.org/).
