...
1. _Specific thermal conductivity_ is a property of the material, irrespective of it's shape, length, size, etc. It has units $ W \cdot m^{-1} \cdot K^{-1} $. This is what we were talking about above.
1. _Absolute thermal conductivity_ (and _absolute thermal resistance_) is used when talking about the thermal conductance of a via, copper plane, PCB, etc. This value takes into account both the material and it's shape/length/size. This has units $ W \cdot C^{-1} $. **Absolute thermal resistance is the value mentioned on component datasheets.**
...
Thermal vias are a very common way of dissipating heat away from an IC or other component.
