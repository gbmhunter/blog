...
authors: [gbmhunter]
date: 2017-11-02
description: A collection of analytical geometry recipes, including tests for whether a point is inside a rectangle, the distance between a point and a line in 3D, and the angle between two lines in 3D.
...
2. For each line segment, calculate the angle between this line and a line formed from the point and the first vertex which makes up the line (angles are measured as positive in the clockwise direction, matching the direction of traversal).
...
$A$, $B$ and $D$ are 3 adjacent vertices of the rectangle<br/>
...
<p className="centered">where:<br/>
$ \times $ is the [cross-product](/mathematics/geometry/cross-product/)<br/>
$ |\text{vector}| $ is the magnitude of the vector<br/></p>
...
<Image src={import('./_assets/3d-point-to-line-distance-diagram-p1-p2-p3-p4.png')} width="475px">A diagram showing a 3D point to line distance problem.</Image>
...
$$ \mathbf{p4} = \mathbf{p1} + u(\mathbf{p2} - \mathbf{p1}) \label{eq:point4} \tag{1} $$

We want to find $u$ so we can then find $\mathbf{p4}$. Since there are two unknowns, we will need another equation. We can form one from the knowledge that the line from $\mathbf{p1}$ to $\mathbf{p2}$ and the line that connects the point $\mathbf{p3}$ to this line will be perpendicular, which means that the dot product will be 0. We can write this as:

$$ \begin{equation} (\mathbf{p3} - \mathbf{p4}) \cdot (\mathbf{p2} - \mathbf{p1}) = 0 \label{eq:dot-product} \tag{2} \end{equation}$$

Substituting $ \eqref{eq:point4} $ into $ \eqref{eq:dot-product} $ gives us:

$$ (\mathbf{p3} - \mathbf{p1} - u(\mathbf{p2} - \mathbf{p1})) \cdot (\mathbf{p2} - \mathbf{p1}) = 0 \label{eq:substituted} \tag{3} $$

Expanding $\eqref{eq:substituted}$ gives us:

$$ \mathbf{p3} \cdot (\mathbf{p2} - \mathbf{p1}) - \mathbf{p1} \cdot (\mathbf{p2} - \mathbf{p1}) - u(\mathbf{p2} - \mathbf{p1})\cdot(\mathbf{p2} - \mathbf{p1}) = 0 $$

Factorizing and realizing that $ (\mathbf{p2} - \mathbf{p1})\cdot(\mathbf{p2} - \mathbf{p1}) $ is the norm squared:

$$ (\mathbf{p3} - \mathbf{p1}) \cdot (\mathbf{p2} - \mathbf{p1}) - u\Vert\mathbf{p2} - \mathbf{p1}\Vert^2 = 0 $$

Re-arrange for $u$:

$$ u = \frac{(\mathbf{p3} - \mathbf{p1}) \cdot (\mathbf{p2} - \mathbf{p1})}{\Vert\mathbf{p2} - \mathbf{p1}\Vert^2} $$
