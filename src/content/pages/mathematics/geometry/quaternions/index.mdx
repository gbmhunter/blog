...
authors: [gbmhunter]
date: 2014-11-13
description: A tutorial on quaternions, including a calculator to convert between quaternions, rotation matrices, Euler angles and axis-angle notation.
...
import Rotations3d from 'src/components/tools/3d-rotations/Rotations3d.jsx';

A quaternion (pronounced _kwo-TER-nee-on_) is a complex-number-like system which contains three imaginary components and one real component. Arguably, the most useful quaternions are a subset of all quaternions called **unit quaternions** (or **versors**), which can be used to describe a rotation in 3D space. This page focuses primarily on these unit quaternions.
...
<p className="centered">  where:<br />
  ...
  (which can be seen as similar to the complex number $\mathbf{i}$)</p>
...
From the rules above, you can determine some identities:
...
Using the rule that $\mathbf{i}^2 = -1$:
...
The same process can be applied the other way around and also to $\mathbf{j}$ and $\mathbf{k}$ resulting in the following six identities:
...
All you need to do is multiply all of the imaginary terms by $ -1 $. The conjugate can also be denoted by $ \mathbf{q}^* $ or (by analogy with the matrix transpose) $ \mathbf{q}^T $. However $ \mathbf{\bar{q}} $ is the preferred choice on this site.

The conjugate of a unit quaternion is the same as its inverse.
...
Interestingly, all rotations in 3D space (which is called the 3D rotation group, or $SO(3)$) can be represented by the unit quaternions (which are a subset of all quaternions). Note that the unit quaternions *double cover* the rotations — $\mathbf{q}$ and $-\mathbf{q}$ represent the same rotation. In fact, you will rarely ever deal with a quaternion that is not a unit quaternion!
...
where the coefficients are stored in an array and the quaternion units are implied by position.
...
- [Euler angles (roll, pitch, yaw)](/mathematics/geometry/euler-angles/)
- [Rotation matrices](/mathematics/geometry/rotation-matrices/)
- Quaternions

Why choose quaternions? One reason is that quaternions do not suffer from the _gimbal lock_ that Euler angles do. This also is related to the fact that they are differentiable (i.e. very small changes in rotation cause very small changes in the quaternion values), which allows for smooth interpolation, important for many use cases including 3D animation. They are also more compact, with only four numbers needing to be stored compared to the nine of a rotation matrix. Quaternions are also easily converted into the angle-axis representation and back again.
...
$$
\begin{bmatrix}0\\v'\end{bmatrix} = q \begin{bmatrix}0\\v\end{bmatrix}\bar{q}
$$

Because the vector is squashed between the quaternion and its conjugate, this is sometimes referred to as the "sandwich product". The multiplications can be done by the basic product rule, or more easily using the _Hamilton product_.

Let's assume we have the vector:
...
This quaternion just so happens to be a rotation of $90^{\circ}$ around the Y axis.
...
$$
q1 = q1_w + q1_x \mathbf{i} + q1_y \mathbf{j} + q1_z \mathbf{k}
$$

then using the Hamilton product, the values of each component in $q3$ are:
...
\begin{align*}
  ...
  q3_z &= q1_z q2_w - q1_y q2_x + q1_x q2_y + q1_w q2_z
\end{align*}
$$

Applying the Hamilton product twice to our example above (first to combine $q$ and the vector quaternion, and then to combine that result with $\bar{q}$) gives:

$$
v' = \begin{bmatrix}0\\0\\0\\-1\end{bmatrix}
$$

Dropping the $w$ component, our rotated vector is $(0, 0, -1)$ — exactly what we'd expect when rotating the X unit vector $90^{\circ}$ around the Y axis.

You can also rotate a vector $\vec{v}$ by a quaternion $q$ by decomposing the quaternion into its scalar part $w$ and its vector part $\vec{r}$:
...
<p className="centered">  where:<br />
  $ \times $ is the vector cross-product<br /></p>

## Rotation Matrix

You can calculate a 3×3 [rotation matrix](/mathematics/geometry/rotation-matrices/) from a quaternion. This is useful if you want to express the rotation as a matrix instead of a quaternion, but comes at the expense of having to store 9 numbers rather than 4! See the conversion section below for the equation.
...
Remember that **quaternion multiplication is not commutative**, so the ordering of $ R_A $ and $ R_B $ is important.
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>Rotation of 180° around the X axis.</td>
        </tr>
        <tr>
            <td>$$ \mathbf{q} = [0, 0, 1, 0] $$</td>
            <td>Rotation of 180° around the Y axis.</td>
        </tr>
        <tr>
            <td>$$ \mathbf{q} = [0, 0, 0, 1] $$</td>
            <td>Rotation of 180° around the Z axis.</td>
        </tr>
        <tr>
            <td>$$ \mathbf{q} = [\sqrt{0.5}, \sqrt{0.5}, 0, 0] $$</td>
            <td>Rotation of 90° around the X axis.</td>
        </tr>
        <tr>
            <td>$$ \mathbf{q} = [\sqrt{0.5}, 0, \sqrt{0.5}, 0] $$</td>
            <td>Rotation of 90° around the Y axis.</td>
        </tr>
        <tr>
            <td>$$ \mathbf{q} = [\sqrt{0.5}, 0, 0, \sqrt{0.5}] $$</td>
            <td>Rotation of 90° around the Z axis.</td>
        </tr>
        <tr>
            <td>$$ \mathbf{q} = [\sqrt{0.5}, -\sqrt{0.5}, 0, 0] $$</td>
            <td>Rotation of -90° around the X axis.</td>
...
SLERP does not work well when the two quaternions are too similar. In this case it is safer to just perform linear interpolation on all four quaternion values independently (making sure to normalize the resulting quaternion before using it).

The following Python code example demonstrates the SLERP algorithm on a pair of quaternions.[^wikipedia-slerp]
...
A rotation in axis-angle form (where the axis is a unit vector):
...
<Rotations3d client:load />

{/* ============================================================================================ */}
{/* REFERENCES */}
{/* ============================================================================================ */}

[^wikipedia-slerp]: Wikipedia. _Slerp_ [wiki]. Retrieved 2026-07-07, from https://en.wikipedia.org/wiki/Slerp.
