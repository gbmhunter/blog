...
authors: [gbmhunter]
date: 2018-07-01
description: A tutorial showing how to fit a polynomial curve to a set of data using the least squares method, including a worked example.
...
<Image src={import('./_assets/many-polynomial-best-fits-on-graph.png')} width="605px">A graph showing how different degree polynomials can be fitted to data.</Image>
...
$$
\begin{align*}
err = \sum_{i = 1}^{n} (y_i - (a_0 + \sum_{j=1}^k a_j x_i^j))^2
\end{align*}
$$
...
We have $k + 1$ unknowns, $a_0, a_1, ..., a_k $. We have to take the derivative of each unknown separately:

$$
\begin{align*}
\frac{\partial err}{\partial a_0} &= -2 \sum_{i=1}^{n} (y_i - (a_0 + \sum_{j=1}^k a_j x_i^j)) &= 0 \\  
\frac{\partial err}{\partial a_1} &= -2 \sum_{i=1}^{n} (y_i - (a_0 + \sum_{j=1}^k a_j x_i^j))x_i &= 0 \\  
\frac{\partial err}{\partial a_2} &= -2 \sum_{i=1}^{n} (y_i - (a_0 + \sum_{j=1}^k a_j x_i^j))x_i^2 &= 0 \\  
\vdots \\  
\frac{\partial err}{\partial a_k} &= -2 \sum_{i=1}^{n} (y_i - (a_0 + \sum_{j=1}^k a_j x_i^j))x_i^k &= 0 \\
\end{align*}
$$

These equations can be re-arranged into matrix form, $\mathbf{A} \mathbf{x} = \mathbf{B}$:
...
\sum x_i^k & \sum x_i^{k+1} & \sum x_i^{k+2} & ... & \sum x_i^{2k} \\  
...
We solve this by re-arranging (which involves taking the inverse of $\mathbf{A}$):

$$ \mathbf{x} = \mathbf{A^{-1}} \mathbf{B} $$

The elements of $\mathbf{x}$ are the coefficients $a_0, a_1, ..., a_k$, so the polynomial curve of best fit is:

$$ y = a_0 + a_1 x + a_2 x^2 + ... + a_k x^k $$

See <a href="/misc/polynomial-curve-fitting/main.py" download>main.py</a> for Python code which performs these calculations.

## Worked Example

Find a degree-2 polynomial that best describes the following points:
...
B_{21} &= \sum x_i y_i = 1 \cdot 1 + 2 \cdot 3 + 3 \cdot 4 + 4 \cdot 8 = 51 \\  
B_{31} &= \sum x_i^2 y_i = 1^2 \cdot 1 + 2^2 \cdot 3 + 3^2 \cdot 4 + 4^2 \cdot 8 = 177 \\
\end{align*}
$$
...
<Image src={import('./_assets/2-degree-poly-best-fit-graph.png')} width="600px">Four data points and a degree-2 polynomial of best fit (using the least squares method).</Image>
