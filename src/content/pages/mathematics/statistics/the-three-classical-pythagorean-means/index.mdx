...
<p className="centered">  where:<br/>
  $ \Pi $ is the product of the terms<br/></p>
...
<Image src={import('./geometric-mean-2d-same-area.png')} width="500px">Taking the geometric mean of a rectangle (side lengths of 4 and 9) gives you length of the sides of a square (side length of 6) with the same area.</Image>
...
<Image src={import('./geometric-mean-3d-same-volume.png')} width="500px">Taking the geometric mean of a box (side lengths of 8, 2 and 4) gives you length of the sides of a cube (side length of 4) with the same volume.</Image>
...
The harmonic mean is used to calculate the $F_1$ score when performing classification. Because the harmonic mean is dominated by the smaller of the values it is given, it punishes a classifier that is good at one of precision/recall but poor at the other — see [Classification Metrics](/programming/artificial-intelligence/classification-metrics/) for what those two measure and how the $F_1$ score is built from them. This is useful in classification algorithms such as [logistic regression](/programming/artificial-intelligence/understanding-logistic-regression/).
