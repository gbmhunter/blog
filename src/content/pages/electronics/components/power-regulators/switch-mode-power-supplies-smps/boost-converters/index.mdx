...
1. When the switch opens ($t_{off}$), the inductor current is forced through the diode to the output. The voltage across the inductor is now $V_{IN} - V_{OUT}$ (negative, since $V_{OUT} > V_{IN}$), so the current ramps back down. The inductor delivers its stored energy to the output **in series with the input source**, which is how the output reaches a higher voltage than the input.
