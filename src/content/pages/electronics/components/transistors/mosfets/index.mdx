...
If you need lower leakage currents than what you can achieve with a MOSFET, try using a J-FET. They have typical leakage currents of $1-10nA$.
...
FinFETs are multi-fin FETs which overcome issues once MOSFETs approach very small sizes (such as 22nm).
...
1. As the current reaches a certain value, $V_{GS}$ across $Q1$ will become negative enough to reach its $V_{GS(th)}$ (which is between -2.1V and -1V for the BSP149[^infineon-bsp149-ds]). At this point it will begin turning $Q1$ off, $Q1$ will begin to drop voltage across it and hence limit the current of the circuit.
...
From the datasheet of the BSP149, $V_{GS(th)}$ is between -2.1V and -1V[^infineon-bsp149-ds]. Unfortunately $V_{GS(th)}$ is never a well defined parameter, so we'll just pick a value in the middle of $V_{GS(th)} = -1.5V$.
...
Avalanche in a MOSFET is when $V_{DS}$ exceeds the breakdown voltage and the drain-body PN junction (body diode) enters reverse breakdown via impact ionization. The voltage field is strong enough that carriers are accelerated to a point where they can knock more carriers loose, creating a chain reaction. Avalanche most commonly occurs at switch off of a MOSFET -- parasitic inductance in the circuit can cause a large voltage spike as the current abrutly stops (remember: V = L * di/dt).
...
The most common metric is the single-shot Unclamped Inductive Switching (UIS) energy ($E_{DS(AL)S}$). Below is a circuit diagrams of the standard Unclamped Inductive Switching (UIS) test circuit used to measure MOSFET avalanche ruggedness ($Q1$). The pulse generator applies a gate voltage and turns $Q1$ on long enough to ramp the inductor current up to a target value, then switches it off; the inductor forces the drain voltage up into avalanche, and the MOSFET dissipates the energy.

<Image iref="fig-uis-test-circuit" src={import('./_assets/unclamped-inductive-load-test-circuit-simple.webp')} width="500px">A circuit diagram of the standard Unclamped Inductive Switching (UIS) test circuit.</Image>
...
The failure mechanism for a single-shot avalanche event in a power MOSFRT

### Avalanche Rated MOSFET Examples

Nexperia's ASFET range are designed for automotive solenoid switching and have guaranteed avalanche performance, tested up to a billion cycles.[^nexperia-asfets-repetitive-avalanche]
...
1. Power Limit: The power limit line is determined by the maximum power dissipation the MOSFET can handle before the junction temperature exceeds its maximum value (typically between 100-200°C). This line is dependent on the case-to-ambient thermal resistance (which is specific to the PCB/environment the MOSFET is used in!) and ambient temperature, so the best the MOSFET manufacturer can do is assume a sensible value (and hopefully state the assumption in the datasheet).
