...
BJT transistors were the second type of transistor to be mass produced (superseding the Type-A transistor), at first in very large quantities.[^bib-pbs-evolution-of-tran]

They are used for making things such as amplifiers, switches, [linear regulators](/electronics/components/power-regulators/linear-regulators/), [current mirrors](/electronics/components/current-sources-and-sinks/current-mirrors/) and digital logic.
...
<Image src={import('./_assets/bjt-schematic-symbols-npn-pnp.png')} width="600px">Schematic symbols for NPN and PNP transistors. Note that the collector and emitter have flipped positions for the PNP, as commonly drawn on schematics.</Image>
...
_BJT characteristics_ is a family of curves that show you how a BJT responds to various currents and voltages across its terminals. Normally two parameters are varied whilst a third is held constant. BJT characteristics can be broken down into:
...
The characteristic output curve for a BJT shows how the collector current $I_C$ changes as the collector-emitter voltage $V_{CE}$ changes, at a fixed base current $I_B$. This curve is shown for a number of base currents to cover a range of operation points, and generally you can interpolate between the curves for your specific base current if needed.

The below figure shows the simulated output transfer characteristics for the popular 2N2222 BJT. $I_C$ is plotted against $V_{CE}$ for a range of base currents $I_B$ varying from $0$ to $5\unit{mA}$:

<Image src={import('./_assets/output-transfer-characteristics-microcap-sim/plot.png')} width="600px">The output transfer characteristics (simulated) for the 2N2222 BJT.</Image>

(Micro-Cap simulation file: [circuit.cir](/misc/bipolar-junction-transistors-bjts/output-transfer-characteristics-microcap-sim/circuit.cir))

## Important Parameters

### Beta And Gain (B, hfe)

The gain of a BJT is the ratio of the collector current to the base current (hence it is a _current gain_), typically when measured with the BJT in a common-emitter configuration. There are few different gains and symbols used, so it's important to know what exact gain is being talked about:
...
All of them depend somewhat on other parameters such as collector current and temperature, but typically the gain is treated as a constant. The main thing to remember is **to not expect the current gain to equal an exact number, even between the same BJT transistor from the same manufacturing batch**.
...
- $F$: The _Forward_ current amplification
- $E$: As measured with the BJT in the _common-Emitter_ configuration 
...
The _Early Voltage_ is a parameter used to determine the _Early effect_. The Early effect describes the small increase in collector current as the collector-emitter voltage increases (with a BJT operating in its active region). As $V_{CE}$ on a BJT increases, the reverse-bias on the $V_{CB}$ junction increases (this is just a PN junction in reverse bias under typical operation). This increases the depletion region of this junction, which **reduces the effective width of the base**. Because the saturation current is inversely proportional to the effective width of the base, an increase in $V_{CE}$ results in an increase in $I_C$.
...
<Image src={import('./_assets/early-voltage-microcap-sim/vce-vs-ic-with-early-voltage.png')} width="700px">Graph of Vce vs. Ic showing how the Early voltage is the extrapolation of the curves (in the active region) back to the x-axis intersection.</Image>

(Micro-Cap simulation file: [early-voltage.cir](/misc/bipolar-junction-transistors-bjts/early-voltage-microcap-sim/early-voltage.cir))

Micro-Cap with the QNB transistor was used to simulate these $I_C$ vs. $V_{CE}$ curves. This model had an Early voltage `VAF` set to `45V`, which agrees well with the extrapolated curves and the x-axis intersection.
...
<p className="centered">where:<br/>
$k$ is Boltzmann's constant in Joules per Kelvin, which is $1.38 \times 10^{-23}\unit{JK^{-1}}$<br/>
$T$ is the temperature of the junction, in Kelvin $\mathrm{K}$<br/>
$q$ is the charge on an electron in Coulombs, which is $1.6 \times 10^{-19}\unit{C}$<br/></p>

At a room temperature of $22\unit{^{\circ}C}$, $V_T$ is approximately $25\unit{mV}$. $25\unit{mV}$ is a good enough approximation for the thermal voltage in many scenarios without taking the actual junction temperature into account. The thermal voltage is used in the hybrid-pi model of the BJT transistor.
...
<Image src={import('./_assets/hybrid-pi-model-simplified.png')} width="600px">A simplified Hybrid-Pi model of a BJT transistor.</Image>
...
By using $h_{fe} = \frac{I_C}{I_B}$ and the expression for $g_m$ above, we can substitute into the equation for $r_{\pi}$ to re-write it as:
...
NPNs require a small positive base-emitter voltage to create a current which flows into the base. This current, multiplied by the gain of the transistor, determines the collector-to-emitter current (well, to be technically correct, the _maximum_ collector current). Because of this, an NPN transistor will only conduct when both the base and collector have a higher voltage than the emitter.
...
- [Common-collector amplifier](/electronics/circuit-design/bjt-common-collector-amplifier/)
...
<Image src={import('./_assets/common-base-amplifier.png')} width="400px">Basic schematic of a NPN BJT common-base amplifier. DC biasing componentry is not shown.</Image>
...
<Image src={import('./_assets/common-base-amplifier-dc-bias.png')} width="700px">Schematic of a NPN BJT common-base amplifier with DC biasing componentry shown.</Image>
...
r_{in} &= \frac{v_{in}}{i_{in}} \\
       &= \frac{v_e}{i_e} \\
       &= \frac{i_e \cdot (r_e'\,||\,R_E)}{i_e}  &\text{Replacing $v_e$} \\
       &= r_e'\,||\,R_E                          &\text{$i_e$'s cancel out}
\end{align*}
$$
...
<table>
  ...
  <tbody>
    <tr><td>Common-emitter</td> <td>Moderate (-Rc/Re)</td> <td>Moderate (B)</td> <td>Moderate</td> <td>High</td></tr>
...
BJTs can be configured to sink a relatively constant amount of current which is independent of the output-side voltage. This can be a useful way of driving an [LED](/electronics/components/diodes/light-emitting-diodes-leds/) from a microcontroller with a constant current, no matter what voltage source is used to drive the LED. BJT current sinks and sources are good for simple, cheap situations where high precision is not the name of the game. If you want high precision, your best bet is to build a [current-sink from an op-amp](/electronics/components/current-sources-and-sinks/voltage-to-current-converters/#op-amp-current-sink).

<Image src={import('./_assets/constant-current-bjt-based-led-driver.png')} width="300px">The simulation schematic for a constant-current BJT-based LED driver.</Image>

The schematic above was designed to drive the LED with $10\unit{mA}$ of current when the BJT was driven from a microcontroller running at $+3.3\unit{V}$. Since $+3.3\unit{V}$ is applied to the base of the NPN transistor, the transistor will always turn on just enough so that the voltage at the emitter is $0.7\unit{V}$ less, e.g.
...
Since we know the emitter voltage is going to be $+2.6\unit{V}$, we can choose the right resistor, $R_1$ to get the LED current we desire (remember that the current out of the emitter is pretty much equal to the current into the collector).
...
So if we want an LED current of $10\unit{mA}$, that means we need $R_1 = 260\unit{\Omega}$. The closest [E12 value](/electronics/components/resistors/) is $270\unit{\Omega}$.

Notice how the LED current is independent of the $+12\unit{V}$. The $+12\unit{V}$ can change to say, $+9\unit{V}$ and the LED current will still be $10\unit{mA}$. The current draw from the microcontroller into the base of the transistor will be very low (somewhere around $100\unit{\mu A}$).

Below are the simulation results for the schematic above, showing the LED current to be indeed $10\unit{mA}$. It works!

<Image src={import('./_assets/constant-current-bjt-based-led-driver-simulation-results.png')} width="800px">The simulation results of a constant-current BJT-based LED driver.</Image>

#### Using A Resistor Divider To Drive The Base

A resistor divider can be a simple way to drive the base of an NPN current-sink if you don't need active control. This works well if the supply voltage is known and stable, as the current will fluctuate with supply voltage (if this is going to be an issue, consider using a [Zener-based circuit](/electronics/components/diodes/zener-diodes/) to drive the base of the NPN BJT). Schematics of the design are shown below.

<Image src={import('./_assets/current-source-npn-voltage-divider-base.png')} width="500px">A NPN BJT current-sink using a resistor divider to drive the base.</Image>

<Aside type="example">

**Design Procedure:**

1. Choose the resistor-divider $R_1$ and $R_2$ to provide a voltage at the base of the transistor in the region of $2.0$ to $5.0\unit{V}$. I choose $R_1 = 10\unit{k\Omega}$ as this is a standard resistance, and then $R_2 = 2.2\unit{k\Omega}$ to give a $V_B = 2.16\unit{V}$. This uses the standard resistance divider rule, which assumes negligible current is drawn from the resistor divider through the base of the transistor.

1. Subtract $0.7\unit{V}$ from $V_B$ to get $V_E$. In this case, $V_E = 1.46\unit{V}$.

1. Size $R_E$ to set the desired current of your current sink. Using Ohm's Law, $R_E = \frac{V_E}{I}$. In this case we wanted $2\unit{mA}$ to drive an LED, so:
    $$
    \begin{align*}
    R_E &= \frac{1.46\unit{V}}{2\unit{mA}} \\
        &= 730\unit{\Omega} \\
        &\approx 732\unit{\Omega} \text{ (closest E96 value)}
    \end{align*}
    $$

1. As a sanity check, make sure the output impedance of the resistor divider is much less than the input impedance looking into the base of the BJT (otherwise the resistor divider output will get significantly loaded and its output voltage will drop). That is:
    $$
    \begin{align*}
    R_1 || R_2 &\ll \beta R_E \\
    \frac{10\unit{k\Omega} \cdot 2.2\unit{k\Omega}}{10\unit{k\Omega} + 2.2\unit{k\Omega}} &\ll 100 \cdot 732\unit{\Omega} \\
    1.80\unit{k\Omega} &\ll 73.2\unit{k\Omega}
    \end{align*}
    $$
...
A _current mirror_ is a current-copying circuit where the current in one BJT is programmed via a resistor and is used to control the current in a second BJT which is used to drive the same current into a load. The current-mirrors shown below are built with BJTs, but other active transistors such as [MOSFETs](/electronics/components/transistors/mosfets/) can also be used. The below image shows a basic PNP BJT-based current mirror. 

<Image src={import('./_assets/current-mirror-pnp.png')} width="400px">A basic PNP-based current mirror programmed to source 1mA into the load. Q1 and Q2 should be a matched transistor pair to achieve good mirroring of the current.</Image>

<Aside type="example">

**Design Procedure:**

1. Decide on the program current, $I_P$. This will also be the current through the load. We'll use this value later! For this example we'll choose $1\unit{mA}$.

1. Find the voltage across $R_1$, noting that $Q_1$ has a diode voltage drop of $0.7\unit{V}$ from emitter to base (with the emitter tied to $V_{CC}$), and that the base and collector of $Q_1$ are tied together and hence at the same voltage:
    $$
    \begin{align*}
    V_{R1}  &= 12\unit{V} - 0.7\unit{V} \\
            &= 11.3\unit{V}
    \end{align*}
    $$
...
For more info, see the [Current Mirrors page](/electronics/components/current-sources-and-sinks/current-mirrors/).

### Capacitance Multipliers

Go to the [Capacitance Multipliers](/electronics/components/capacitors/#capacitance-multipliers) section to see how BJT transistors are used in capacitance multipliers.
...
<Image src={import('./_assets/bc548-transistor-to92-photo.png')} width="300px">A photo of the ubiquitous BC548 BJT transistor in the TO-92 package. Image from https://www.dnatechindia.com/bc-548-npn-transistor-buy-online-india.html.</Image>

### S8050/S8550

The `S8050` is a popular NPN BJT made by a number of manufacturers. Most versions generally have a maximum $I_C$ of $700\unit{mA}$ and a maximum $V_{CEO}$ of $20\unit{V}$.[^bib-unisonic-s8050-ds] $h_{FE}$ can vary from approx. $120$ to around $400$ depending on the suffix code. The `S8050` is normally found in the [TO-92-3 case](/pcb-design/component-packages/to-92-component-package/).
...
<Image src={import('./_assets/multiple-emitter-bjt-schematic-symbol-u1.png')} width="300px">The schematic symbol for a multiple-emitter BJT.</Image>
...
<Image src={import('./_assets/basic-two-input-tll-nand-gate-schematic.png')} width="600px">The schematic of a basic two-input TTL NAND gate.</Image>
...
1. Secondary breakdown (only applicable to power BJTs, see the section below)

<Image src={import('./_assets/bjt-safe-operating-area.png')} width="500px">A typical representation of the safe operating area (SOA) of a BJT. Normally multiple curves are drawn, one for DC and a number for pulses of various lengths. Both $V_{CE}$ and $I_C$ are on logarithmic axes.</Image>

### What Is Second Breakdown?

_Second breakdown_ (a.k.a. _secondary breakdown_) is a limitation on the SOA that is typically only an issue for power BJTs which are designed to handle high voltages and currents. Under large voltages and currents, hot spots can develop across the working area of the BJT device. Because a BJT has a negative temperature coefficient, these hot spots can cause thermal runaway and destroy the BJT.
...
BJTs do not act as perfect switches. When "off", a small amount of leakage current will flow depending on the applied voltage to each of its terminals. Leakage currents for BJT are normally in the $\mathrm{nA}$ range. For many applications, this amount of leakage does not affect the operation in any significant way. However, for certain applications such as extremely low-power devices and low-current measurement circuits, some $\mathrm{nA}$ of leakage can be large enough to cause issues.
...
$I_{cbo}$ (collector-base cut-off current) is the leakage from the collector to base with the emitter open-circuit (i.e. not connected to anything). The `o` stands for "open". This is essentially the leakage current of the collector-base diode with reverse bias. It is the most common leakage current value (if any) that you'll find on a BJTs datasheet. The onsemi BC547BTF gives a $I_{cbo,max} = 15\unit{nA}$ in its datasheet.[^bib-onsemi-bc546-ds]
...
<Image src={import('./_assets/leakage-current-diagrams.png')} width="800px">Circuits showing how each 1 of the three BJT leakage currents is measured.</Image>
...
I also found this older "Micronta Transistor Tester" device on TradeMe many years ago, I bought it purely out of interest (Micronta being a brand belonging to Radio Shack):

<div style={{ display: 'grid' }}>
<div style={{ gridRow: 1, gridColumn: '1/3' }}><Image src={import('./_assets/micronta-transistor-tester-box.jpg')} width="600px">The front box.</Image></div>
<div style={{ gridRow: 2, gridColumn: '1' }}><Image src={import('./_assets/micronta-transistor-tester-front-panel.jpg')} width="300px">The front panel.</Image></div>
<div style={{ gridRow: 2, gridColumn: '2' }}><Image src={import('./_assets/micronta-transistor-tester-internals.jpg')} width="300px">The internal circuitry.</Image></div>
</div>
...
If you are looking for a slice of history and some informative transistor information, check out the [1964 edition of the GE Transistor Manual](https://files.mbedded.ninja/pdfs/ge-transistor-manual-1964.pdf).

[Phototransistors](/electronics/components/sensors/phototransistors/) are very similar to a standard NPN BJT, but use light to drive the base rather than an electrical signal.

{/* ======================================================================== */}
{/* REFERENCES */}
{/* ======================================================================== */}

[^bib-haenichen-interview]: Transistor Museum. _Oral History - Jack Haenichen, Page 11_ [interview]. Retrieved 2021-06-20, from http://www.semiconductormuseum.com/Transistors/Motorola/Haenichen/Haenichen_Page11.htm.
[^bib-wikipedia-soa]: Wikipedia. _Safe operating area_ [wiki]. Retrieved 2021-08-23, from https://en.wikipedia.org/wiki/Safe_operating_area.
...
[^bib-wikipedia-hybrid-pi-model]: Wikipedia (2020, Mar 22). _Hybrid-pi model_ [wiki]. Retrieved 2022-08-14, from https://en.wikipedia.org/wiki/Hybrid-pi_model.
...
[^bib-onsemi-bc546-ds]: onsemi (formerly Fairchild) (2002). _BC546 / BC547 / BC548 / BC549 / BC550: NPN Epitaxial Silicon Transistor_ [datasheet]. Retrieved 2022-10-10, from https://nz.mouser.com/datasheet/2/308/BC550_D-1802078.pdf.
...
[^bib-unisonic-s8050-ds]: Unisonic. _S8050 NPN Silicon Transistor: Low Voltage High Current Small Signal NPN Transistor_ [datasheet]. Retrieved 2022-10-24, from http://www.unisonic.com.tw/datasheet/S8050.pdf.
