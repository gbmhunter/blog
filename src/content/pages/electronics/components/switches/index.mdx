...
Toggle switches are a type of switch than can be toggled between multiple positions (typically two). SPDT (which has three leads) is a very common toggle switch style. <IRef iref="fig-toggle-switch-spdt-photo" /> shows a photo of the ANT11SECQE SPDT toggle switch from CIT.[^digikey-cit-ant11secqe-spdt-toggle-switch]

<Image iref="fig-toggle-switch-spdt-photo" src={import('./_assets/toggle-switch-spdt-photo.webp')} width="400px">A photo of the ANT11SECQE SPDT toggle switch from CIT.[^digikey-cit-ant11secqe-spdt-toggle-switch]</Image>
