...
250-693 Wh/L (0.90-2.49 MJ/kg)[^wikipedia-lithium-ion-battery]
...
<Image src={import('./_assets/lithium-ion-self-discharge-vs-time.svg')} width="640">State of charge over time for a lithium-ion battery starting at 100 %, plotted for 1 %, 3 % and 5 % monthly self-discharge rates. Because each month's loss is a percentage of the *remaining* charge, the decay is exponential ($SoC(t) = 100 \cdot (1 - r)^t$).</Image>
...
[Protection circuit modules](/electronics/components/power-management-ics/pcms/) (PCMs -- not to be confused with Pulse Code Modulation) are circuits designed to protect Lithium batteries (and other chemistries) from overcharge, over-discharge and over current conditions. They normally feature a analogue switch (i.e. two back-to-back [MOSFETs](/electronics/components/transistors/mosfets/)) to disconnect the load from the battery cell during a fault condition. They reconnect the load after the fault condition is removed and/or after a certain duration.
