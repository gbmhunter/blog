...
authors: [gbmhunter]
date: 2011-09-05
description: An introduction to capacitors, a fundamental component used in circuit design. A walk-through of the different types, properties, uses and capacitor equations.
...
import CapacitorCharge from 'src/components/tools/capacitor-charge/CapacitorCharge.jsx';
import CapacitorEnergy from 'src/components/tools/capacitor-energy/CapacitorEnergy.jsx';
import CapacitorImpedance from 'src/components/tools/capacitor-impedance/CapacitorImpedance.jsx';
import ParallelCapacitance from 'src/components/tools/parallel-capacitance/ParallelCapacitance.jsx';
import SeriesCapacitance from 'src/components/tools/series-capacitance/SeriesCapacitance.jsx';
...
The below image shows the common schematic symbols used for various types of capacitors:

<Image src={import('./_assets/capacitor-symbols.png')} width="500px">Schematic symbols used for various types of capacitors.</Image>
...
A **feedforward capacitor** is the name given to a capacitor placed between the VOUT and ADJ pins of a linear regulator to improve stability.
...
Class 1 capacitors primarily use calcium zirconate as their dielectric[^kemet-heres-what-makes-mlcc-dielectrics-different], which has a very stable permittivity across both temperature and DC bias. However, its permittivity (and hence dielectric constant) is much less than the dielectrics used for Class 2 MLCCs, and so the capacitances are much smaller (typically pF to nF).

The below table shows the EIA temperature codes for Class 1 ceramic capacitors:
...
`NP0` is used to refer to the same material as `C0G`, and so they are the same thing. As the image below shows, some manufacturers and suppliers refer to them together as `C0G/NP0`. `NP0` stands for "negative positive 0" and refers to the capacitance not having a positive nor negative change in capacitance with respect to temperature.

<Image src={import('./_assets/c0g-np0-capacitor-temp-coeff-grouping-digikey.png')} width="500">DigiKey, like many other suppliers, groups together C0G and NP0 as one temperature coefficient. Screenshot from https://www.digikey.com/product-detail/en/tdk-corporation/CGA4C2C0G1H392J060AA/445-6942-1-ND/2672960.</Image>

**Class 2:**

The below table shows the Class 2 temperature codes, based on the EIA RS-198 standard:
...
The below table lists codes from the JIS standard. They are not commonly seen in industry, the EIA codes above are far more popular.
...
<Image src={import('./_assets/murata-deformation-of-pcb-by-electrostrictive-phenomenon.jpg')} width="450">The deformation of a PCB due to the electrostrictive phenomenon in ceramic chip capacitors. Image from http://www.murata.com/products/capacitor/solution/naki.html.</Image>
...
AVX has coined the term _FLEXITERM_ for their range of ceramic chip capacitors that can withstand higher bending flex. The image below shows a diagram of the test setup for measuring the maximum bending flex.

<Image src={import('./_assets/avx-flexiterm-test-for-ceramic-chip-caps.png')} width="500">Diagram showing the test setup for measuring the maximum bending flex for AVX chip capacitors with "FLEXITERM" terminals[^bib-avx-auto-mlcc].</Image>
...
* Typical Marking: Because of their large size, the capacitance is usually printed in its absolute form on the cylinder.
* Uses:
  * Power supply bulk decoupling
  * Filtering
  * Audio bypass capacitors

Electrolytic capacitors use a very thin electrically deposited metal oxide film ($Al_2 O_3$) as their dielectric. They have a high capacitance density (well, that was before super-caps came along). They are usually cylindrical in shape, and come in through-hole (axial and radial) and surface-mount types.

In over-voltage conditions, holes can be punched through the dielectric layer and the capacitor will begin to conduct. The good news is that if the over-voltage disappears quickly enough (e.g. just a surge or spike), the capacitor can self-heal. The bad news is that if the capacitor heats up enough, the dielectric can boil, create vapours, and the cap explodes. Most electrolytics have a specific "weak spot" on the case which is designed to break in an over-pressure situation. This can make quite a bang, and can be dangerous if you happen to be peering closely at the circuit while this happens.

For the common size codes and sizes of SMD Electrolytic capacitors, see the [SMD Electrolytic Capacitor Packages page](/pcb-design/component-packages/smd-electrolytic-capacitor-packages).
...
The _capacitor plague_ was a period between 1999 and 2007 when **electrolytic capacitors experienced higher than expected failure rates** due to faulty design. The back story is one of intrigue, with the issue starting in 1999 with a scientist working for Rubycon Corporation in Japan stealing a mis-copied formula for a capacitor electrolyte and giving it to his former employer, Luminous Town Electric company in China. The faulty formula was further stolen abroad to Taiwan, and many faulty capacitors were produced as a result. Bulging and bursting electrolytic capacitors in things such as PC motherboards began making headlines in 2002[^bib-passive-component-industry-electrolytic-failures].
...
<Image src={import('./_assets/pcb-with-electrolyte-leakage-zoomed-out.jpg')} width="1000">Photo of a PCB in a Dell sound system. Highlighted is leaking electrolytic capacitor, which also corroded right through the lead of a neighbouring resistor.</Image>

The image below shows a close-up of the faulty electrolytic capacitor. You can't see the corroded resistor as it is hidden above the orange disk-style capacitor and inductor-looking component. 

<Image src={import('./_assets/pcb-with-electrolyte-leakage-zoomed-in.jpg')} width="800">A close-up of the faulty electrolytic capacitor.</Image>
...
Tantalum capacitors are a special type of electrolytic capacitor. But they are categorized distinctly from general electrolytic capacitors because of their special properties and wide-spread use. They have lower ESR, lower leakage and higher temperature ranges (up to 125°C) than their general electrolytic counterparts. The image below shows an example of through-hole tantalum capacitors.

<Image src={import('./_assets/container-of-th-tantalum-caps.jpg')} width="700">Through-hole tantalum capacitors.</Image>
...
_Wet_ tantalum capacitors are a newer form of tantalum capacitor, with a better energy density than solid types. Higher price than solid tantalum capacitors. Wet tantalum capacitors have the interesting property of being able to **self-heal** and stop leakage current due to breakdown[^bib-nasa-self-healing-tantalum]. The wet electrolyte is able to deliver oxygen and reform the oxide layer in locations where it is compromised[^bib-wikipedia-tantalum-capacitor].

#### Construction

At the heart of a tantalum capacitor is a pellet of sintered tantalum metal ($Ta$); the dielectric is a thin layer of tantalum pentoxide ($Ta_2 O_5$) grown on its surface, as shown in the image below.

<Image src={import('./_assets/cross-section-of-tantalum-capacitor.png')} width="500">Cross-section of a SMD tantalum capacitor.</Image>
...
The SILLIEST THING about tantalum capacitors is that the polarity indicator is a stripe, next to the POSITIVE end. It goes against pretty much all other stripy-polarity-mark thingies, which all indicate which end is the negative end (think electrolytics, diodes, etc.). So, be very careful and vigilant when using these, for it is so easy to forget this rule!

Tantalum capacitors are more susceptible to reverse and over-voltage than their electrolytic counterparts. At a high enough voltage, the dielectric breaks down and the capacitor begins to conduct. The current can generate plenty of heat, and here's the best part, it can start off a **mini-thermite** reaction between tantalum and manganese dioxide.
...
* Typical Marking: Because of their large size, the capacitance is usually either in `<number><number><multiplier><tolerance>` picofarad form (e.g. `105K` equals `10e^5pF` equals `1uF`), or because of their large size printed in its absolute form (e.g. `0.1uF`) on the block somewhere.
* Uses:
  * Power supplies
  * Audio circuits
...
<div style={{ display: 'flex' }}>
  <Image src={import('./_assets/yellow-potted-film-capacitor-alibaba.png')} width="200">A yellow potted film capacitor. Image from alibaba.com.</Image>
  <Image src={import('./_assets/blue-potted-film-capacitor-hitano.png')} width="200">A blue potted film capacitor from Hitano.</Image>
</div>

The image below shows a film capacitor that has a rounded, red case that's been coated in an epoxy lacquer, with the leads coming out of the same side.

<Image src={import('./_assets/red-radial-film-capacitor-photo-ecq-p1h822gz3-digikey.png')} width="300">A red radial film capacitor (Panasonic ECQ-P1H822GZ3). Image from digikey.com.</Image>
...
Metallized film capacitors have a self-healing effect when an over-voltage event occurs, while others such as ceramic capacitors do not. This makes them safer to use in high-power applications. The image below shows a blown 1uF 250VAC metallized polyester film capacitor:

<Image src={import('./_assets/20191227-capacitor-blown-in-circuit.jpg')} width="800">A broken 1uF (marking 105K) 250VAC metallized film capacitor (red bulge with cracks in it) I found inside my mum's paper shredder.</Image>
...
Door knob (or barrel) capacitors are a form of ceramic capacitor named after their look-alike appearance to a door knob. They are usually rated for high voltages (kV's), and used in RF applications. They have a low dielectric loss and linear temperature co-efficient of capacitance. They are typically used in the frequency range from 50kHz-100MHz. The image below shows some examples of high-voltage door knob capacitors:

<Image src={import('./_assets/door-knob-capacitors.jpg')} width="600">Ceramic, high-voltage 'door-knob' capacitors. Image from www.trademe.co.nz.</Image>

## Relative Permittivity Of Common Materials

_Dielectrics_ are insulators or very poorly conducting materials but efficient supporters of electric fields[^britannica-dielectric]. When dielectrics are placed in an electric field, practically no current flows in them because they do not allow electrons to move (unlike metals). However, they become _polarized_ due to positive and negative charges within the material being displaced by tiny amounts. The polarization creates an internal electric field which reduces the overall electric field in the dielectric (the sum of the external field and its internal field)[^wikipedia-dielectric]. In terms of capacitors, a dielectric placed between the two plates of a capacitor increases the capacitance compared to that in a vacuum. 

The amount of polarization that any material allows is called its permittivity and is normally characterized by a number called its _relative permittivity_. The relative permittivity is the ratio of the permittivity of the material compared to that of a vacuum[^tutorials-point-difference-between-dielectric-constant-and-permittivity]. It is also called the _dielectric constant_, although this term is considered out-dated and deprecated by standards organizations in engineering[^wikipedia-relative-permittivity]. The below table shows the dielectric constant of common materials, sorted by alphabetic order.
...
That is, in parallel, *the total equivalent capacitance is the sum of the individual capacitances*. This is shown in the image below.

<Image src={import('./_assets/capacitors-in-parallel.webp')} width="600">Diagram showing the resulting capacitance from two capacitors in parallel.</Image>

*Connecting capacitors in parallel increases the capacitance.* Parallel-connected capacitors occur everywhere in circuit design. A classic example is bulk decoupling for a switch-mode power supply, which will typically have more than one large capacitor connected in parallel on the input.
...
Use the calculator below to find the total capacitance of two or more capacitors in parallel. Each input accepts metric prefixes (e.g. <code>10u</code>, <code>100n</code>, <code>4.7p</code>).

<ParallelCapacitance client:load />
...
<Image src={import('./_assets/capacitors-in-series.webp')} width="800">Diagram showing the equivalent capacitance from two capacitors connected in series.</Image>
...
$V$ = voltage (Volts)<br/></p>

### Capacitor Charge Calculator

Use the calculator below to solve $Q = CV$ for any one of charge, capacitance or voltage given the other two. Inputs accept metric prefixes (e.g. `5u`, `4.7n`, `100`).

<CapacitorCharge client:load />
...
$C$ = capacitance (Farads)<br/>
$V$ = voltage across the capacitor (Volts)<br/></p>

As shown by the equation, the energy stored in a capacitor is related to both the capacitance and voltage of the capacitor. A typical 100nF, 6.5V capacitor can store 2.11uJ. Not much huh! If you are really considering capacitors for their energy storage capabilities, you must look at supercapacitors, which have typical values of 100F and 2.5V (as of 2011). This gives 313J of energy, which is a useful amount for powering something.

### Capacitor Energy Calculator

Use the calculator below to solve $E = \frac{1}{2}CV^2$ for any one of energy, capacitance or voltage given the other two. Inputs accept metric prefixes (e.g. `2.11u`, `100n`, `6.5`).

<CapacitorEnergy client:load />
...
$d$ = separation distance between the two plates, in meters<br/></p>

## Capacitance of Shapes

Different shaped conductors have different capacitances. For basic shapes there a simple well-known formulas for calculating their capacitance, including shapes like parallel plates (the traditional capacitor), discs and spheres.
...
<p className="centered">where:<br/>
$\epsilon_0$ is the electric constant ($8.854 \times 10^{-12}Fm^{-1}$)<br/>
...
$d$ is the distance between the plates $[m]$<br/></p>

### Single Disc Capacitance

<Image src={import('./_assets/diagram-for-disc-capacitance-equation.svg')} width="320">Diagram for the disc-to-infinity capacitance equation.</Image>

The capacitance of a single thin plate as shown in the above image, with a ground at 'infinity' (or more practically, just very far away) is:
...
$d$ = diameter of the thin plate (meters)<br/></p>

### Sphere Capacitance

<Image src={import('./_assets/diagram-for-sphere-capacitance-equation.svg')} width="320">Diagram for the sphere-to-infinity capacitance equation.</Image>

The capacitance of a single sphere as shown in the image above, again with a ground at infinity is[^bib-qs-study-capacitance-spherical]:
...
$r$ is the radius of sphere [$m$]<br/></p>

### Concentric Cylinder Capacitance

<Image src={import('./_assets/diagram-for-coaxial-cylinders-capacitance-equation.png')} width="320">Diagram for the coaxial cylinder capacitance equation. Image from http://www.capsense.com/capsense-wp.pdf.</Image>

The capacitance of two concentric cylinders as shown in the image above is:

$$
\begin{align}
C = \frac{2 \pi \epsilon_0 \epsilon_r}{\ln (\frac{b}{a})} L
\end{align}
$$
...
and all other variables as previously mentioned<br/></p>
...
Since the ESR is proportional to the capacitor's plate area, for similar capacitor designs, the ESR decreases with increasing capacitance.

## Reactance and Impedance

The opposition a capacitor presents to an alternating current is called its _reactance_, $X_C$. Unlike a resistor, this opposition depends on frequency:

$$
X_C = \frac{1}{2\pi f C}
$$

where $f$ is the frequency of the signal and $C$ is the capacitance. Because $f$ is in the denominator, the reactance is large at low frequencies (a capacitor blocks DC, where $f = 0$ gives infinite reactance) and small at high frequencies (it readily passes high-frequency AC). This frequency dependence is what makes capacitors useful for [decoupling](#decoupling) and [mains line filters](#mains-line-filters) -- they present a low impedance to high-frequency noise, shunting it to ground.

For example, a $1\unit{uF}$ capacitor has a reactance of $X_C = \frac{1}{2\pi \cdot 100\unit{kHz} \cdot 1\unit{uF}} = 1.6\unit{\Omega}$ at $100\unit{kHz}$, but $159\unit{\Omega}$ at $1\unit{kHz}$.

### ESR and ESL

The formula above is for an _ideal_ capacitor. A real capacitor also has parasitic elements. The first two that are usually considered are the [equivalent series resistance](#equivalent-series-resistance-esr) ($\mathrm{ESR}$) and _equivalent series inductance_ ($\mathrm{ESL}$) -- the latter arising from the capacitor's leads, electrodes and internal geometry. They are modelled as elements in series with the capacitor as shown below:

<Image src={import('./_assets/esr-esl-capacitor-schematic.webp')} width="500px">Schematic showing the capacitor model with added ESR and ESL (both in series, as their names suggest).</Image>

<Aside type="tip">
Sometimes ESR is labelled $R_S$ and ESL $L_S$ ($S$ for _series_).
</Aside>

The impedance under this model is:

$$
Z = \mathrm{ESR} + j\left(2\pi f \cdot \mathrm{ESL} - \frac{1}{2\pi f C}\right)
$$

and its magnitude:[^rohm-bypass-capacitor-impedance]

$$
|Z| = \sqrt{\mathrm{ESR}^2 + \left(2\pi f \cdot \mathrm{ESL} - \frac{1}{2\pi f C}\right)^2}
$$

This gives the characteristic "V-shaped" impedance curve as shown below:

<Image src={import('./_assets/capacitor-impedance-vs-frequency.png')} width="800px">The impedance magnitude of a real 1µF ceramic capacitor (ESR = 20 mΩ, ESL = 0.8 nH) vs. frequency. The capacitive region falls at -20 dB/decade, bottoms out at the ESR at the self-resonant frequency (SRF), then rises in the inductive region. The ideal reactance $X_C$ is shown dashed.</Image>

There are three regions:

* **Capacitive region (low frequency):** The $\frac{1}{2\pi f C}$ term dominates and the impedance falls with frequency at $-20\unit{dB}$/decade, just like the ideal reactance.
* **Self-resonant frequency (SRF):** At one particular frequency the inductive and capacitive reactances cancel ($2\pi f \cdot \mathrm{ESL} = \frac{1}{2\pi f C}$), leaving only the $\mathrm{ESR}$. This is the point of minimum impedance, given by:

$$
f_{\mathrm{SRF}} = \frac{1}{2\pi \sqrt{\mathrm{ESL} \cdot C}}
$$

* **Inductive region (high frequency):** Above the SRF the $2\pi f \cdot \mathrm{ESL}$ term dominates and the impedance _rises_ with frequency -- the capacitor behaves like an inductor!

### Capacitor Impedance Calculator

Use the calculator below to compute a capacitor's reactance and impedance at a given frequency, and to see this V-shaped curve for your own values. Enter a capacitance and frequency, and optionally the parasitic ESR and ESL (leave them blank for an ideal capacitor):

<CapacitorImpedance client:load />

The practical takeaway is that a capacitor only behaves like a capacitor _below_ its self-resonant frequency. Smaller packages have lower $\mathrm{ESL}$ and therefore a higher SRF, which is why high-frequency decoupling uses small (e.g. 0402 or smaller) ceramic capacitors, and why several different capacitance values are often placed in parallel -- each covers a different part of the frequency spectrum.

### Paralleling Capacitors Of Different Values

A single capacitor only achieves a low impedance near its self-resonant frequency. A widely-used decoupling technique (you'll often see this next to the supply pins of high-frequency ICs) is to place several capacitors of different values in parallel -- a large one to cover the low frequencies and progressively smaller ones for the higher frequencies -- so that at any given frequency, *something* in the network is presenting a low impedance.

Capacitors in parallel combine like any other parallel impedances (their admittances add):

$$
Z_{parallel} = \frac{1}{\sum_i \frac{1}{Z_i}}
$$

The table below lists typical values for a range of X5R/X7R SMD MLCCs.

| Capacitance | Package | ESR | ESL | Self-resonant freq. |
|---|---|---|---|---|
| 10µF | 0805 | 2mΩ | 0.9nH | ≈1.7MHz |
| 1µF | 0603 | 8mΩ | 0.6nH | ≈6.5MHz |
| 100nF | 0402 | 30mΩ | 0.5nH | ≈22MHz |
| 10nF | 0402 | 80mΩ | 0.5nH | ≈71MHz |

Placing all four in parallel gives the combined impedance shown in the image below:

<Image src={import('./_assets/parallel-capacitor-impedance-vs-frequency.png')} width="800px">The impedance of a $10\unit{\mu F}$, $1\unit{\mu F}$, $100\unit{nF}$ and $10\unit{nF}$ MLCC in parallel (bold red), compared to each capacitor on its own (dashed). Each capacitor dominates near its own self-resonant frequency, and the combination keeps the impedance low (tens of mΩ) across a broad band from roughly $1\unit{MHz}$ to past $100\unit{MHz}$.</Image>

The combined curve broadly follows the _lower envelope_ of the individual curves -- the effective impedance stays low across a far wider frequency range than any single capacitor could manage on its own.

<Aside type="caution">
Paralleling different values isn't a free lunch. In the band between each pair of self-resonant frequencies, the larger capacitor is already inductive while the smaller one is still capacitive, forming a parallel LC tank. This creates **anti-resonance peaks** where the combined impedance rises back up and can exceed what a single capacitor would do (for example, the bump at about $4\unit{MHz}$ in the plot above goes higher than the $10\unit{\mu F}$ cap alone). This is the same phenomenon whereby a parallel LC tank rises to an (ideally) infinite impedance at resonance. For this reason, spreading values across many decades can actually be worse than using several capacitors of the _same_ value, whose self-resonant frequencies coincide and so lower the impedance without introducing anti-resonance[^rohm-bypass-capacitor-impedance].
</Aside>

## Leakage Currents

Leakage currents are present in all types of capacitor. Leakage current is the sum of electrical losses from energy required to build up the oxide layers, weaknesses in the dielectric, tunnel effects, and cross currents. They typically increase proportionally to the capacitance of the capacitor. We can reduce the leakage current down to two main factors, the absorption current $I_{abs}$, and the intrinsic leakage current $I_{il}$.
...
Capacitors that have had a relatively constant voltage across them for a decent amount of time typically exhibit far less absorption current than one which has not been charged in the short-term past. This is due to a phenomenon called ‘self-healing’, whereby a charged capacitor will heal defects in the electrolyte. Uncharged electrolytic capacitors may have weakened electrolyte due to ‘dissolution’, the destruction of the dielectric when no charge is present.

The leakage current through a capacitor can be modelled with a resistor in parallel with the actual capacitance, shown below:

<Image src={import('./_assets/capacitor-with-parasitic-series-resistance-leakage-current.png')} width="300px">A capacitor showing the parasitic parallel resistance present in all real capacitors, which creates a leakage current.</Image>
...
$ C $ = the capacitance of the capacitor, and again, choice of units is up to the manufacturer<br/></p>
...
$ R_{insulation} $ = the insulation resistance as specified on the capacitors datasheet<br/></p>
...
Leakage current becomes an important parameter to consider when designing long-life battery powered circuits. This is especially true for circuits powered off primary batteries with high internal resistance, such as lithium thionyl chloride batteries (LiSOCl2), because large (>100uF) capacitors can be required to help provide energy during high pulse current situations. These capacitors can have significant leakage current.

### Measuring The Leakage Current Of A Capacitor

Because of the small currents/total energy involved, you can't really measure the leakage current of a capacitor with a standard multimeter. One way is to use a dedicated high-resistance meter, commonly called a megaohm meter or insulation resistance tester.

## DC Bias Characteristic

The _DC Bias Characteristic_ is a measure of how a capacitor's **capacitance changes with applied DC voltage**. Some types of capacitors (such as [MLCC capacitors](#ceramic)) have a capacitance which significantly reduces as its DC voltage increases.

The good news is this can be manipulated to make things such as voltage-controlled oscillators (VCOs), where the capacitance is part of a resonant circuit, and the resonant frequency is changed by modifying the voltage on the capacitor, hence changing the capacitance. [Diodes](/electronics/components/diodes) also offer this feature and can be used to make FM radio signals by modulating a high-frequency waveform.

The bad news is that this also adversely affects the capacitance in situations where you want it to stay constant. The **drop in capacitance can upset op-amp gains, frequency cut-off points of filters, the time constant of RC oscillators, and the noise removal of decoupling caps**. This can actually be a very significant problem, especially with small [package](/pcb-design/component-packages/) size ceramic capacitors (such as 0603 and 0805 SMD chip capacitors). An excellent explanation of this effect is Maxim Integrated's [Temperature and Voltage Variation of Ceramic Capacitors, or Why Your 4.7uF Capacitor Becomes a 0.33uF Capacitor](http://www.maximintegrated.com/app-notes/index.mvp/id/5527). The image below is from Maxim's page, and just serves as an example to show by how much the capacitance can vary in normal operation conditions!

<Image src={import('./_assets/graph-of-temperature-variation-of-ceramic-chip-4-7uf-capacitors.png')} width="800">Graph of the capacitance variation (w.r.t. voltage) of a select group of 4.7uF ceramic chip capacitors, Image from http://www.maximintegrated.com/app-notes/index.mvp/id/5527.</Image>
...
It's important to note that the **DC bias problem gets worse with smaller ceramic chip capacitor packages**. The image below shows the difference in capacitance change for 4 caps which are almost identical except for the package they are in.

<Image src={import('./_assets/dc-bias-capacitance-derating-graph-from-murata.webp')} width="600">Graph showing how the package size changes the DC bias behaviour. All 4 capacitors are from the same manufacturer, and have the same ratings (capacitance, voltage, dielectric) except for package size. Graph was generated by Murata's SimSurfing tool[^murata-sim-surfing-tool-mlcc].</Image>
...
Capacitors are commonly used for decoupling, as shown in the image below.

<Image src={import('./_assets/decoupling-caps-schematic-example-on-r-pi-pcb.png')} width="400">Example usage of decoupling capacitors for ICs. Schematic is from the Raspberry-Pi PCB. Image from http://www.raspberrypi.org/wp-content/uploads/2012/04/Raspberry-Pi-Schematics-R1.0.pdf.</Image>
...
They are also given a number to represent their impulse test rating, as shown in the table below.
...
The image below shows a charge pump circuit used to generate -5V from a +5V PWM signal.

<Image src={import('./_assets/charge-pump-based-negative-voltage-gen-schematic.png')} width="800">This schematic shows a charge pump circuit used to generate -5V from a +5V PWM signal.</Image>

For more information, see the [Electronics->Components->Power Regulators->Charge Pumps page](/electronics/components/power-regulators/charge-pumps/).

## Feedthrough Capacitors

Feedthrough (or feedthru) capacitors are special three-terminal capacitors (sometimes with four connections) used for **suppression** of **RF noise**. They are also known under the more general name of an _EMI suppression filter_ or _three-terminal capacitor_. The image below is a render of a 3-terminal 0603 feedthrough capacitor:

<Image src={import('./_assets/feedthrough-capacitor-3d-render-0603-1608-3-connections.jpg')} width="350">A 3D render of an 0603-sized feedthrough capacitor with 3 connections. Image from www.digikey.com.</Image>
...
<Image src={import('./_assets/feedthrough-capacitor-schematic-symbol-3-connection.png')} width="400">The schematic symbol for a 3-terminal feedthrough capacitor.</Image>
...
* Providing low-pass filtering into heavy loads, where just a standard RC filter would suffer from too much voltage drop (or power dissipation) across the resistor.
...
The image below shows a simple capacitance multiplier made from 1 resistor, 1 capacitor and 1 NPN BJT transistor:

<Image src={import('./_assets/capacitance-multiplier-simple.svg')} width="500">A simple capacitance multiplier consisting of a single resistor, capacitor and NPN BJT transistor. The effective capacitance is approximately the capacitance of C1 multiplied by the current gain B of the transistor.</Image>

The BJT is configured as an emitter-follower (common collector). The output voltage will always be $0.7V$ less than the voltage across the capacitor. When the load draws current at $V_{OUT}$, rather than all of that current loading the RC filter, only the proportion $\frac{1}{\beta + 1}$ gets drawn through the base from the RC filter, the rest of it gets delivered directly from $V_{IN}$ via the collector. The effective capacitance seen by the circuit is the capacitance of C1 multiplied by the current gain $\beta + 1$ of the transistor:
...
<Image src={import('./_assets/capacitance-multiplier-resistor-divider.svg')} width="500">A capacitance multiplier which has improved filtering performance compared to the single resistor version.</Image>
...
The cut-off frequency for the 2 resistor capacitance multiplier is the same equation but with the equivalent resistance being $R1$ and $R2$ in parallel:
...
The "effective capacitance" is exactly the same as in the single-resistor version (but remember, you don't use this for the cutoff frequency calculations).

<Aside type="example">

Let's go through the design process for a capacitance multiplier circuit, and then simulate it using KiCad/ngspice to see what its frequency response is.
...
Given the noise fluctuations can be up to $1V_{p2p}$, we want the *BJT transistor to be dropping more than $1V_{p2p}$ so that the input voltage is always larger than our output voltage*, even with the ripple present on the input. Let's aim for nominal $10V$ output at no load.
...
We'll use the time-honoured 2N2222 NPN transistor, just because, well, I had a simulation model for it (power consumption shouldn't be too high that we'd need something beefier, but more on that below). The forward current gain ($\beta$) of the 2N2222 is simulated at $200$.
...
Then using the equation that constrained the ratio of $R1$ and $R2$, that means $R2$ must be:
...
Now we have found $R1$ and $R2$, $C1$ is determined for us by the cut-off equation:
...
<Image src={import('./_assets/capacitance-multiplier-sim/schematic.png')} width="800">Simulation-ready schematic of the capacitance multiplier we are designing.</Image>

Running AC analysis using ngspice gives us the frequency response (bode plot) shown in the image below.

<Image src={import('./_assets/capacitance-multiplier-sim/out.png')} width="800">The simulated frequency response of the capacitance multiplier circuit above.</Image>
...
Our cut-off frequency should then be $-3dB$ on-top of that, i.e. at $-1.75dB - 3dB = -4.75dB$. Plotting across and then down does indeed give us a cut-off frequency of approx. $10Hz$. The roll-off in the stop band is the same as a standard RC low-pass filter at $-20dB/decade$.

<Aside type="caution">
Watch out for the transistors power dissipation! In our example, $100mA$ was going through the 2N2222 which was dropping $2.1V$. This gives $210mW$, which should be o.k. for the 2N2222 (when in the old style TO-18 can package). But capacitance multipliers are typically used in high-current situations (many amps and more), in where careful attention has to be given to the power dissipation in the transistor. Large transistors and/or heat-sinking may be required. The voltage drop can also be reduced, but this also leads to a poorer filtering capability (i.e. less headroom).
</Aside>

Capacitance multipliers don't have to be built from BJT transistors, they can also use MOSFETs or op-amps as their active component. BJTs are used in a majority of cases though because of their cost, power dissipation capability, and simplicity.
...
<Image src={import('./_assets/circuit-schematic-showing-how-to-repair-an-electrolytic-capacitor.png')} width="300">Schematic showing how to repair/reform an old electrolytic capacitor. The voltage on the capacitor should slowly rise to the rated voltage.</Image>

This can be done with a power supply set to the rated voltage and a high-power (e.g. 5W) 30kΩ resistor in series with the capacitor, as shown in the image above. Connect the circuit and measure the voltage across the capacitor. It should start at 0V and increase as an inverse exponential as the current through the capacitor decreases. Wait until the voltage across the capacitor gets above 90% of the rated voltage before disconnecting the circuit, as shown in the image below. If the voltage stabilises below 90% of the full-rated voltage, the capacitor is stuffed and can be thrown out.

<Image src={import('./_assets/capacitor-voltage-graph-while-charging-through-resistor-for-reforming.png')} width="400">Graph of an electrolytic capacitor charging through a resistor to reform the aluminium oxide. The graph part of the image from http://www.antonine-education.co.uk/Pages/Electronics_2/Timing_Subsystems/RC_Networks/further_page_2.htm.</Image>

## Energy Loss While Charging A Capacitor

An interesting phenomenon occurs when charging a capacitor from a fixed voltage source (e.g. battery or power supply). Assuming a real-world situation, there is always going to be some resistance between the voltage source and capacitor. **Exactly the same amount of energy** is dissipated through this resistance as is stored in the capacitor when charging it up to the voltage source voltage $V_{cc}$. It **doesn't matter how large or small** this resistance is! The resistance could just be the ESR of the capacitor, or it could be a dedicated resistor placed in series to limit the inrush current.

The image below shows an example schematic.

<Image src={import('./_assets/energy-loss-while-charging-a-capacitor-schematic.webp')} width="600">A schematic showing the basic circuit while charging a capacitor, highlighting the energy lost in the series resistance.</Image>

This quite significant and fixed energy loss has implications when it comes to charging caps in low-power circuits (e.g. running off a battery), and sizing resistors (including NTC thermistors) for limiting the inrush current to capacitors, normally as part of the front end to a power supply or motor driver.
...
Now using the equation for charge in its integral form $Q = \int{I \cdot dt}$ we can write:
...
[^rohm-bypass-capacitor-impedance]: ROHM Co., Ltd. (2020, Sep). _Impedance Characteristics of Bypass Capacitor_ [application note]. Retrieved 2026-06-05, from https://fscdn.rohm.com/en/products/databook/applinote/common/bypass_capacitor_impedance_characteristics_an-e.pdf.
