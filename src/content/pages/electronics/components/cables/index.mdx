...
import WireGauge from 'src/components/calculators/wire-gauge/WireGauge.jsx';
...
$A$ is the cross-sectional area of the conductor, in $m^2$<br/>
$\rho$ is the resistivity of the conductor (e.g. $1.68 \times 10^{-8}\ \Omega \cdot m$ for copper), in $\Omega \cdot m$<br/>
$L$ is the length of the cable, in $m$<br/>
$I$ is the current through the cable, in $A$<br/>
$V_{drop}$ is the acceptable voltage drop across the full length, in $V$<br/></p>

Converting the area to a diameter ($d = \sqrt{4A/\pi}$) and then to an AWG gauge using $n = 36 - 39\frac{\log(d/0.127\text{mm})}{\log 92}$ gives the wire gauge to specify. Round down (smaller AWG number = thicker wire) for a safety margin.
...
## Protecting Lose Ends

Sometimes you might have to cut a cable and leave some of the individual wires unconnected. One risk with this is that the exposed metal right on the end of the wire can touch other wires and/or neighbouring conductive materials (circuitry, enclosures, e.t.c) and cause problems.

One way to fix this is to add small pieces of heat-shrink to the ends of the wires, as shown in <IRef iref="fig-wires-with-exposed-ends-vs-heatshrink-to-prevent-shorts-photo" />. The heat-shrink is slipped over so that half of it is on the end of the wire, and then heated. You have to get tight fitting heat-shrink so that it grips the wire well and doesn't slip off during use.

<Image iref="fig-wires-with-exposed-ends-vs-heatshrink-to-prevent-shorts-photo" src={import('./_assets/wires-with-exposed-ends-vs-heatshrink-to-prevent-shorts-photo.webp')} width="800px">Wires with exposed ends vs those with added heat-shrink to prevent shorts.</Image>
