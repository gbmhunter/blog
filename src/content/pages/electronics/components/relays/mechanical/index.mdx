...
There are a number of different configurations relay contacts (the output pins) of a relay can have.

Normally open and normally closed are terms that describe the state specific pairs of relay contacts when the relay is not energized. 
...
The National Association of Relay Manufacturers (now called the Relay and Switch Industry Association) define 23 electrical contact "forms". These apply to relays, but are used to describe switches. Below are the most common ones that apply to relays:[^wikipedia-electrical-contact]

* **Form A:** Contacts are NO and become closed when the relay is energised. Also known as SPST-NO.
* **Form B:** Contacts are NC and become open when the relay is energised. Also known as SPST-NC.
* **Form C:** There are three output pins, with a common to one NO and one NC contact. When the relay is energised, the NO contact will be connected to the common and the NC contact will be disconnected from the common. It is performed in a break-before-make sequence (see Form D for the opposite). These are also called NO-C-NC or SPDT relays.[^wikipedia-electrical-contact]. Can be used as a Form A or Form B relay by ignoring one of the outputs.
* **Form D:** Similar to Form C, but with a make-before-break sequence rather than a break-before-make. This is an uncommon form.
* **Form X:** Is equivalent to two Form A relays in series, which are mechanically linked and actuated at the same time. These are commonly found in contactors and toggle switches designed to handle high power inductive loads[^wikipedia-electrical-contact].
