...
authors: [gbmhunter]
date: 2012-10-25
description: Mechanical relays are electronic components which can switch a load on and off from an isolated control signal using an electromagnet coil and mechanical actuator. This page covers terminology, parameters, contact arrangements, inductive kickback, flyback diodes, latching, common relay packages, supplier links and more.
image: ./_assets/cover-image.webp
lastUpdated: 2026-06-18
tags: [relay, relays, solid state, SSRs, single-pole, double-pole, single-throw, double-throw, inductive kickback, wet contacts, dry contacts, armature, reed]
...
- Turning on mains voltage devices from 5-12V circuits, while providing isolation
- Switching large currents
- Providing isolation between two circuits (although [opto-couplers](/electronics/components/optocouplers/) are normally used for this)
...
- The Contacts: The output pins which get connected or disconnected when the relay turns on
...
The table below summarises the acronyms used.
...
The diagram below shows the different relay contact forms.

<Image src={import('./_assets/relay-contact-forms.svg')} width="800px">A diagram showing the different relay contact forms.</Image>
...
The coil of a relay is basically an [inductor](/electronics/components/inductors/). Due to the rapid $\frac{di}{dt}$ when the relay is switched off, the inductive coil will generate a large negative voltage to try and keep the current flowing (remember: inductors "resist" the change in current). This is called _inductive kickback_. This voltage spike can cause havoc in neighbouring components (e.g. killing the transistor used to switch the relay on, or arcing across switch terminals) if not suppressed. A [_flyback_ diode](/electronics/components/diodes/general-purpose-diodes/) in anti-parallel across the coil of the relay will clamp the voltage spike to approx. no more than $-0.7\unit{V}$.

<Image src={import('./_assets/flyback-diode-relay-schematic.svg')} width="500px">Always add a \"flyback\" diode in anti-parallel across the coil of a relay to quench voltage spikes due to the rapid di/dt through the inductor when the relay is switched off. </Image>
...
Relays are rated for switching specific voltages and currents through their contacts (the output side of the relay).

You might be wondering why a relay might be rated for a higher current with AC than at DC. Switching AC signals are gentler than DC signals on relays. This is because the voltage and current drops to zero twice per cycle in an AC signal. When you break an AC connection, the arc that forms is typically extinguished at the zero-crossing point. An arc in DC is harder to extinguish as the current flow is constant. The image below shows an example of this with the CIT J107F family of relays.

<Image src={import('./_assets/cit-j107f-relay-ul-and-tuv-contact-ratings-with-ac-dc-differences-highlighted.webp')} width="700px">The UL and TUV ratings for the family of CIT J107F relays. The differences in contact ratings between AC and DC are highlighted.</Image>

The image below shows another example, this time it's from the datasheet of the Omron G5LE-1 DC24 general purpose relay.

<Image src={import('./_assets/omron-g5le-1-dc24-relay-switching-capacity-and-durability-graphs.webp')} width="800px">A screenshot from the datasheet of the Omron G5LE-1 DC24 relay showing the difference between the AC and DC ratings on the switching capacity and durability graphs.[^omron-g5le-relay-ds]</Image>

There can also be a difference in current rating for a relay which has normally-open and normally-closed contacts. This is due to the contact pressure difference when the contacts are closed. For a normally-open contact, the contacts are held closed by the magnetic force of the relay coil. For a normally-closed contact, the contacts are only held closed by the spring. The force from the coil is much stronger than the spring, and contact pressure directly impacts the contact resistance. The contacts held closed by the coil have a lower contact resistance, and can therefore handle more current. The image below shows an example of this with the Omron G5LE relay family.[^durakool-contacts-resistance-vs-contact-pressure]

<Image src={import('./_assets/omron-g5le-relay-no-vs-nc-current-difference.webp')} width="400px">A screenshot from the datasheet of the Omron G5LE relay family showing the difference in current rating between the normally-open (NO) and normally-closed (NC) contacts.[^omron-g5le-relay-ds]</Image>
...
- **RT IV**: Sealed relay. Relay provided with a case that has no venting to the outside, with a time constant better than $2 \times 10^4\unit{s}$ in accordance with IEC 60068-2-17.
- **RT V**: Hermetically sealed relay. Sealed relay having an enhanced level of sealing, with a time constant better than $2 \times 10^6\unit{s}$ in accordance with IEC 60068-2-17.

<Image src={import('./_assets/te-t9g-series-relay-environmental-protection-ratings-rtii-and-rtiii.webp')} width="800px">A screenshot from the datasheet of the TE T9G series relay showing its RT II and RT III environmental protection ratings.[^te-potter-and-brumfield-t9g-series]</Image>

## Common Relay Packages

Most PCB mount relays are through-hole components. My guess is that they have not gone SMD like many other components because they are quite bulky and heavy -- using SMD pads would probably pose a risk of the pads being ripped off the PCB. Through-hole leads gives much better mechanical strength.
...
Armature relays are the most common type of mechanical relay. They have a magnetized armature which is moved by the magnetic field of the coil (electromagnet). The contacts are connected to the armature and make or break the output circuit. Armature relays are more popular than reed relays, primarily due to the higher current carrying capabilities. They are also more resistant to external magnetic fields. However, they are slower to switch and cannot handle very high voltages like reed relays.

### Reed Relays

Reed relays are a type of relay that uses an electromagnet to move a reed switch. The contacts in the reed switch are magnetic and the magnetic field directly moves the contacts. This is what distinguishes them from armature relays, which require an armature to move non-magnetic contacts.[^wikipedia-reed-relay]

<Image src={import('./_assets/different-reed-relay-form-factors.webp')} width="600px">A diagram of various reed relays available to purchase. a) Coto Technology 9091-05-01, b) Coto Technology 9001-05-11, c) Standex-Meder Electronics HE24-1A83-02, d) Celduc Inc. D31A3100, e) Standex-Meder Electronics HI12-1A85.</Image>

Because they do not have an armature, they can switch much faster because the contacts are very light weight. They also typically require less electrical power to operate. Many reed relays seal the reed switch in a thin and long glass tube to protect the contacts from corrosion.[^wikipedia-reed-relay]

Reed relays typically cannot handle as high a current as armature relays (most are rated for 100 mA to 3 A), but can typically handle higher voltages (e.g. 1000-10,000 V) due to the glass sealed environment the contacts operate in.

<Image src={import('./_assets/photo-of-2x-meder-mre12-18a5-reed-relays-on-pcb.webp')} width="900px">A photo of two Meder MRE12-18A5 reed relays on a PCB. These relays are quite "beautiful" in the sense that they are not encased in a square enclosure and you can see the insides! This PCB was inside a Fluke 5322A "Electrical Safety Tester Calibrator" (test equipment).</Image>

<Aside type="note">
Reed relays are very similar to reed switches. A reed switch is just a component which contains the magnetic contacts. This needs an external magnetic field to switch. A reed relay adds in a electromagnet coil to drive the switch which forms the relay.
</Aside>

DigiKey's reed relays category can be found [here](https://www.digikey.com/en/products/filter/reed-relays/964).

## Product Links

The G5Q/G5LE is a popular range of through-hole PCB mount relays from Omron. The G5Q series can go up to 250VAC at 10A.
...
[^wikipedia-reed-relay]: Wikipedia (2025, Jul 20). _Reed relay_. Retrieved 2025-10-06, from https://en.wikipedia.org/wiki/Reed_relay.
