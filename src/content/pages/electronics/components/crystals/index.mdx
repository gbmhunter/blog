...
**_Crystals_ are piezoelectric components which can be used to build an oscillator**. Combined with driving circuitry, they form an oscillator which can be made to output a periodic waveform to be used as a clock source for digital logic (e.g. flip-flops, microcontrollers, FPGAs, e.t.c.). They can also be called _piezoelectric resonators_.
...
<Image src={import('./_assets/tuning-fork-32.678khz-crystal-shape.svg')} width="700px">Basic shape of a 32.678kHz crystal.</Image>
...
<Image src={import('./_assets/32768khz-crystal-drift.png')} width="500px">The common shape of a temperature vs. drift curve for a 32.768kHz crystal, highlighting the turnover point at the maxima. In this model, $T_O=25^{\circ}C,\ \alpha = -0.034ppm^{\circ}C^2$.</Image>
...
Some OCXOs contain double ovens for even better stability. One example is the Taitien NI-10M-3560 shown in <IRef iref="fig-taitien-ni-10m-3560-10mhz-double-oven-ocxo"/>. It is a 10MHz OCXO with a frequency stability of $\pm 0.1ppb$, and an aging performance of $\pm 0.05ppb/day$ or $\pm 10ppb/year$[^digikey-taitien-ni-10m-3560-product-page]
