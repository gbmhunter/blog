...
An op-amp can be combined with a regular diode to make an "ideal diode" (also called a _superdiode_[^superdiode], _precision rectifier_, or _half/full-wave rectifier_), one that rectifies and has *no voltage drop*. Ideal diode circuits are useful when:
...
The maximum input offset voltage you can compensate for with this circuit is given by Eq $\ref{eq:trim-inverting-offset}$[^analog-devices-input-offset-voltage].
...
V_{OUT} &= V_Z + V_{R2} \\
        &= V_Z + I_{R2} \cdot R \\
        &= V + \frac{V}{Z} \cdot R &  &\text{Subs. in \ref{vzeqv} and \ref{i-r2}}
\end{align}
$$
