...
authors: [gbmhunter]
date: 2024-01-25
description: Real-Time Clock (RTC) ICs are chips which accurately track calendar time. They are generally designed for low-power operation so that they can run off a small battery such as a coin cell. They are normally included on circuits and connected to a microcontroller when the application requires the microcontroller to keep track of time.
image: ./_assets/cover-image.webp
lastUpdated: 2026-06-18
tags: [electronics, electronic components, real time clocks, RTC, time keeping, calendar time, UNIX time, broken-down time, Zephyr]
title: Real Time Clock (RTC) ICs
type: page
---

_Real-Time Clock_ (RTC) ICs are chips which accurately track calendar time. They are generally designed for low-power operation so that they can run off a small battery such as a [coin cell](/electronics/components/batteries/coin-cells/). They are normally included on circuits and connected to a microcontroller when the application requires the microcontroller to keep track of time. They are better at keeping track of time than the MCUs internal oscillator due to:

1. Increased time keeping accuracy. Even when the MCU also has a 32.768kHz crystal, a dedicated RTC is normally more accurate due to the ability on many RTCs to tune/calibrate the frequency.
1. Very low power operation. When a RTC is powered off a small battery like a coin cell it will continue running for many years and will not lose track of time when the MCU is unpowered.

RTCs normally keep track of either UNIX time (or any arbitrary start point you choose) and/or calendar based time variables, which are also referred to as _broken-down time_.[^zephyr-peripherals-rtc] The broken-down time is normally represented by:

- Second of minute (a value in the range `[0, 59]`)
- Minute of hour (a value in the range `[0, 59]`)
- Hour of day (a value in the range `[0, 23]`)
- Day of month (a value in the range `[1, 31]`)
...
## Conversion Between UNIX Time and Calendar Time

The following C code shows how to convert to/from UNIX time and calendar time, which can be useful when dealing with RTCs in firmware:
...
The below image shows the block diagram for this IC. You can see the pins that connect to an external [32.768kHz oscillator](/electronics/components/crystals/#32768khz-crystals) in the bottom left corner.
...
[^zephyr-rtc-interface]: Zephyr. _RTC Interface_ [API documentation]. Retrieved 2024-09-20, from https://docs.zephyrproject.org/latest/doxygen/html/group__rtc__interface.html.
