...
You might notice that some OLED's specify a voltage range of 3.3-4.2V for the charge pump and 1.8-3.3V for the logic. And then you would think, well I really wanted to run them of the same supply (one of the main reasons why they would include an internal charge pump, right?), so can I run them from 3.3V? Well, although the charge pump input specifies a minimum of 3.3V, you can actually run it at that or lower, sacrificing a little screen brightness in the process (see the below graph). And the logic circuitry seems to handle 3.3V+- the variation of a normal linear regulator without too much trouble.
...
The [sparkfun/SparkFun_Qwiic_OLED_Arduino_Library repo on GitHun](https://github.com/sparkfun/SparkFun_Qwiic_OLED_Arduino_Library) contains Arduino based C++ code for driving the SparkFun Qwiic range of OLED screen dev boards. All of these boards use the Solomon SSD1306 IC. It provides common graphics functionality such as drawing pixels, lines, rectangles, filled rectangles, circles, bitmaps, fonts and raster operations.[^github-sparkfun-sparkfun-qwiic-oled-arduino-library]

It has a "dirty rect" feature that only updates parts of the screen that have change based of bounding rectangles. This increases the average update rate of the screen under normal use cases.[^github-sparkfun-sparkfun-qwiic-oled-arduino-library] It can be easily ported from Arduino to other frameworks by replacing the included I2C driver. I ported it to [Zephyr](/programming/operating-systems/zephyr/) and it took a few hours to get it working.
