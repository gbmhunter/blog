...
<Aside type="tip">
You can calculate the approximate forward voltage of a LED just based of it's wavelength using the equation:
...
This gives the energy in Joules. To convert to $eV$, divide by the elementary charge ($1.602 \times 10^{-19} C$). This means that $1eV = 1.602 \times 10^{-19} J$. So

$$
\begin{align}
E_{eV} = \frac{hc}{\lambda \times 1.602 \times 10^{-19}}
\end{align}
$$

We can combine the $h$ (Planck's constant, $6.626 \times 10^{-34} JHz^{-1}$), $c$ (speed of light, $3 \times 10^8 ms^{-1}$) and $1.602 \times 10^{-19}$ into a single constant, which is $1.241e^{-6}\ eV\ m$ (or $1241 eV\ nm$). So the equation simply becomes:

$$
\begin{align}
V_F = \frac{1241\,nm\,V}{\lambda_{nm}}
\end{align}
$$
...
<Image src={import('./_assets/led-connected-to-micro-for-light-detection-schematic.png')} width="500">Schematic showing how to connect an LED to a general microcontroller for light detection. The LED and resistor are connected to GPIO pins.</Image>
...
<Image src={import('./_assets/led-pulse-extending-circuit-schematic-annotated-rc-mosfet.png')} width="650">The schematic for a LED pulse width extending circuit. It converts a short pulse that would not be seen into a longer pulse which is visible.</Image>
...
<Image src={import('./_assets/zynq-7000-bootrom-led-error-code-waveform.webp')} width="800">The waveform of the LED error code output by the Zynq 7000 BootROM[^amd-zynq-7000-trm].</Image>
