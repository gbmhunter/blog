...
* Symbol: $I_F$
* Units (typical): $\mathrm{mA}$

This is the maximum forward current the LED can continuously be driven at. For small indicator LEDs, the maximum forward current is typically 20-30mA. Normally an indicator LED with a max. forward current of 20mA would be driven at less than the max., somewhere between 2-10mA. 
...
You can calculate the approximate forward voltage of a LED just based on the wavelength of light it emits! First you convert the wavelength to a frequency, and then calculate the energy of a photon (the energy of a photon is purely determined by the frequency, via the Planck-Einstein equation). Once you have the energy of a photon in Joules, you can convert this to $\mathrm{eV}$ by dividing by the elementary charge ($1.602 \times 10^{-19}\unit{C}$). This gives you the approximate forward voltage of the LED.
...
$c$ is the speed of light, $3 \times 10^8\unit{m\,s^{-1}}$<br/>
$f$ is the frequency, in $\mathrm{Hz}$<br/>
$\lambda$ is the wavelength, in $\mathrm{m}$<br/></p>
...
<p class="centered">where:<br/>
$E$ is the energy of the photon, in $\mathrm{J}$<br/>
$h$ is the Planck constant, $6.626 \times 10^{-34}\unit{J\,Hz^{-1}}$<br/>
$f$ is the frequency, in $\mathrm{Hz}$<br/></p>
...
This gives the energy in Joules. To convert to $\mathrm{eV}$, divide by the elementary charge $e$ ($1.602 \times 10^{-19}\unit{C}$). This means that $1\unit{eV} = 1.602 \times 10^{-19}\unit{J}$. This energy in $\mathrm{eV}$ happens to be similar to the required forward voltage (it's not a coincidence). So our equation becomes:
...
We can combine the $h$ (Planck's constant, $6.626 \times 10^{-34}\unit{J\,Hz^{-1}}$), $c$ (speed of light, $3 \times 10^8\unit{m\,s^{-1}}$) and $e$ (elementary charge, $1.602 \times 10^{-19}\unit{C}$) into a single constant, which is $1.241 \times 10^{-6}\unit{eV\,m}$ (or $1241\unit{eV\,nm}$). Let's round this to $1240\unit{eV\,nm}$ for convenience. So the equation simply becomes:

$$
\begin{align}
V_F = \frac{1240\unit{eV\,nm}}{\lambda_{[\mathrm{nm}]}}
\end{align}
$$
...
$$
\begin{align}
V_F &= \frac{1240\unit{eV\,nm}}{660\unit{nm}} \nonumber \\
    &= 1.88\unit{V}
\end{align}
$$

Red LEDs usually have a forward voltage of around $1.8-2.2\unit{V}$, so this agrees well!
...
Use the calculator below to work out the series resistor needed to drive an LED at a desired current. Pick a colour to use a typical forward voltage from the table above, or select <em>Custom</em> to enter your own $V_F$. The required resistance is calculated using Ohm's law, $R = (V_{dd} - V_F) / I_{LED}$, and the resistor power dissipation as $P_R = (V_{dd} - V_F) \cdot I_{LED}$. Inputs accept metric prefixes (e.g. `5`, `20m`, `3.3V`).

<LedResistor client:load />
...
The photocurrent of an LED is about 10-100 times smaller than that of a purpose-built photo-diode. The wavelength of peak sensitivity is usually a little less than the peak wavelength that it emits light at.
...
You can get RGB LEDs which already have the control and drive circuitry (e.g. the constant current source) for the LEDs inside them. These are normally connected to a microcontroller via a digital communication bus (e.g. [SPI](/electronics/communication-protocols/spi-communication-protocol)), or sometimes a custom protocol).

One popular example, the Worldsemi WS2811, uses its own custom communications protocol running at 800kHz.

<Image src={import('./_assets/ws2811-rgb-led-front-and-back-photo.png')} width="500">The WS2811, a popular RGB LED, with integrated controller and drive circuitry (constant current supply). Communicates via a custom 800kHz protocol to a microcontroller.</Image>

Overview of Worldsemi's RGB LED product offering:

* WS2811: LED driver IC, no built-in LEDs.
...
UVC light is especially effective at killing bacteria and viruses (germicidal effect). <IRef iref="fig-germicidal-effectiveness-of-uv-light-graph" /> shows the germicidal effectiveness of UV light vs. its wavelength.
...
<Image src={import('./_assets/laser-diode-with-integrated-fet-and-cap.png')} width="500">A laser diode with an integrated FET and capacitor for high-speed, high-power switching.</Image>
...
The Planck-Einstein equation gives a direct relationship between the frequency of a photon and its energy[^wp-planck-relation]:
...
Knowing this, you can convert an energy from $mW$ into a number of photons. One issue however is that the light from an LED is not emitted all at one precise frequency, it is usually spread across a small band of wavelengths known as its spectrum. One approximation is to just pretend all the light is emitted at _peak wavelength_. A more precise method is to take the spectrum, digitize it, and then calculate the proportion of photons for each small increment in wavelength.
...
$$
\begin{align}
PAR_{[mols/s/m2]} &= \frac{{Irr}_{[uW/cm2]}\lambda_{peak}}{hcN_A} \frac{1 \times 10^4}{1 \times 10^6} \nonumber \\
PAR_{[umols/s/m2]} &= \frac{{Irr}_{[uW/cm2]}\lambda_{peak}}{hcN_A} \frac{1 \times 10^4 \times 1 \times 10^6}{1 \times 10^6} \nonumber \\
PAR_{[umols/s/m2]} &= \frac{{Irr}_{[uW/cm2]}\lambda_{peak}}{hcN_A} \times 10^4 \nonumber \\
\end{align}
$$
...
$$
\begin{align}
PAR_{[umols/s/m2]} &= 83593 \cdot {Irr}_{[uW/cm2]}\lambda_{peak} \\
\end{align}
$$
...
$$
\begin{align}
PAR_{[umols/s/m2]} &= 8.36 \times 10^{-5} \cdot {Irr}_{[uW/cm2]} \cdot \lambda_{peak[nm]} \\
\end{align}
$$

<Aside type="caution">
Remember that PAR is defined as radiation between 400 and 700nm, so this equation is only applicable to wavelengths within this band (although if you drop the "photo-synthetic" bit, this conversion could be useful for other purposes). Also don't forget that just using the single peak wavelength of the light source in the calculation rather than looking at its entire spectrum could introduce significant errors. 
</Aside>
...
You can get LED's in a variety of SMD packages. Common SMD LED packages include the 0603 and 0402 chip packages.
...
However, some LED manufacturers to give lifetime and reliability information. This is typically done for higher power, specific purpose LEDs such as powerful illumination LEDs (think streetlights, car headlights) or agricultural growing lights. The lifetime is typically expressed as a curve of _power maintenance_ over time (in terms of hours the LED has been on for). Power maintenance is how powerful the LED is (in terms of light output power) compared to when it was new. Typical single-value lifetimes can be described as the number of hours until the LED light output power reduces to 70 or 50% of its initial output power (a 70 or 50% power maintenance).
...
Gallium aluminium arsenide (GaAlAs) LEDs became available in the 1980s, and offered significant brightness improvements over the GaP LEDs.[^marktech-optoelectronics-history-of-leds]
