...
<Image src={import('./_assets/four-rc-filter-sine-wave-generator-schematic.png')} width="900px">Schematic of a sine wave generator circuit using a square wave input and 4 RC filter stages.</Image>

If you can remember Fourier analysis, a square wave can be thought of as being made up of an infinite number of sine waves, starting at the fundamental frequency and then going up with all the odd numbered harmonics. The filtering works by **allowing the fundamental frequency through (it's in its pass band), whilst blocking all the higher harmonics**. The image below shows a $10\unit{kHz}$, $3.3\unit{V}$ square wave and its FFT. You can see the spike which represents the fundamental at $10\unit{kHz}$. The next spike is the 3rd harmonic at $30\unit{kHz}$.

<Image src={import('./_assets/sim-four-stage-rc-filter-fft-of-input-plot.png')} width="600px">The time domain voltage signal of a 10kHz 3.3V square wave (top) and the FFT of this signal (bottom).</Image>
...
Let's say we want a sine wave at $10\unit{kHz}$. We'll configure a timer or PWM peripheral on a microcontroller to output a square wave at $10\unit{kHz}$ with a $50\%$ duty cycle.

For each RC filter, we get to pick either the capacitance or resistance and then calculate the other. Let's pick a capacitance of $10\unit{nF}$.
...
$f_c$ is the cut off frequency ($-3\unit{dB}$ point) in Hertz [$\mathrm{Hz}$]<br/>
...
\begin{align*}
R &= \frac{1}{2\pi f_c C} \\
  &= \frac{1}{2\pi \cdot 10\unit{kHz} \cdot 10\unit{nF}} \\
  &= 1.592\unit{k\Omega} \\
\end{align*}
$$

<Aside type="caution">
Setting the cut-off frequency for each stage to 10kHz does not mean the entire filter will attenuate the input signal by only $-3\unit{dB}$. It will attenuate the signal by much more than that!
</Aside>

Remember, all the $R$ and $C$ values should be the same. This gives us the following schematic:

<Image src={import('./_assets/sim-four-stage-rc-filter-schematic.png')} width="700px">Schematic for our 10kHz sine wave generator.</Image>
...
<Image src={import('./_assets/sim-four-stage-rc-filter-output-plot.png')} width="700px">Simulation results for the four-stage RC filter based sine wave generator circuit.</Image>

It's also interesting to point out that the final output the sine wave has a DC offset of $1.65\unit{V}$. This is because the DC component (average voltage) of the input $3.3\unit{V}$ square wave at $50\%$ duty cycle is $3.3\unit{V} / 2 = 1.65\unit{V}$. If you want to remove this bias, you can connect an AC coupling capacitor to the output to center the sine wave around $0\unit{V}$.

The bode plots (frequency response) of this circuit are shown below. Each successive filter stage increases the slope of the attenuation. At the fundamental frequency (which is the frequency we want to pass) of $10\unit{kHz}$, the attenuation is $23\unit{dB}$ (14x reduction). At $30\unit{kHz}$ (which is the 3rd harmonic of the square wave, and we want to block this), the output is attenuated by $45\unit{dB}$ (178x reduction).

<Image src={import('./_assets/sim-four-stage-rc-filter-bode-plot.png')} width="700px">Bode plots for the four-stage RC filter based sine wave generator circuit.</Image>
...
**Is a four stage RC filter going to produce a perfect sine wave? No.** It just happens that four stages produces a sine wave which is "good enough" for most applications. We can do an FFT on the resulting output to see how "good" our sine wave is. A pure sine wave will have a single spike in the FFT at the fundamental frequency. Any distortion will show up as other spikes in the FFT. And while we are at it, let's do an FFT of the output from each intermediate stage, to see how the quality improves as it makes its way through the circuit.

The following plots show the waveform and its FFT at the input and at the output of each RC filter stage. The DC component of the waveform has been removed so as not to swamp the FFT plot (we usually don't care about the DC component anyway, we could easily remove this with an AC coupling capacitor).

<Image src={import('./_assets/sim-four-stage-rc-filter-thd.png')} width="700px">Plots showing the reduction in harmonic distortion through each successive RC filter stage.</Image>

One figure of merit we can use for each output is the _total harmonic distortion_ (THD). This is found by taking the square root of the sum of the squares of the RMS harmonic magnitudes in the FFT plots, and dividing it by the RMS voltage of the fundamental frequency. If we do this, we get the following graph:

<Image src={import('./_assets/sim-four-stage-rc-filter-thd-bar-plot.png')} width="600px">Plot showing the THD at each stage in the sine wave generator circuit.</Image>
...
Using cascaded RC filters is only one way of many to create sine waves! Some other popular methods include:
