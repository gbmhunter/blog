...
authors: [gbmhunter]
date: 2021-01-24
description: Small-signal analysis is a method of analysing circuits by linearising non-linear components and removing DC sources. This page explains the method and shows examples.
...
<Image src={import('./_assets/resistor-divider-01-before-replacing-vin.png')} width="500">Resistor divider, before replacing Vin.</Image>

We assume Vin is a perfect voltage source, therefore we can replace it with a short.

<Image src={import('./_assets/resistor-divider-02-vin-replaced-with-short.png')} width="500">Resistor divider, Vin replaced with short.</Image>

These two resistors are now in parallel, we can re-arrange the circuit as below to clearly show this:

<Image src={import('./_assets/resistor-divider-03-rearranged.png')} width="500">Circuit re-arranged to highlight both resistors in parallel.</Image>

The two resistors in parallel can be replaced with a single equivalent resistor (remembering $ R_{parallel
} = \frac{R_1 R_2}{R_1 + R_2} $):

<Image src={import('./_assets/resistor-divider-04-equivalent-output-resistance.png')} width="500">Equivalent resistance of the two resistors in parallel.</Image>
