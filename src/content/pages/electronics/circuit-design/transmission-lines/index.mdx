...
A _transmission line_ is a wire/cable/track on the PCB, that, because the frequency of the electrical signals going through has reached a certain point (typically RF), you can no longer just treat the wire as a short circuit. Instead, the connection must be treated as a _transmission line_, and must be _impedance matched_ all the way from the driver to the receiver. Impedance matching is done to maximise power transfer and minimise reflections from the load. The typical way to impedance match is to make sure the transmission line is _terminated_ correctly.

<Image src={import('./_assets/transmission-line-schematic-symbol.png')} width="500px">Schematic symbol for a transmission line, which represents a piece of coax cable. Because this usually isn't a component to appear on the BOM (if it's a PCB track), it's not normally given a designator.</Image>

_Termination_ is the process of adding components (usually resistors, but sometimes capacitors and inductors) to the ends of transmission lines (the start, the end, or both) to prevent things like reflections due to impedance mis-matches. See the [Termination](/electronics/circuit-design/transmission-lines/termination/) page for more info.

<Image src={import('./_assets/microstrip-track-example-in-isometric-view-microchip.gif')} width="300px">An isometric diagram of a microstrip. Image from www.maximintegrated.com.</Image>
...
**The characteristic impedance of a transmission line is usually called $Z_C$.** It is dependent on the per unit length inductance $L$ and capacitance $C$ of the transmission line. The equation is: 
...
**Wires, cables, PCB tracks etc. have to be treated as transmission lines when the signal passing through it changes rapidly compared to the time the signal takes to travel through the transmission line.**
...
$\lambda$ is the wavelength, in $\mathrm{m}$<br/>
$c$ is the speed of light, in which $3 \times 10^8\unit{m/s}$ is a good enough approximation<br/>
$f$ is the frequency of the signal, in $\mathrm{Hz}$<br/></p>

For example, electronicdesign.com states[^elec-design-imp-matching-p1]:
...
$$
\begin{align*}
\lambda &= \frac{3 \times 10^8\unit{m/s}}{780\unit{MHz}} \\
        &= 385\unit{mm} \\
\end{align*}
$$
...
$ \epsilon_r $ is the dielectric of the material the electro-magnetic wave is travelling through, and is unitless. In our case this will be the dielectric of the PCB. For standard FR4, this is about 4.35.<br/></p>

Thus, the minimum wavelength of the USB 2.0 signal is 1.65m, and the characteristic impedance of the track is only important if the total track length is greater than 165mm.

</Aside>

## Reflections

**Whenever the signal passes from one medium to another with different characteristic impedance, a reflection occurs at the boundary.** The reflection appears at the boundary and propagates in both directions (both forwards with the signal, and backwards towards the signal source). The magnitude (in terms of voltage) of the reflection is determined by the voltage reflection coefficient.

Voltage reflection coefficients are usually represented with the Greek uppercase "Gamma" ($\Gamma$), with subscripts representing the direction of signal as it crosses the boundaries between two "mediums" (cables, circuit traces, etc.) of different impedance. The voltage reflection coefficient is defined as the ratio of the reflected wave to the incident wave:
...
<p className="centered">where:<br/>
$ A $ is the voltage of the incident wave, in $\mathrm{V}$<br/>
$ B $ is the voltage of the reflection (going back to the source), in $\mathrm{V}$<br/></p>
...
<p className="centered">where:<br/>
$Z_1$ is the characteristic impedance of medium 1, in $\mathrm{\Omega}$<br/>
$Z_2$ is the characteristic impedance of medium 2, in $\mathrm{\Omega}$<br/></p>
...
As you can see, with $Z_2 = 0$ the $Z_1$ terms cancel and you are left with $\Gamma = -1$. Thus the reflected wave is equal in magnitude but 180° out of phase with the incident wave. This means they cancel each other out, giving $0\unit{V}$ at the receiver. This makes sense! There cannot be any voltage across a short (by definition)!
...
Co-planar waveguides (CPW) are a third alternative to [microstrips](/electronics/circuit-design/transmission-lines/microstrips/) and [striplines](/electronics/circuit-design/transmission-lines/striplines/) for controlling the impedance of a circuit board trace. There are two variants, the basic _co-planar waveguide_ and the _co-planar waveguide with ground_. The main advantage of a co-planar waveguide is that it **does not require a solid power plane** above or below it, making it a more flexible choice for layer and space constrained PCBs.

<Image src={import('./_assets/coplanaer-waveguide-for-gsm-antenna.png')} width="490px">A co-planar waveguide for a GSM antenna feed-line. The co-planar waveguide is the curved track that starts at E1. The three 0402 components are for a impedance matching circuit.</Image>
...
[^elec-design-imp-matching-p1]:  Lou Frenzel (2011, Oct 25). _Back to Basics: Impedance Matching (Part 1)_. ElectronicDesign. Retrieved 2022-02-01, from https://www.electronicdesign.com/technologies/communications/article/21796367/back-to-basics-impedance-matching-part-1.
