...
[555 timers](/electronics/components/timers-and-multivibrators/) can be configured to create a variable digital delay. To wait for a low-to-high transition on the input, delay for a while, and then present a fixed width high pulse on the output you have to use two 555 timers. You will want to configure both 555's in monostable mode.

Note one of the key behaviours with the above 555 circuit is that when the input transitions from low-to-high, the output only stays high for a fixed amount of time before returning to it's original state. You could view it more of a pulse generator than a delay circuit.
...
<Image iref="fig-bucket-brigade-delay-basic-internal-schematic" src={import('./_assets/bucket-brigade-delay-basic-internal-schematic.webp')} width="900px">The basic internal design of a bucket brigade IC[^electrosmash-bucket-brigade-devices-mn3007].</Image>
