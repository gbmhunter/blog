...
tags: [Altium, PCB, scripting, API]
title: PCB-Related API
type: page
---

This page covers the parts of the Altium scripting API related to the PCB editor. See the parent [Altium Scripting and Using the API](/electronics/general/altium/altium-scripting-and-using-the-api/) page for the general basics, and [Reading and Writing to Files](/electronics/general/altium/altium-scripting-and-using-the-api/reading-and-writing-to-files/) for file I/O from within a script.
...
The following code prompts the user to select a position on the PCB. The position (as x and y co-ordinates) the user clicks can then be used in subsequent code.
...
To prompt the user to select objects, use the function `GetObjectAtCursor();` on a PCB board (`IPCB_Board`) object. Its syntax is:
    
```txt
IPCB_Primitive GetObjectAtCursor(
...
The `TObjectSet` variable is normally made by calling the function `MkSet()`, which makes a set from the passed in object(s) (this is only required when scripting in Delphi, as Delphi doesn't natively support sets). The objects you can pass into `MkSet()` are part of the TObjectID enumeration and includes:
...
The example below requests the user to select a component from a layer.
...
Use the following function to create an arc and add it to the contour (which will be used once all the segments have been added to generate the region).
...
Make `Aclockwise` be `true` if going clockwise, `false` if going anti-clockwise.

### To Generate A Straight Section Of A Region
...
```txt
contour.AddPoint(x : integer, y : integer);
```

### How To Finally Add The Region To The PCB

Once you have defined a region by using the contour object, you can add it to the PCB with the following code. This assumes you have already created and populated a `contour` object as shown above.
...
Without specifically calling the functions `PCBServer.PreProcess` and `PCBServer.PostProcess`, you will lose undo/redo functionality when/after running Altium scripts.
...
You can get the following error when trying to save if you haven't called `PCBServer.PreProcess` an equal number of times as `PCBServer.PostProcess`. So far, if this does occur, I haven't worked out how to save the document (all changes since last save are lost!).

<Image src={import('./_assets/pcb-script-bug-a-command-is-currently-active-and-save-cannot-be-completed-at-this-time.png')} width="800px">You might get this error in Altium when trying to save because you have run a script which hasn't called PCBServer.PreProcess an equal number of times as PCBServer.PostProcess.</Image>
...
You can change the zoom on a PCB by using the following commands (VBscript):
...
PCB Components have a child object called `Name`, which refers to the designator.
