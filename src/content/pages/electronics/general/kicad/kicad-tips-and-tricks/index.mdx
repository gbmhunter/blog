...
DigiKey maintains the [digikey-kicad-library](https://github.com/Digi-Key/digikey-kicad-library), a KiCad schematic and footprint library of a large number of components that can be supplied by DigiKey. The aim of this library is to provide a collection of visually consistent, accurate library parts that have been curated by the DigiKey team. The organization of the library follows the DigiKey family taxonomy.

### Symbol Creation

The default grid step size in the symbol editor is 50mil (0.050" or 1.27mm). I recommend you leave the symbol grid size to its default when placing pins into symbols, as this will make the symbols consistent when placing onto schematics and prevent pins mis-aligning with the grid.
...
KiCad expects the PCB filename to have the same basename (i.e. excluding the `.kicad_pcb`) as the project file (`.pro`). This has to be true if you want to open the PCB from the KiCad application by pressing the `PCB Layout Editor` button.

### Output File Types

```text
.gbr            # Gerber plot file
.drl            # Drill file. KiCad will name the files xxx-PTH.drl and xxx-NPTH.drl
...
On Linux, KiCad uses the system-installed Python (whatever the command `python` points to). On all other platforms, KiCad ships with its own version of Python.
...
The Windows version of KiCad ships with Python v2.7. If you want to use Python v3.x instead, you can rename `python.exe` located at `C:\Program Files\KiCad\bin\python.exe` to something like `python.exe.old`. This will cause the system Python to be invoked instead of the version shipped with KiCad. The big limitation with this technique is that none of the scripts will be able to `import pcbnew`, so it is only really suitable for running scripts which do not depend on the Python KiCad API.
...
- `<ProjectName>.pro`
- `<ProjectName>.sch`: This is the "root level" schematic sheet. You don't need to rename other schematic sheets, just the one at the top-level in the hierarchy.
- `<ProjectName>.sch-bak`
...
- **laser_driver**: Good example if you want to know how to include [op-amps](/electronics/components/op-amps/) and transistors into your simulation. Notice that the pinout for the op-amp used in this schematic is very different to that of the op-amps in KiCad's two built-in simulation component libraries.
...
- `Spice_Primitive`: This determines the _element type_ (ngspice nomenclature) `R` for resistors, `C` for capacitors, `D` for diodes, `X` for subcircuits (anything that is built out of many primitives, i.e. ICs), etc. See the below table for a list of all the supported primitives.
- `Spice_Model`: The name of the SPICE model. This is the subcircuit name when `Spice_Primitive = X`, or the name of a `.model` card for primitives such as diodes (see the Zener diode example below).
...
- `SpiceMapping`: Appears to be an older equivalent of `Spice_Node_Sequence`.
...
If you are going to run an AC analysis (i.e. using the `.ac` command), you don't generally want more than one AC source in your schematic. If you have more than 1 AC source, the AC analysis results will be a superposition of all AC voltage sources and will be hard to interpret.
...
To simulate say, a 10k resistor, start with any two pin symbol (using a resistor symbol would make sense, but that is not a requirement, you could use a two-pin smily face emoji if you want :-D) and make the following changes to its symbol properties (adding new properties where they don't already exist):
...
To simulate a basic [Zener diode](/electronics/components/diodes/zener-diodes/), start with the `D_Zener` KiCad symbol, and make the following changes to its symbol properties (adding new properties where they don't already exist):
...
OR add another symbol property called `Spice_Lib_File` and add the path to a `.lib` file containing the same line as you would have added to a text field above. `Spice_Lib_File` can be either absolute or relative to a library root path.

Change the value `5.0` to whatever the breakdown voltage of your Zener is.
...
The default pin ordering for [BJT](/electronics/components/transistors/bipolar-junction-transistors-bjts/) SPICE models is:

1. Collector
2. Base
3. Emitter

However, KiCad's default pin ordering for its BJT symbols is:
...
For discrete [MOSFETs](/electronics/components/transistors/mosfets/), the recommended approach is to use the `VDMOS` intrinsic model. This uses a standard 3-pin MOSFET symbol in the following order:
...
Looking for `VDMOS` intrinsic models? LTspice ships with over 1000 of them. Once you install LTspice, you can find all the models within a single text file at `LTspice<version>/lib/cmp/standard.mos`. For example, on Windows with `LTspice XVII` the full path would be:
...
KiCad can properly convert its multiple part symbols (e.g. an op-amp IC with many op-amps, or a hex inverter IC) into a single "component" when creating netlists for ngspice.
...
Syntax: `.dc srcname vstart vstop vincr`
...
Note that if frequency is plotted on the x-axis, KiCad/ngspice will incorrectly export the data as `Time` in the csv file.
...
This error usually arises if you have not assigned unique designators to all components. Perhaps there are still `R?`, `C?` etc. designators on the schematic? Or multiple instances of properly labelled components, such as two `R1`'s. The best way to fix this is just to reset and re-annotate the schematics.
