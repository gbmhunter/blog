...
<Image src={import('./_assets/tekbox-emcview-screenshot.png')} width="700">Screenshot of the Texbox EMCview software[^bib-tekbox-emcview]. Image © 2021, Texbox.</Image>
...
At the heart of the LISN is the $50\unit{uH}$ inductor. It sits in the main power path between SOURCE and DUT. The impedance of an inductor is given by:

$$
Z_L = 2\pi f L
$$

At 50 Hz the impedance of this inductor is tiny ($Z_L = 2\pi \cdot 50\unit{Hz} \cdot 50\unit{uH} = 15.7\unit{m\Omega}$) and the inductor acts like a piece of wire -- passing the mains power from `SOURCE` to `DUT`. Across the conducted emissions band (150kHz to 30MHz) however, its impedance climbs from around $47\unit{\Omega}$ (comparable to the $50\unit{\Omega}$ of the spectrum analyzer) up to almost $10\unit{k\Omega}$ (effectively an open circuit), isolating the DUT from the supply so that the DUTs conducted noise is forced into the measurement port rather than back into the mains.

<Image src={import('./_assets/lisn-inductor-impedance-vs-frequency.png')} width="700px">The impedance of the LISN's 50µH series inductor vs. frequency, showing the 50Hz mains point and the 150kHz-30MHz conducted emissions band.</Image>
...
<Image src={import('./_assets/lit-930a-transient-limiter-freq-response.png')} width="600px">Frequency response of the LIT-930A transient limiter by Com-Power[^bib-com-power-lit-930a-ds]. Graph shows a flat 10dB attenuation across the pass-band between 9kHz and 30MHz. Image © Com-Power.</Image>
