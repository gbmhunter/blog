...
authors: [gbmhunter]
date: 2016-05-03
description: EMC design guidelines covering SMPS hot loops, inductor polarity, solid reference planes under traces, oscilloscope probe measurements and mu-metal shielding.
...
[Switch-mode power supplies (SMPS)](/electronics/components/power-regulators/switch-mode-power-supplies-smps/) can be a **large source of EMI in a circuit**, which can cause your PCB to fail EMC tests. Most of the noise is emitted at the switching frequency and its harmonics. Many manufacturers will provide recommended layouts for their SMPS ICs and surrounding components, and it's generally a good idea to follow these as closely as possible in your design. This includes things such as:

- Input and output [capacitors](/electronics/components/capacitors/) as close as possible to the SMPS IC.
- Keeping the current loops involving the capacitors, [inductor(s)](/electronics/components/inductors/) and switching node(s) as small as possible (w.r.t. to their area).
...
When inductors are used in switch-mode power supplies, one side of the inductor is normally connected to what is called the _switching node_. In a buck converter, the switching node is the side of the inductor that gets switched from $V_{IN}$ to $\mathrm{GND}$. In a boost converter, the switching node is the side of the inductor which gets switched from $V_{OUT}$ to $\mathrm{GND}$.

This switching node has a high $\frac{dV}{dt}$ --- its voltage changes rapidly between two values. The copper on the switching node can act as an antenna and radiate energy, contributing to generated EMC noise. One way to mitigate this is to minimize the amount of copper on the switching node, by placing the components close together and make sure it is shielded with a ground plane.
...
<Image src={import('./_assets/gap-in-reference-plane.webp')} width="500px">A gap in the reference plane forces the high frequency return current to detour around the gap, increasing the loop area and worsening EMI.</Image>
...
The loop made with the probe tip and its ground connection forms an unshielded antenna which will pick up high frequency noise in its vicinity.

Most general-purpose oscilloscopes have very poor common-mode rejection. The common-mode rejection capability of an oscilloscope probe can be gauged by firstly connecting the probe tip and ground together, and then connecting both up to a noisy PCB track. Ideally, the oscilloscope waveform should still be a flat line. The more noise seen on the oscilloscope, the poorer the probe's common-mode rejection.
...
For example, the output impedance of a SMPS may be a few $\unit{m\Omega}$, which then connects to the PCB tracks and scope probe, which may have an impedance of $50\unit{\Omega}$. This then connects to the oscilloscope which has an impedance in the $\unit{M\Omega}$ range.

These reflections can be minimised by adding the appropriate [termination resistors](/electronics/circuit-design/transmission-lines/termination/), e.g. a $50\unit{\Omega}$ resistor in series with the SMPS (as close to the SMPS output as you can), and then another $50\unit{\Omega}$ resistor in parallel (shunt to ground), as close to the oscilloscope input as you can.
