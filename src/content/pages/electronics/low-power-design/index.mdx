...
Indeterminate (floating or otherwise not either high are low) voltages on digital [CMOS](/electronics/circuit-design/logic-families/) inputs can cause the input circuitry to draw significant current. This is because a voltage in the middle of the range partially turns on both the N-channel and P-channel MOSFETs that form the input buffer. This causes shoot-through current to flow directly from VCC to GND.

Make sure that all of your digital inputs will be in defined states during sleep. This can be done by adding pull-up or pull-down resistors to the inputs. Add a pull-up resistor if the default pin state is HIGH, and a pull-down resistor if the default pin state is LOW so that you are not drawing extra current.

Another way to fix this is to change the pin to an analog input (via firmware) during sleep. This will disconnect the CMOS input buffer and allows you to keep the pin floating if needed.
