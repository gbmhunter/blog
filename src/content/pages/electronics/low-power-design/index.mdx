...
## Executing Code from RAM Instead of Flash

Program code it typically executed from flash memory. However, since RAM is just a different memory address, there is generally no reason why code cannot be executed from RAM, as long as you copy the code to RAM on startup and setup the linker correctly. Executing code from RAM can provide two avenues for power savings:

- Executing from RAM is typically faster than flash, reducing the time the MCU is awake before it sleeps again.
- Executing from RAM can allow flash memory to be disabled (if your MCU supports this) to save power.

These options are not mutually exclusive, i.e. you may execute from RAM and see benefits from faster execution AND lower power consumption from the flash controller.

I have heard of this technique being used on ST Microelectronics STM32U5 and WCH MCUs.

<Aside type="tip">
Executing from RAM may also be useful in bootloaders to update flash -- you can't execute from code that you're in the process of modifying!
</Aside>
