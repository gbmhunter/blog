...
<Image src={import('./_assets/mqtt-logo.png')} width="500px">The MQTT logo[^eclipse-paho].</Image>
...
On March 7, 2019, MQTT Version 5 was released, and is the most recent major version.
...
The MQTT broker is the central server which "brokers" messages between clients. Clients connect to the broker and publish messages to specific topics. Other clients can then subscribe to those topics and receive the messages. Brokers are typically run on a cloud-based server, while clients run on embedded devices and inside software applications.
...
By default the above command will start an MQTT broker listening on port `1883` (the default port for non-encrypted traffic). The broker will not run as a daemon, so you can stop the broker with `Ctrl-C` (or equivalent). It also uses the following defaults:
...
[Paho](https://pypi.org/project/paho-mqtt/) is a popular MQTT client for Desktop and server machines running Windows, Linux or macOS. Its API is reasonably well documented at https://pypi.org/project/paho-mqtt/.
...
MQTT-SN (MQTT for Sensor Networks, previously known as just MQTT-S[^bib-mqtt-mqtt-specifications]) is a version of the MQTT protocol which has been optimized for use on low-power, low-performance, wireless IoT devices. It is designed to work on non-TCP/IP networks, such as Zigbee.[^bib-mqtt-mqtt-specifications]

The diagram below shows the high-level MQTT-SN architecture, and how MQTT-SN clients talk to a standard MQTT broker.

<Image src={import('./_assets/mqtt-sn-architecture.png')} width="600px">High-level diagram of the MQTT-SN architecture[^bib-oasis-open-mqtt-protocol-spec-v1.2].</Image>

Some of the changes in MQTT-SN include (compared to standard MQTT):

1. Shortened two-octet (byte) topic IDs are used instead of full topic names (strings).[^bib-ublox-mqtt-sn] This can cut down on the number of bytes sent for topic identification considerably, e.g. the topic name `device1/sensor5/temperature` now just becomes topic ID `54`.
...
[^bib-ublox-mqtt-sn]: ublox (2020, Jun 16). _MQTT-SN – lowering the cost of IoT at scale_ [blog post]. Retrieved 2022-06-20, from https://www.u-blox.com/en/blogs/insights/mqtt-sn.
[^bib-mqtt-mqtt-specifications]: MQTT. _MQTT Specifications_ [website]. Retrieved 2022-06-20, from https://mqtt.org/mqtt-specification/.
[^bib-oasis-open-mqtt-protocol-spec-v1.2]: Andy Stanford-Clark and Hong Linh Truong (2013, Nov 14). _MQTT For Sensor Networks (MQTT-SN) Protocol Specification: Version 1.2_ [specification]. Retrieved 2022-06-21, from https://www.oasis-open.org/committees/download.php/66091/MQTT-SN_spec_v1.2.pdf.
[^bib-wikipedia-mqtt]: Wikipedia (2022, Jun 21). _MQTT_ [wiki]. Retrieved 2022-06-22, from https://en.wikipedia.org/wiki/MQTT.
[^bib-paessler-brief-history-mqtt]: Shaun Behrens (2019, Aug 09). _From Oil Pipelines to the IoT: A Brief History of MQTT_ [blog post]. Paessler. Retrieved 2022-06-22, from https://blog.paessler.com/a-brief-history-of-mqtt.
[^bib-stackoverflow-is-message-order-preserved-in-mqtt-messages]: Stack Overflow. _Is message order preserved in MQTT messages?_ [forum post]. Retrieved 2022-10-03, from https://stackoverflow.com/questions/30955110/is-message-order-preserved-in-mqtt-messages.
[^bib-oasis-mqtt-v3.1.1-specifications]: OASIS. _MQTT Version 3.1.1 Specifications_ [specification]. Retrieved 2022-10-03, from http://docs.oasis-open.org/mqtt/mqtt/v3.1.1/os/mqtt-v3.1.1-os.html.
[^eclipse-paho]: Eclipse. _Paho_ [website]. Retrieved 2021-03-30, from https://www.eclipse.org/paho/.
[^influx-a-guide-to-mqtt]: Influx. _A Guide to MQTT - MQTT Guide - Definition, Use Cases, Applications, and Resources_ [website]. Retrieved 2024-09-25, from https://www.influxdata.com/mqtt/.
