...
It is not commonly used for intra-board communication (i.e. between two devices on the same PCB) because in these scenarios there is normally no need for the noise immunity and transmission distances RS-485 provides, and communication protocols such as [UART](/electronics/communication-protocols/uart-communication-protocol/), [SPI](/electronics/communication-protocols/spi-communication-protocol/) and [I2C](/electronics/communication-protocols/i2c-communication-protocol/) are more prevalent.

A common and cheap way to implement RS-485 is to use half-duplex transceivers. Examples are the SP485 (cheap), or the SP4082 (slightly more expensive, but with slew-rate limiting and ESD protection).
...
RS-485 transceivers usually top out at about 10-50Mbps.
...
This implies that $120\Omega$ termination resistors should be used with this twisted-pair cable. Termination resistors of $120\Omega$ connected between the two differential lines work well at reducing reflections along long cables.
...
There is somewhat of a standard pinout for RS-485 transceivers in 8-pin component packages. The table below lists this pinout.
...
<table>
  ...
  <caption>Pinout for a typical 8-pin RS-485 transceiver component.</caption>
</table>

8-pin packages that RS-485 transceivers come in include DIP-8, SOIC-8, TSSOP-8 and MSOP-8. Example transceivers that follow this "standard" include the [Texas Instruments DS485](http://www.ti.com/lit/ds/symlink/ds485.pdf) and [Linear Technology LTC1480](http://cds.linear.com/docs/en/datasheet/1480fa.pdf).
...
[PROFIBUS](/electronics/communication-protocols/profibus-protocol/) can use RS-485 as its physical layer.
...
<Image src={import('./_assets/typical-rs-485-a-b-waveforms-during-communication.png')} width="800px">The typical appearance of the voltages on the A (top) and B (bottom) nets of a RS-485 bus under normal operation. This was operating at 115200 baud.</Image>
...
They usually are a bidirectional [TVS diode](/electronics/components/diodes/tvs-diodes/) (or diode array) with asymmetric breakdown voltages, that match the maximum voltage specifications of +12V and -7V for RS-485 data lines.
...
One such example is the Semtech SM712 diode array. The figure below shows the internal schematic of the component.

<Image src={import('./_assets/sm712-tvs-diode-array-designed-for-rs-485-data-lines.png')} width="300px">The SM712 TVS diode array, a diode array with asymmetric breakdown voltages, specifically designed for protecting RS-485 data lines. Image from http://www.semtech.com/.</Image>
...
<Image src={import('./_assets/rs-485-transceiver-with-open-collector-modifications-wired-or-sae-j1708.png')} width="800px">A standard RS-485 transceiver with a modified bus connection to provide open collector (wired OR) arbitration for the SAE-J1708 bus. Image adapted from content found at http://www.ti.com/lit/an/snla038b/snla038b.pdf.</Image>
...
The RS-485 and CAN bus are both protocols that tick many of the same boxes when you are thinking about a multi-drop, differential communications protocol for an application. So which one should you use?
...
Due to the extra complexity of a CAN peripheral, using a CAN bus will typically be more expensive than using a RS-485 bus. This might be a consideration for high-volume, cost-sensitive products.
...
[^ti-an-1057-ten-ways-to-bullet-proof-rs-485]: Texas Instruments (2013, Apr). _AN-1057: Ten Ways to Bulletproof RS-485 Interfaces_ [application note]. Retrieved 2024-10-10, from https://www.ti.com/lit/an/snla049b/snla049b.pdf.
