...
tags: [CAN bus, bus, communication protocol, CAN1.0, CAN2.0, CAN base frame, CAN extended frame, USB adapters, NoCAN, encoding, controller, CANopen, NMEA 2000, termination resistors, FlexRay, SAE, J1850, J1939, ISO 11783, ISOBUS, isolation, mailboxes, CAN FD, NXP, Secure CAN, OBD2]
title: CAN Protocol
type: page
---
...
<Image src={import('./_assets/can-bus-logo-bosch.png')} width="270px">The CAN bus logo. Image from www.bosch.com.</Image>
...
<Image src={import('./_assets/oh-look-its-a-can-bus.png')} width="500px">Oh look, it's a CAN bus!</Image>
...
<Image src={import('./_assets/can-bus-bit-rate-vs-bus-length.png')} width="500px">Graph showing the relationship between the CAN bus transmission rate and maximum bus length.</Image>
...
* Bueno Electric's _Maximum Cable Length For a CAN Bus_ page[^bueno-electric-max-cable-len] gives a maximum cable stub length of 0.3m for all speeds from 50kb/s to 1Mb/s.
* OnSemi[^on-semi-topo-high-speed-can] give the following rule:
    $$
    \begin{align}
    L_{STUB\_MAX} = \frac{T_{PROP\_SEG}}{50 \cdot T_{PROP(BUS)}}
    \end{align}
    $$
...
<Image src={import('./_assets/can-bus-cia-ds-102-9-pin-sub-d-connector-pin-assignment.png')} width="500px">CAN bus pin assignments for the DE-9 serial connector from the CiA DS-102 standard.</Image>
...
<Aside type="tip">
Car thieves have been cleverly using a special transmitter that also drives to the dominant level to override other messages on a vehicle's CAN bus network, which has allowed them to break in and steal cars by accessing the CAN bus from the front headlights[^ken-tindell-keyless-car-theft]. See [CAN Injection: Keyless Car Theft](https://kentindell.github.io/2023/04/03/can-injection/) for more info.
</Aside>
...
If we zoom in on one of the arbitration events as below we can see it aligns with the bit period of 8us (the CAN bus was running at 125kb/s).

<Image src={import('./_assets/arbitration-waveform/waveform-zoomed-in-annotated.png')} width="600px">Zoomed in view of the above scope capture, showing the width of the arbitration "events" aligns with the bit period of 8us (CAN bus was running at 125kb/s).</Image>

## CAN Bit Timing

The below diagram shows the timing of a single CAN bit. **A single CAN bit is broken down into different time segments**. Each time segment is defined by an integer number of _time quanta_ ($t_q$). Time quanta is determined by the clock provided to the CAN transceiver. This CAN clock is usually provided from a faster system clock by dividing by a programmable pre-scalar. The minimum is 5, but there is usually 8 or more $t_q$ per CAN bit. The _time quanta_ is the smallest amount of time that the CAN transceiver can adjust its timing by (i.e. its resolution).

1. `SYNC_SEG`: This is always $t_q$. The transceiver expects the bit transitions to occur during this time period. If they don't, the transceiver performs [resynchronization](#resynchronization) to re-align itself.
1. `PROP_SEG`: This is the time segment of the bit in which it is expected that the transition will finish propagating to all nodes. It is the _two way propagation_ that matters, i.e. the total time for the transmitted bit to propagate to all nodes and then the node's response to propagate back. This has to include the propagation time of the signal on the wire (5ns/m is a good approximation), plus propagation delay through the CAN transceivers.
1. `PHASE_SEG_1`: Phase segment 1.
1. `SAMPLE_POINT`: This is the point in time in-between `PHASE_SEG_1` and `PHASE_SEG_2` at which the transceiver samples the value on the bus (recessive or dominant).
1. `PHASE_SEG_2`: Phase segment 2.

<Image src={import('./_assets/can-bit-timing.webp')} width="900px">Timing diagram of a single CAN bit.</Image>
...
<table>
  ...
  <tbody>
    ...
    <tr><td>PHASE_SEG_2</td>  <td>$max(IPT, t_{PHASE\_SEG\_1})$</td></tr>
  </tbody>
</table>
...
<Image src={import('./_assets/can-bit-timing-prop-delay-between-two-nodes.webp')} width="800px">Diagram showing why the two-way propagation of the bit must occur between any two nodes before the end of the `PROP_SEG`.</Image>
...
TIP: Remember that the dominant state is driven, hence the recessive-to-dominant transition is always going to be sharper than the dominant-to-recessive transition, hence why the former edges are used for resynchronization.

A transmitting node will not resynchronize on a positive phase error. This means that a transmitting node will not resynchronize due to propagation delays of its own message. Instead, it is left up to the receivers to resynchronize.
...
<Image src={import('./_assets/can-standard-base-frame-format-data-remote.png')} width="850px">The standard (base) CAN frame format for both data and remote frames.</Image>
...
_**r0 bit:**_ This bit is reserved for future CAN bus standards use. Always recessive.
...
_**2-bit ACK:**_ The sender sends a recessive value for the first acknowledge bit of the acknowledge field (called the _ACK slot_). The receiver(s) drives the first acknowledge bit to the dominant state if it wants to acknowledge the bit. The 2nd bit of the acknowledge field is the ACK delimiter and is driven recessive by the transmitter.
...
* CAN Base Frame (CAN2.0A)
* CAN Extended Frame (CAN2.0B)
...
To prevent faulty CAN nodes from permanently disturbing/blocking a CAN network, the CAN standard defines a somewhat sophisticated _fault confinement process_ that nodes must adhere to. This fault confinement process is usually implemented in CAN controllers/peripherals, such that the main application does not have to deal with this itself (many users of CAN systems may be unaware that these fault confinement processes even exist!).
...
* In the _Error Passive_ state, a node will transmit _Passive Error flags_ when it detects an error. After transmitting a message it must also wait an additional 8 bit times (the _suspend transmission_ period) before it is allowed to start a new transmission.
...
The CANopen standard is defined by the CiA (CAN in Automation) group. The documents for these standards can be found at https://www.can-cia.org/groups/specifications/. The most important document is CiA 301, which defines the CANopen application layer. If the above link is down, you can view the [local cached copy, v4.2.0, accessed June 2020](/misc/can-protocol/cia-301-canopen-specification-document.pdf).

All CANopen nodes must have an object dictionary.

### FlexRay

FlexRay is a newer protocol that has been designed to overcome some of the limitations of the CAN bus. It supports much longer message data lengths and has improved CRC/error detection. However it is more expensive to implement than CAN and, as of June 2020, is still not as popular worldwide as CAN.

### ISO 11783

ISO 11783 is titled "Tractors and machinery for agriculture and forestry—Serial control and communications data network" and is commonly called _ISOBUS_. It is based on the SAE J1939 protocol (which includes the CAN bus).
...
### OBD2, ISO 15765-4 and ISO-TP

OBD2 (On Board Diagnostics) is a standard to read diagnostic information from a car. It is normally thought of as the OBD2 connector. One of the protocols used by OBD2 is ISO 15765-4, which is a CAN-based protocol.

<WrappableRow>
<Image src={import('./_assets/konnwei-obd2-car-scanner.webp')} width="400px">A Konnwei OBD2 car scanner. It plugs into the OBD2 port in a car, and communicates with a phone via Bluetooth. You use an app to control the device.</Image>
<Image src={import('./_assets/car-scanner-app-showing-obd2-can-connection.webp')} width="300px">A car scanner app with the CAN connection highlighted.</Image>
</WrappableRow>
...
<Image src={import('./_assets/can-bus-licensing-fee-highlighted-bosch.png')} width="500px">A screenshot of the CAN bus licensing fee details from Bosch. Image from http://www.bosch-semiconductors.de/media/automotive_electronics/pdf_2/ipmodules_3/can_protocol_license_1/Bosch_CAN_Protocol_License_Conditions.pdf.</Image>

## NoCAN

NoCAN is a communications protocol that is **built on-top of the CAN bus**. It provides a layer of abstraction on-top of a 125kb/s CAN bus which adds _publish-subscribe based messaging_ and _automated address assignment_. With many wireless options available for IoT devices, NoCAN was borne out the idea that there is a need for an easy-to-use wired communications solution for IoT devices. The protocol was created by Omzlo and was [funded in part by a KickStarter campaign](https://www.kickstarter.com/projects/1242572682/nocan-the-wired-iot-platform-for-makers) in 2019.

<Image src={import('./_assets/omzlo-white.png')} width="200px">The Omzlo logo.</Image>
...
The 82527 is an older CAN controller made by Intel[^intel-82527]. It was Intel's first CAN controller that supported CAN Specification 2.0.
...
<Image src={import('./_assets/nxp-tja115x-secure-can-application-principle.png')} width="500px">The basic principle of the TJA115x Secure CAN range of CAN transceivers from NXP[^nxp-tja115x-secure-can].</Image>
...
<Image src={import('./_assets/pca82c200-can-controller-block-diagram.png')} width="800px">Block diagram of the PCA82C200 CAN controller IC[^philips-pca82c200-ds].</Image>
...
## Contactless CAN Bus Sniffers

Contactless CAN bus sniffers are devices which can read the data on a CAN bus without electrically connecting to it. They work by attaching themselves around the CAN high and CAN low wires (they require a small section of these wires to be physically separated from one another). I assume they detect the magnetic field generated by these two wires and use that to read the data. They are commonly used in the automotive industry to attach 3rd party electronic devices into a vehicle without the physical difficulty and compliance issues with electrically connecting to the bus.

<Image src={import('./_assets/cancrocodile-contactless-can-bus-sniffer.webp')} width="500px">The CANCrocodile contactless CAN bus sniffer from Technoton.[^technoton-cancrocodile]</Image>

They will not work if you have a shielded twisted pair cable, as the shield and the twisting will block/cancel out the magnetic field. You need a small section of the wire where you can separate the CAN high and CAN low wires.
...
Espressif calls their ESP32 CAN bus peripheral the _Two-Wire Automotive Interface (TWAI)_[^espressif-esp32-twai], presumably to avoid [Bosch licensing fees](https://en.m.wikipedia.org/wiki/CAN_bus#Licensing). There is quite a lot of [Errata for their TWAI peripheral](https://www.espressif.com/sites/default/files/documentation/esp32_errata_en.pdf), and some of the bugs seem serious. Be sure to read that if you're deciding whether or not to use the ESP32 TWAI peripheral!
...
Many microcontroller CAN peripherals contain CAN _mailboxes_. A mailbox is a storage place in hardware for a CAN frame (message) which is either being sent or received. Microcontrollers typically have 4-16 mailboxes, with them being a mixture of fixed transmit or receive mailboxes, or having the ability to configure each mailbox as either for transmit or receive. The concept of a _mailbox_ significantly reduces the CPU load on the microcontroller when transmitting and receiving CAN frames of interest. To send CAN frames, you will need to use at least one mailbox, but you can have multiple if needed. Multiple transmit mailboxes can be useful if you want to schedule multiple frames for transmission on the bus, and also provide a priority (higher priority frames will be sent first).
...
* STM32F microcontrollers with CAN peripherals have a number of transmit/receive mailboxes.
...
<Image src={import('./_assets/iso-11898-2-can-bus-repeater.png')} width="600px">A CAN bus repeater topology from the CiA ISO 11898-2 standard.</Image>
...
<Image src={import('./_assets/amis-42770-on-semiconductor-can-bus-repeater-ic.png')} width="600px">Application diagram from the AMIS-42770 IC's datasheet which shows how it can be configured to act as a CAN bus repeater.</Image>
...
<Image src={import('./_assets/ems-crep-s4-can-bus-repeater.png')} width="500px">The CRep S4 CAN Bus repeater by EMS Wuensche.</Image>
...
Link: [http://www.ti.com/lit/ug/tidudb5a/tidudb5a.pdf?ts=1591658758534](http://www.ti.com/lit/ug/tidudb5a/tidudb5a.pdf?ts=1591658758534) [local cached copy](/misc/can-protocol/ti-can-fd-repeater-reference-design-tida-01487.pdf)

One informative diagram in this document is the block-level architecture of the repeater:

<Image src={import('./_assets/reference-ti-can-repeater-design-tida-01487.png')} width="700px">The block-level architecture of the CAN bus repeater design by Texas Instruments. Image from http://www.ti.com/lit/ug/tidudb5a/tidudb5a.pdf?ts=1591658758534.</Image>
...
<Image src={import('./_assets/can-bus-tvs-diodes-littelfuse-aq24canfd-block-diagram-and-application-example.png')} width="600px">Block diagram and application example for the CAN bus AQ24CANFD TVS diode from Littelfuse. Image from https://www.littelfuse.com/~/media/electronics/datasheets/tvs_diode_arrays/littelfuse_tvs_diode_array_aq24canfd_datasheet.pdf.pdf, acquired 2021-04-27.</Image>
...
<Image src={import('./_assets/peak-pcan-usb-photo.jpg')} width="400px">Product photo of the PEAK PCAN-USB USB to CAN bus dongle[^peak-pcan-usb].</Image>
...
<Image src={import('./_assets/can-fd-frame-structure-feff-arbitration-and-control-fields.webp')} width="1000px">The CAN FD FEFF frame structure showing the arbitration and control fields.</Image>
...
[^bueno-electric-max-cable-len]: Bueno Electric. _Maximum Cable Length For a CAN Bus_. Retrieved 2021-10-11, from https://www.buenoptic.net/encyclopedia/item/537-maximum-cable-length-for-a-can-bus.html.
...
[^technoton-cancrocodile]: Technoton. _CANCrocodile Mini contactless CAN bus data sniffer_ [product page]. Retrieved 2025-05-26, from https://jv-technoton.com/products/contactless-readers/cancrocodile-mini/.
