...
UART (_Universal Asynchronous Receiver/Transmitter_) is a lower-voltage, microcontroller friendly equivalent of the RS-232 digital data transmission protocol with origins dating back to the 1960's. It is universal in the sense that the timing, voltages, flow control and error checking can be configured.
...
The first bit in a UART frame. Always just one bit period wide. The receiver uses the falling (first) edge of the start bit to synchronize it's sampling clock, the first data bit will be sampled 1.5 bit periods after this transition.
...
UART sends data in groups of 5, 6, 7 or 8 bits at a time across the bus (typically 8, to equal a single byte) in what is called a _UART frame_. The bus idles "high" when no data is sent. To indicate the start of a frame, the transmitter drives the line low for one bit period, this is called the _start bit_. The receiver synchronizes its incoming bit sampling clock with the falling edge of the start bit. The start bit lasts for one bit period, at which point the transmitter sends the bits of data by driving the line either high or low. This is then followed by an optional _parity bit_ (which is commonly excluded). Then this is followed by 1, 1.5 or 2 _stop bits_.
...
UART has no say in what the data means, it is up to the application code to make sense of what these bits of data mean and to group them into larger data structures.
...
- 600, 1200, 2400, 4800, 9600, 14400, 19200, 28800, 38400, 56000, 57600, 115200, 128000, 256000, 460800, 921600
...
The transmitter sends a break signal by **driving its TX line low for a period longer than one frame**. There are two types of breaks, short breaks and long breaks. A short break is when the TX line is driven low for a period of between 1 and 2 frame lengths, and a long break is when it is driven low for a period of more than 2 frame lengths.
...
When using UARTs on microcontrollers, be careful about _backfeeding_ (a.k.a _backpowering_) devices through the `TX` line! If a microcontroller loses its power rails during normal operation, the `TX` line connected to the microcontroller's `RX` can keep its `VCC` afloat by conducting through the internal ESD diodes (remember that in the idle state, the TX line is driven high).
...
Spurious garbage can be sent along a microcontroller's UART TX line when the microcontroller resets. When microcontrollers reset, all of their GPIO pins typically default back to high-impedance inputs. This will cause the voltage on the TX line, typically idling HIGH, to collapse and signal `LOW`. The UART receiver on the other end of the bus could interpret this as data and give nonsensical garbage. As shown in the below image, a solution to this is to add a pull-up resistor to $V_{CC}$, which keeps the TX line HIGH when the microcontroller resets.
...
A easy to use and powerful terminal program for Windows. Stolen from the website, its description is:
...
A simple and tidy Windows terminal program. Personally, it doesn't get the same amount of respect as RealTerm because of its simplicity and slightly buggy nature. When decoding into hex, the program can hang if you're receiving large amounts of data. It can also hang if you disable the COM port while it is still connected.
...
Some UART protocols have radiation tolerant devices, such as the DRS4485, a Dual RS-485 Interface Transceiver made by Aeroflex.[^aeroflex-drs4485-dual-rs485-interface-transceiver]
...
RS-485 is another very common protocol that UART is converted to and from. It is usually chosen over RS-232 when longer distances and/or larger noise immunity is needed. For more information, see the [RS-485 page](/electronics/communication-protocols/rs-485-protocol).

## Cables

You can get null-modem USB-to-USB serial port emulator cables. These are awesome for transferring data between two computers (or any 2-USB host devices) without reverting to a true USB-to-USB A cable (which requires use of a more complicated protocol).
...
The FTDI FT232RNQ is a USB to UART IC in a [QFN-32](/pcb-design/component-packages/qfn-component-package/) or SSOP-28 package. It supports USB 2.0 Full Speed (12 Mbps) and data rates up to 3 Mbaud. It also has 4 GPIO pins which can be controlled from the USB host for auxiliary functions. It supports I/O voltages from 1.8 to 5.5 V.[^ftdi-ft232rnq-product-page]

<Image iref="fig-3d-render-of-ftdi-ft232rnq-ic" src={import('./_assets/3d-render-of-ftdi-ft232rnq-ic.webp')} width="200px">A 3D render of the FTDI FT232RNQ IC.[^ftdi-ft232rnq-product-page]</Image>

See the [Controlling FTDI Devices](/programming/ftdi/) page for more information on how to control these devices from software such as Python and the `pyftdi` library.
...
The WCH CH348 is a USB to 8 UART IC, similar to the WCH CH9344 USB to 4 UART IC. Like the CH9344, the UARTs have independent voltage rails that can be set to 1.8V, 2.5V or 3.3V. As shown in <IRef iref="fig-ch348-usb-to-8-uart-ic-pinout" />, the IC comes in two different packages. The larger LQFP-100 package provides flow controls pins for every single one of the UARTs.

<Image iref="fig-ch348-usb-to-8-uart-ic-pinout" src={import('./_assets/ch348-usb-to-8-uart-ic-pinout.webp')} width="900px">The pinout of the WCH CH348 USB to 8 UART IC, in the two different package offerings. The CH348Q is a QFN-48 48-pin 7x7mm package and the CH348L is a LQFP-100 100-pin 14x14 mm package.[^cnx-software-wch-ch348-8-port-usb-to-serial-chip]</Image>
...
[^ftdi-ft232rnq-product-page]: FTDI Devices. _Products >  FT232RNQ_ [product page]. Retrieved 2025-12-02, from https://ftdichip.com/products/ft232rnq/.
[^aeroflex-drs4485-dual-rs485-interface-transceiver]: Aeroflex (2010, May 27). _DRS4485 Dual RS485 Interface Transceiver_ [datasheet]. Retrieved 2026-06-02, from https://pdf.datasheet.live/datasheets-1/aeroflex_%5Bcolorado_springs%5D/DRS4485-S.pdf.
