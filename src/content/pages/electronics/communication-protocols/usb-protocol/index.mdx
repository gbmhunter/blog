...
OTG hosts running of a LiPo battery supply can use a 1.5X inductor-less charge pump to provide power for Vbus. They typically are limited to providing 100mA to the peripheral.
...
* USB2.0: Maximum transmission speed = 400Mbit/s
* USB3.0: Maximum transmission speed = 5Gbit/s
...
Alternate Modes are a way to use the USB C connector for transmitting data which is not defined in the USB specification. The four high-speed lanes (TX1, TCX2, RX1, RX2), two side-band pins (SBU) and five additional pins (for dock, detachable device and permanent-cable applications only) are available for use with alternate modes. The high-speed lanes can be used in any direction, i.e. they are not restricted to the directions imposed by the USB specification. If at least two high-speed lanes are unused by the alternate mode, a USB 3.0/3.1 connection can be created at the same time as the alternate mode. Given alternate modes to not use the D+/D- pins, a USB 2.0 link is always available.[^wikipedia-usb-c]
...
DisplayPort can be used over a USB C connector it what is called DisplayPort Alt Mode. It uses the official alternate mode of the USB specification. VESA (an international nonprofit corporation that you might of heard of via a "VESA mount") published the standard. DisplayPort Alt Mode was the first video and audio standard to be supported over USB C. USB C cables which explicitly support DisplayPort Alt Mode may show a DisplayPort logo (DP) near the connector.[^dell-guide-to-display-port-over-usb-c]
