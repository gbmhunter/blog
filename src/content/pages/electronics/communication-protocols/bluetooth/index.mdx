...
import BleAdvDecoder from 'src/components/calculators/ble-adv-decoder/BleAdvDecoder.jsx';
...
<Image src={import('./_assets/bleuio-products-and-pricing.webp')} width="800px">A screenshot from the BlueIO website showing their products and pricing, as of Nov 2025.[^blueio-homepage]</Image>
...
The nRF21540 is a range extender FEM (front-end module) IC for 2.4 GHz RF communications, specifically targetted towards Bluetooth. 

Below is a picture of the contents of the nRF21540 DB (development bundle). It contains two boards:

1. **nRF51540 EK**: An analogue nRF21540-only dev. board in which you can pass RF in and out.
...
The most crucial regulation that most engineers will find them concerned with is the transmit power of the Bluetooth signal. The transmit power is usually adjustable in firmware -- not only to meet local regulations but also to optimize power and limit range on purpose (sometimes limiting range on purpose is useful -- imagine if your credit card tap-to-pay worked from a kilometer away!).
...
In the USA the transmission power is more permissive, with the FCC limits Bluetooth transmission power to +30 dBm but PSD (power spectral density) must be below 8 dBm / 3kHz if no adaptive FHSS is implemented. There are two channels with exceptions:[^dewine-labs-bluetooth-tx-power-regulations]
...
[^blueio-homepage]: BlueIO. _Homepage_ [website]. Retrieved 2025-11-11, from https://www.blueio.com/.
