...
Shortened Local Name | 0x08 | A shortened version of the complete local name. A shortened name should only contain contiguous characters from the start of the full name.[^bluetooth-sig-data-types-specification] |
Complete Local Name | 0x09 | The human readable name of the device. A user would typically see this name when scanning for devices on their mobile phone. |
Manufacturer Specific Data |  | A place for custom data that doesn't fit in any of the other specified types. This field is commonly used. |
