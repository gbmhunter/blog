...
In the 2-wire variant, both the power and signal are transmitted over the same pair of wires. These are called _self-powered_ or _loop-powered_ devices.
...
NAMUR, an international association of process instrumentation companies, made a recommendation called _NAMUR NE43_ to standardise sensor fault indication on the 4-20mA current-loop protocol.[^divize-namur-ne43]
...
<Image src={import('./_assets/namur-ne-43-current-loop-current-ranges-diagram.png')} width="560px">The current ranges allowed by the NAMUR NE 43 current-loop standard.</Image>
...
<Image src={import('./_assets/4-20ma-current-loop-interface-to-micro-schematic-with-filter-and-buffer.png')} width="800">A schematic showing how to interface a 4-20mA current loop signal to the ADC on a microcontroller. The circuit also has a low-pass filter and a buffer.</Image>
...
The [Voltage-To-Current Converters page](/electronics/components/current-sources-and-sinks/voltage-to-current-converters/) has more info (including schematics), on how to build 4-20mA transmitters.
...
[^divize-namur-ne43]: DIVIZE industrial automation. _Namur NE43_. Retrieved 2026-06-03, from https://www.divize.com/techinfo/namur-ne43.html.
