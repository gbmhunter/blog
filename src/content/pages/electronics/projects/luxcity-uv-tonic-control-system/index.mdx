...
authors: [gbmhunter]
date: 2012-09-28
description: The design and build of a control system to sequence 64 relays and 62 solenoid valves, pumping UV-reactive tonic water through pipes for the Luxcity 2012 event in Christchurch. Uses an Arduino Uno, Freetronics relay driver shields, and the Vixen light sequencing software.
...
or the Luxcity event page itself ([http://www.luxcity.co.nz/](http://www.luxcity.co.nz/), **as of Dec 2017, website dead**)
...
Post-event there has been another news article:
...
The repo for this project can be found on GitHub at [https://github.com/gbmhunter/LuxcityTonicUVControlSystem](https://github.com/gbmhunter/LuxcityTonicUVControlSystem). It contains the firmware and datasheets.
...
The [I2C](/electronics/communication-protocols/i2c-communication-protocol/) controlled shield provides 8 outputs that can drive standard [relays](/electronics/components/relays/). Being I2C controlled, it only uses 2 pins of the Arduino. Luckily, the I2C address settings are 3-bit, meaning I could just control all 8 from a single I2C line. The boards feature the MCP23017 I2C expander IC ([datasheet](http://ww1.microchip.com/downloads/en/DeviceDoc/21952b.pdf)).
...
64 relays are needed to control the 62 tonic water/air solenoid valves (plus 2 spare). Since there were 64 of them, they needed to be cheap. These ones were bought for around NZ\$3 each.
...
<Image src={import('./_assets/solenoid-minimum-operating-pressure.png')} width="500px">The solenoid valve datasheet showing the minimum operating pressure of 20kPa.</Image>
...
The LCD screen was one of the easiest parts of this project to set up. Built-in libraries for controlling an LCD are included in the Arduino IDE. It literally took about 5 minutes to get this working (see below).
...
To get around the problem with Vixen not outputting cells with an intensity of 0 (the firmware can't recognise what output value is for what channel unless all 64 are output in the correct order), I made the minimum allowable intensity equal to 1, so that Vixen outputs a value for every cell at every time step. The firmware then only turns a relay on when it sees an intensity of 255.

<Image src={import('./_assets/vixen-screenshot-tonic-test-sequence.png')} width="600px">A screenshot of the Vixen software with the tonic test sequence loaded.</Image>
...
The relay boards and LCD were stacked on top of each other.

<Image src={import('./_assets/arduino-and-relay-shield-stack-vertical.jpg')} width="600px">An Arduino stack with the Uno (at bottom), 8 relay shields, and an LCD shield on top.</Image>

Wiring up the relay boards.

<Image src={import('./_assets/relay-shiled-wiring-02.jpg')} width="600px">More wires!</Image>

Placement of the relays onto [stripboard](/pcb-design/prototyping/#stripboard) (they JUST managed to fit all on one board).
...
After testing a solenoid valve with the system, the comms between the computer and Arduino dropped out when the solenoid was turned off. I realised that although the relay shield boards had protection [diodes](/electronics/components/diodes/), I still needed to add some to the solenoids (on the relay outputs), as this inductive kickback was coupling into the digital system. Luckily I had a string of 1N4003 diodes on hand...
...
The bottom-side of the prototype board, fully soldered. Notice the heavy duty rails that deliver the power to the relays/solenoids. Even with these thick lines of solder, when half (31) of the solenoids were turned on, the board got considerably warm around the PSU entry point.

<Image src={import('./_assets/finished-soldering-the-prototype-board.jpg')} width="600px">The bottom-side of the prototype board, fully soldered. Notice the heavy duty rails that deliver the power to the relays/solenoids. Even with these thick lines of solder, when half (31) of the solenoids were turned on, the board got considerably warm around the PSU entry point.</Image>
...
The solenoids were numbered so sequencing wouldn't become confusing when out in the field.
...
<Image src={import('./_assets/vixen-screenshot-tonic-test-sequence.png')} width="600px">The Vixen software sequence used to test the solenoids. Each solenoid is turned on for 1 second in sequential order.</Image>

The settings to configure the 'Generic Serial' output plugin in Vixen.

<Image src={import('./_assets/vixen-tonic-generic-serial-output-plugin-settings.png')} width="600px">The settings used to configure the 'Generic Serial' output plugin in Vixen.</Image>
...
Carrying the water tank for the gin and tonic drinks down the road to the construction site.


<Image src={import('./_assets/carrying-tank-down-road.jpg')} width="600px">Carrying the water tank for the gin and tonic drinks down the road to the construction site.</Image>
...
<Image src={import('./_assets/scary-tree.jpg')} width="600px">A scary-looking tree at the Luxcity event.</Image>

## Firmware

I firstly considered DMX Control (as described in a FreakLabs tutorial at http://freaklabs.org/index.php/Tutorials/Software/Light-Sequencing-and-Decoding-DMX-with-an-Arduino.html, **as of Dec 2017, URL unavailable**), but this required hacking the Arduino software platform (specifically, the HardwareSerial.cpp file) to get the UART to work correctly. I found it more elegant to use the Vixen output plugin 'Generic Serial' instead, as this worked without any hacking.

The firmware can be found in the project repo under 'src' on GitHub at [https://github.com/gbmhunter/LuxcityTonicUVControlSystem](https://github.com/gbmhunter/LuxcityTonicUVControlSystem). Since there wasn't much to it (one file of around 320 lines or so), I have also shown it below. Note that changes may have been made to the src after this code below was posted, so always use the repo on GitHub if you want to get the most recent code.
...
//!
//!  Vixen must be configured with the 'Generic Serial' output, at 57600 baud, and to send the
