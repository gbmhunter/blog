...
date: 2011-09-03
description: Capacitors, zeners, TVS diodes, MOVs, GDTs, spark gaps, series resistance and more info on ESD protection.
draft: false
...
import cmos_inputs_showing_internal_protection_diodes from './_assets/cmos-inputs-showing-internal-protection-diodes.svg'

<Image src={cmos_inputs_showing_internal_protection_diodes} width="400">Diagram of a CMOS digital I/O pin, highlighting the internal protection diodes prevalent in many designs (even if the ICs datasheet does not mention them).</Image>
...
import basic_esd_protection_with_tvs_diode from './_assets/basic-esd-protection-with-tvs-diode.svg'

<Image src={basic_esd_protection_with_tvs_diode} width="800">A good habit to get into --- place TVS diodes across inputs/outputs to a PCB (and not just power rails, although that is what is shown in this schematic). Place the TVS as close as possible to the place of entry onto the PCB.</Image>
...
import basic_esd_protection_with_tvs_diode_and_reverse_polarity_fuse from './_assets/basic-esd-protection-with-tvs-diode-and-reverse-polarity-fuse.svg'

<Image src={basic_esd_protection_with_tvs_diode_and_reverse_polarity_fuse} width="800">Using both a fuse and a TVS on a +12V power supply to a PCB. In this case, the fuse is placed before the TVS diode so that it would also blow if the +12V was hooked up the wrong way around (current going through F1 and D1).</Image>
...
import protection_diodes_to_disable_esd_diodes_on_cmos_io from './_assets/protection-diodes-to-disable-esd-diodes-on-cmos-io.png'

<Image src={protection_diodes_to_disable_esd_diodes_on_cmos_io} width="489">Adding external diodes to disable the internal ESD diodes in an IC. Image from http://www.intersil.com/content/dam/Intersil/documents/isl4/isl43l410.pdf.</Image>
...
import test_pulse_graph_8_20us from './_assets/test-pulse-graph-8-20us.svg'

<Image src={test_pulse_graph_8_20us} width="800">Transient ESD events are typically specified with two numbers, $t_1$ and $t_2$. $t_1$ is the time for the current to reach the peak value. $t_2$ is the time from the start to when the current decays to half of the peak value (as shown).</Image>
