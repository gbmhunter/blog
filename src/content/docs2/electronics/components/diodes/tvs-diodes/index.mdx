...
When the battery is very flat, the alternator is generating a large amount of current to charge the battery. When the battery is suddenly disconnected, the load on the alternator suddenly decreases. The voltage regulation circuitry which typically controls the output voltage of the alternator cannot reduce the current fast enough (because the alternator windings have a large inductance) to keep the voltage "in regulation", and thus a large voltage spike occurs[^wikipedia-load-dump].
