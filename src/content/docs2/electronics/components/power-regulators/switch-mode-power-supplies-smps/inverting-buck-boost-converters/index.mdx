...
import inverting_buck_boost_converter_basic_schematic from './_assets/inverting-buck-boost-converter-basic-schematic.svg'

<Image src={inverting_buck_boost_converter_basic_schematic} width="600">The basic schematic of a inverting buck-boost converter. SW1 is typically a MOSFET switched by control logic (not shown).</Image>
...
import vout_to_vin_vs_duty_cycle_buck_boost from './_assets/vout-to-vin-vs-duty-cycle-buck-boost.png'

<Image src={vout_to_vin_vs_duty_cycle_buck_boost} width="500">Relationship between duty cycle and the voltage ratio for a inverting buck-boost converter, ignoring the sign (so applicable for a SEPIC also). Dotted line drawn where $V_{OUT} = V_{IN}$, at $D=0.5$.</Image>
