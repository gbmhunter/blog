...
<Image src={import('./_assets/inverting-buck-boost-converter-basic-schematic.svg')} width="600">The basic schematic of a inverting buck-boost converter. SW1 is typically a MOSFET switched by control logic (not shown).</Image>
...
<Image src={import('./_assets/vout-to-vin-vs-duty-cycle-buck-boost.png')} width="500">Relationship between duty cycle and the voltage ratio for a inverting buck-boost converter, ignoring the sign (so applicable for a SEPIC also). Dotted line drawn where $V_{OUT} = V_{IN}$, at $D=0.5$.</Image>
