...
import floating_buck_boost_converter_basic_schematic from './_assets/floating-buck-boost-converter-basic-schematic.svg'

<Image src={floating_buck_boost_converter_basic_schematic} width="500">Schematic of a floating buck-boost converter. Note that $V_{IN}$ is referenced to ground, but $V_{OUT}$ is not (measured across $R_{LOAD}$ as shown).</Image>

The below image shows the current paths through the floating buck-boost converter when the switch is closed[^bib-onsemi-floating-buck-boost]. $V_{IN}$ provides a linearly increasing charging current to $L1$, whilst output capacitor $C_{OUT}$ provides current to the load. $D1$ is reverse-biased and so open-circuit.

import floating_buck_boost_converter_current_when_switch_closed from './_assets/floating-buck-boost-converter-current-when-switch-closed.svg'

<Image src={floating_buck_boost_converter_current_when_switch_closed} width="500">Current paths through the floating buck-boost converter when the switch is **closed** (during $t_{on}$).</Image>
...
import floating_buck_boost_converter_current_when_switch_open from './_assets/floating-buck-boost-converter-current-when-switch-open.svg'

<Image src={floating_buck_boost_converter_current_when_switch_open} width="500">Current paths through the floating buck-boost converter when the switch is **open** (during $t_{off}$).</Image>
