...
<Image src={import('./relay-dec-dh1u-5vdc-tv-8.jpg')} width="200px">The DEC DH1U-5VDC relay that can switch 250VAC at 15A.</Image>

<Image src={import('./relay-tyco-t9as5d12-12-12vdc-240vac.jpg')} width="350px">The Tyco T9AS5D12 relay. Coil is driven with 12VDC and switches 240VAC. Notice the two different contact ratings, 10A for the N.C. (normally closed) and 20A for the N.O. (normally open).</Image>
...
<Image src={import('./solid-state-relay-mgr-1-d4825.jpg')} width="500px">The MGR-1 D4825 solid state relay from Mager. The input takes 3-32VDC to switch the output 24-480VAC contacts at up to 25A.</Image>
...
<Image src={import('./solid-state-relay-in-schematic-with-input-resistor.png')} width="600px">This is the typical schematic symbol for a solid-state relay, along with a resistor connected to the input to limit the current through the internal LED.</Image>
...
<Image src={import('./relay-no-nc-schematic-symbol.png')} width="500px">Schematic symbol for a relay with a normally-open (NO), normally-closed (NC) and common contact.</Image>
...
<Image src={import('./flyback-diode-relay-schematic.png')} width="500px">Always add a \"flyback\" diode in anti-parallel across the coil of a relay to quench voltage spikes due the rapid di/dt through the inductor when the relay is switched of. </Image>
...
<Image src={import('./flyback-zener-relay-schematic.png')} width="500px">Adding a Zener in series with a general purpose diode as shown will improve the turn-off time and help prevent sticking, yet still provides over-voltage protection. </Image>
...
<Image src={import('./latching-relay-circuit.png')} width="400px">A simple latching relay circuit. The RESET push button can be replaced with short if you only need the circuit to reset on power off.</Image>
...
<Image src={import('./relay-in-din-socket.jpg')} width="500px">An Omron relay on a DIN mounted relay \"socket\".</Image>
