...
<Image src={require('./_assets/spi-basic-master-slave-diagram.webp').default} width="600">The basic connections needed between an SPI master and a single SPI slave.</Image>
...
<Image src={require('./_assets/cpol-and-cpha-diagram.webp').default} width="800">Diagram showing the different behaviours for different CPOL and CPHA settings.</Image>
...
<Image src={require('./_assets/example-transmission-cpol0-cpha0.webp').default} width="800">Example single-byte transfer with CPOL=0 and CPHA=0 (Mode 0). Master sends 0xA1, slave sends 0x75. Data is sampled at the dotted red vertical lines.</Image>
...
<Image src={require('./_assets/example-transmission-cpol1-cpha1.webp').default} width="800">Example single-byte transfer with CPOL=1 and CPHA=1 (Mode 3). Master sends 0xA1, slave sends 0x75. Data is sampled at the dotted red vertical lines.</Image>
...
<Image src={require('./_assets/microchip-sam-d21-spi-transfer-modes-table.png').default} width="800">Screenshot of the SPI modes table from the Microchip SAM D21 datasheet[^bib-microchip-samd21-datasheet].</Image>
...
<Image src={require('./_assets/fxos8700cq-freescale-magnetometer-note-supports-only-point-to-point-spi-protocol.png').default} width="745">The note from the Freescale FXOS8700CQ magnetometer stating that it only supports the 'point-to-point' SPI protocol[^bib-nxp-fxos8700cq-ds].</Image>
...
<Image src={require('./_assets/microchip-mcp41x1-dpot-spi-connections.png').default} width="600">Schematics from the MCP4XXX DPOT datasheet showing to connect the shared MOSI/MISO pin (called SDI/SDO) to a SPI bus and SPI host[^bib-microchip-mcp4xxxx-dpot-ds].</Image>
...
<Image src={require('./_assets/dual-spi-diagram.webp').default} width="500">A Dual SPI bus, where MOSI and MISO have been replaced with bi-directional IO0 and IO1.</Image>
...
<Image src={require('./_assets/quad-spi-diagram.webp').default} width="500">A Quad SPI (QSPI) bus, which is like a Dual SPI bus but with two additional data lines, IO2 and IO3.</Image>
...
<Image src={require('./_assets/infineon-s25fs128s-qspi-flash-connection-diagram.png').default} width="500">Wiring diagram for the Infineon S25FS128S QSPI flash IC[^bib-infineon-s25fs128s-ds].</Image>
...
<Image src={require('./_assets/ws2812b-comms-protocol-info.webp').default} width="900">Data showing how the WS2812 RGB LED communication protocol relies of the timings of HIGH and LOW parts of a bit to determine a 0 or 1[^worldsemi-ws2812b-rgb-led-ds].</Image>
