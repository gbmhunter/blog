...
<Image src={require('/images/programming-csharp/visual-studio-config-manager-window-single-image.png').default} width="600px">Making InstallShield produce a single file output install (e.g. setup.exe) in the Visual Studio configuration manager.</Image>
...
<Image src={require('/images/programming-csharp/another-version-is-installed-error-from-install-shield.png').default} width="450px">The 'Another version is installed' error that InstallShield gives you if you haven't set the updates up correctly.</Image>
...
1. You have a upgrade path.
	...
	<Image src={require('/images/programming-csharp/install-shield-adding-new-upgrade-path.png').default} width="800px">Adding a new update path, so newer versions of the program will install over older versions.</Image>
...
2. You generate a new "Product Code" everytime you want the application to update. **Note that this is different to the Product Version**, which is a smaller, user-generated number (e.g. 1.2.3), rather than the longer computer-generated Product Code.
	...
	<Image src={require('/images/programming-misc/change-product-code-not-upgrade-code-install-shield.png').default} width="700px">If you want a install to replace/upgrade and older one, make sure to re-generate a different product code (the circled number).</Image>
...
<Image src={require('/images/programming-misc/install-shield-launch-this-program-end-of-installation.png').default} width="500px">The option to allow the user to launch the program once the installation has finished. This normally doesn't work without tweaking a few settings.</Image>
