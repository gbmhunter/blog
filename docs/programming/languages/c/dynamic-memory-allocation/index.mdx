...
<Image src={require('./where-malloc-variables-get-added-heap.svg').default} width="600px">When you call malloc(), space for the variable gets reserved (allocated) on the heap, which causes the heap to grow into the unused (grey) area. This memory arrangement in representative of a ARM Cortex microcontroller.</Image>
