---
authors: [Geoffrey Hunter]
categories: [Electronics, PCB Design, Component Packages]
date: 2015-04-04
draft: false
lastmod: 2024-04-27
tags: [component packages, PCB design, EEP, SMPS, Texas Instruments, R-PDSS-T5, R-PDSS-T6]
title: EEP Component Package
type: page
---

## Overview

EEP is a component package used by Texas Instruments for some of their higher power DC/DC converters. Has a varying number of pins depending on functionality (but this does not change the footprint). The pitch is multiples of 2.54mm (100mill).

## Synonyms

* R-PDSS-T5
* R-PDSS-T6

## Photos

<Image src={require('./_assets/eep-component-package-photo.png').default} width="384px">Photo of a DC/DC converter by Texas Instruments in an EEP package.</Image>

## Dimensions

<Image src={require('./_assets/eep-dimensions.png').default} width="500px">Dimensions of the EEP package.</Image>
